...
//    Function definitions of the fcf class
#include "fcf.hh"
...
fcf<Mesh_type, data_type>& 
fcf<Mesh_type, data_type>::operator=( data_type x )
{
...
//    for assignment of members of one fcf to another
template<class Mesh_type, class data_type>
fcf<Mesh_type, data_type>& 
fcf<Mesh_type, data_type>::operator=( const fcf& rhs )
{
    ASSERT( &Mesh == &rhs.Mesh );
    ASSERT( data.size() == rhs.data.size() );
    
    for( unsigned int face = 0; face <= data.size(); face++ )
        data[face] = rhs.data[face];
...
template<class Mesh_type, class data_type>
std::vector<data_type>& fcf<Mesh_type, data_type>::
