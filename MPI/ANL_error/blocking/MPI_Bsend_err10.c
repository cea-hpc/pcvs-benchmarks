...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Bsend_err10: Message larger than attached Bsend buffer ");
    ...
    sprintf(info_buf, "about to call MPI_Bsend w/ Message larger than attached Bsend buffer ");
    ...
    dest = MPITEST_nump - 1;
    ierr = MPI_Bsend(float_send, 200 , MPI_FLOAT, dest, 1234, MPI_COMM_WORLD);
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Bsend /w  Message larger than attached Bsend buffer (%d) did not FAIL", buffsize);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Bsend /w  Message larger than attached Bsend buffer returned %d (%s), expected MPI_ERR_BUFFER",
