...

{
    ...
    sprintf(testname, "MPI_Bsend_err2: Rank > group size");
    ...
    sprintf(info_buf, "about to call MPI_Bsend w/  Rank larger than group size");
    ...
    /* Let all nodes do the Bsend out of range  */
...
/*---------------------------------  BSEND  ---------------------------------*/
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Bsend /w Destination rank larger than group size (%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Bsend /w Destination rank larger than group size returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
