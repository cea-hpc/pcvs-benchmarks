...

{
    int
        ...
        *maxtag,   	/* maximum tag value                                 */
        messtag;	/* tag value used in Rsend                           */
    ...
    sprintf(testname, "MPI_Rsend_err6:  Call with tag greater than max allowed");
     ...
     */
    sprintf(info_buf, "About to call MPI_Rsend w/  tag greater than MPI_TAG_UB attribute ( > %d)  Tag =  %d  ", *maxtag, messtag);
    ...
    /*-----------------------------------  RSEND  ---------------------------*/
    ierr = MPI_Rsend(input, 1, MPI_CHAR, dest, messtag, MPI_COMM_WORLD);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Rsend /w   Call with tag greater than MPI_TAG_UB attribute (%d/%d) did not FAIL", *maxtag, messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Rsend /w  Call with tag greater than MPI_TAG_UB attribute returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
