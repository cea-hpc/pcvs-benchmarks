...

{
    ...
    sprintf(testname, "MPI_Rsend_err7, message length count < 0");
     ...
     */
    sprintf(info_buf, "about to call MPI_Rsend w/ message length count < 0 ");
    ...
    ierr = MPI_Rsend(input, messlength, MPI_CHAR, dest, 0, MPI_COMM_WORLD);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Rsend /w negative message length count (%d) did not FAIL", messlength);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Rsend /w negative message length count returned %d, expected MPI_ERR_COUNT",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
