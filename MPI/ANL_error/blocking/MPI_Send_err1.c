...
*
* This test verifies that the correct error is returned if MPI_Send is
...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Send_err1:   Destination non-existent rank (rank negative)  ");
     ...
     */
    sprintf(info_buf, "about to call MPI_Send w/ Destination rank negative");
    ...
    ierr = MPI_Send(input, 1, MPI_CHAR, dest, 0, MPI_COMM_WORLD);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Send /w negative rank destination(%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Send /w negative rank destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
