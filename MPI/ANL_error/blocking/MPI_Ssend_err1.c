...
*
* This test verifies that the correct error is returned if MPI_Ssend is
* called with an invalid argument.
**
*
*  Buffered Blocking Ssends error tests
*  -----------------------------------
...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Ssend_err1:   Destination non-existent rank (rank negative)  ");
     ...
     */
    sprintf(info_buf, "about to call MPI_Ssend w/ Destination rank negative");
    ...
    ierr = MPI_Ssend(input, 1, MPI_CHAR, dest, 0, MPI_COMM_WORLD);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Ssend /w negative rank destination(%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Rsend /w negative rank destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
