...
/*---------------------------------------------------------------------------*/
    ...
    sprintf(testname, "MPI_Ssend_err4:  Call with a freed communicator");
    ...
    */
     ...
     */
    sprintf(info_buf, "About to call MPI_Ssend w/ Call with a freed communicator");
    ...
    ierr = MPI_Ssend(input, 1, MPI_CHAR, dest, 0, comm);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Ssend /w  Call with a freed communicator did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Ssend /w Call with a freed communicator returned %d, expected  MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
