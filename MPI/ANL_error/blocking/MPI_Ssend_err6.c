...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                          Error test for MPI_Ssend
*
*  Blocking Ssends error tests
*  -----------------------------------
...
*
*          This test will hang because Ssend is fully blocking, and the Ssend 
...
{
    int
        dest,   	/* Destination of Ssend message                      */
        ...
        *maxtag,   	/* maximum tag value                                 */
        messtag;	/* tag value used in Ssend                           */

    int
        input[10];	/* input to Ssend                                    */
    ...
    sprintf(testname, "MPI_Ssend_err6:  Call with tag greater than max allowed");
     ...
     */
    sprintf(info_buf, "About to call MPI_Ssend w/  tag greater than MPI_TAG_UB attribute ( > %d)  Tag =  %d  ", *maxtag, messtag);
    MPITEST_message(MPITEST_INFO1, info_buf);


    ierr = 0;

    /* Let all nodes do the invalid Ssend  */

    dest = MPITEST_nump - 1;

    /*-----------------------------------  SSEND  ---------------------------*/

    ierr = MPI_Ssend(input, 1, MPI_CHAR, dest, messtag, MPI_COMM_WORLD);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Ssend /w   Call with tag greater than MPI_TAG_UB attribute (%d/%d) did not FAIL", *maxtag, messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Ssend /w  Call with tag greater than MPI_TAG_UB attribute returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
