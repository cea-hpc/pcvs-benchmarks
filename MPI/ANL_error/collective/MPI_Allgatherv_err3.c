...
sprintf( testname, "MPI_Allgatherv_err3");
...
**  Invalid size
*/
sprintf(info_buf, "about to call MPI_Allgatherv w/ size = -1");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Allgatherv(input, -1, MPI_SHORT, output, recv_counts, recv_displs, MPI_SHORT, MPI_COMM_WORLD);

if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Allgatherv w/ size = -1 did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COUNT)
      {
	fail++;
	sprintf(info_buf, "MPI_Allgatherv w/ size = -1 returned %d, expected MPI_ERR_COUNT",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
