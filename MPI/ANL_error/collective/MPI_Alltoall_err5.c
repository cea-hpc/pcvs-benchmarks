...
sprintf( testname, "MPI_Alltoall_err5");
...
sprintf(info_buf, "about to call MPI_Alltoall(Free comm)");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Alltoall(input, MPITEST_nump, MPI_INT, output, MPITEST_nump, MPI_INT, comm);
if (ierr == MPI_SUCCESS)
  {
    sprintf(info_buf, "MPI_Alltoall(Free Comm) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Alltoall(Free Comm) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
