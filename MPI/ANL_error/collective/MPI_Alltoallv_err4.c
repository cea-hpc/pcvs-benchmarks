...
sprintf( testname, "MPI_Alltoallv_err4");
...
*/
sprintf(info_buf, "about to call MPI_Alltoallv(MPI_COMM_NULL)");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Alltoallv(input, send_counts, send_displs, MPI_SHORT, output,
		recv_counts, recv_displs, MPI_SHORT, MPI_COMM_NULL);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Alltoallv(MPI_COMM_NULL) did not FAIL");
  ...
  {
	fail++;
	sprintf(info_buf, "MPI_Alltoallv(MPI_COMM_NULL) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
