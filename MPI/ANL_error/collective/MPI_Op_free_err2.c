...
sprintf( testname, "MPI_Op_free_err2");
...
*/
sprintf(info_buf, "about to call MPI_Op_free w/ MPI_SUM");
...
if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Op_free w/ MPI_SUM did not FAIL");
    MPITEST_message(MPITEST_NONFATAL, info_buf);
  }
else
  {
    MPI_Error_class(ierr, &errorclass);
    if ((errorclass != MPI_ERR_OP) && (errorclass != MPI_ERR_ARG))
      {
	fail++;
	sprintf(info_buf, "MPI_OP_free w/ MPI_SUM returned %d, expected MPI_ERR_OP/ARG",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
