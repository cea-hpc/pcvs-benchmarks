...
sprintf( testname, "MPI_Reduce_err8");
...
sprintf(info_buf, "about to call MPI_Reduce(Free comm)");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Reduce(input, output, 1, MPI_INT, MPI_SUM, 0, comm);

if (ierr == MPI_SUCCESS)
  {
    sprintf(info_buf, "MPI_Reduce(Free Comm) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Reduce(Free Comm) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
