...
sprintf( testname, "MPI_Reduce_scatter_err2");
...
**  Mis-matched types:
*/
sprintf(info_buf, "about to call MPI_Reduce_scatter w/ mismatched types");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
if (MPITEST_me == 0)
    ierr = MPI_Reduce_scatter(input, output, count, MPI_INT, MPI_SUM,
	MPI_COMM_WORLD);
else
    ierr = MPI_Reduce_scatter(input, output, count, MPI_UNSIGNED, MPI_SUM,
	 MPI_COMM_WORLD);
...
if (MPITEST_nump < 2)
  ...
  {
    fail++;
    sprintf(info_buf, "MPI_Reduce_scatter w/ mismatched types did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Reduce_scatter w/ mismatched types returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
