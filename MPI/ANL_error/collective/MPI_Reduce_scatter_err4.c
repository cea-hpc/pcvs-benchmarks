...
sprintf( testname, "MPI_Reduce_scatter_err4");
...
**  Invalid count
*/
sprintf(info_buf, "about to call MPI_Reduce_scatter w/ count = -1");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
count[0] = -1;
ierr = MPI_Reduce_scatter(input, output, count, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
count[0] = 1;
...
if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Reduce_scatter w/ count = -1 did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COUNT)
      {
	fail++;
	sprintf(info_buf, "MPI_Reduce_scatter w/ count = -1 returned %d, expected MPI_ERR_COUNT",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
