...
-----------------------------------------------------------------------------*/
/*******************************************************************************
*                          Error test for MPI_Scatter()
*
* This test verifies that the correct error is returned if MPI_Scatter is
...
sprintf( testname, "MPI_Scatter_err1");
...
**  Mis-matched types:
*/
sprintf(info_buf, "about to call MPI_Scatter w/ mismatched types");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
if (MPITEST_me != 0)
    ierr = MPI_Scatter(input, 1, MPI_CHAR, output, 1, MPI_UNSIGNED_CHAR, 0,
	MPI_COMM_WORLD);
else
    ierr = MPI_Scatter(input, 1, MPI_UNSIGNED_CHAR, output, 1, MPI_CHAR, 0,
	MPI_COMM_WORLD);
...
if (MPITEST_nump < 2)
  ...
  {
    fail++;
    sprintf(info_buf, "MPI_Scatter w/ mismatched types did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Scatter w/ mismatched types returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
