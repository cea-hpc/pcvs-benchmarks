...
sprintf( testname, "MPI_Scatter_err5");
...
**  Invalid root > # nodes
*/
sprintf(info_buf, "about to call MPI_Scatter w/ root > # nodes");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Scatter(input, 1, MPI_SHORT, output, MPITEST_nump, MPI_SHORT,
	MPITEST_nump + 1, MPI_COMM_WORLD);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Scatter w/ root > # nodes did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_ROOT)
      {
	fail++;
	sprintf(info_buf, "MPI_Scatter w/ root > # nodes returned %d, expected MPI_ERR_ROOT",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
