...
sprintf( testname, "MPI_Scatterv_err7");
...
sprintf(info_buf, "about to call MPI_Scatterv(Free comm)");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Scatterv(input, send_counts, send_displs, MPI_INT, output,
	MPITEST_nump, MPI_INT, 0, comm);
if (ierr == MPI_SUCCESS)
  {
    sprintf(info_buf, "MPI_Scatterv(Free Comm) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Scatterv(Free Comm) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
