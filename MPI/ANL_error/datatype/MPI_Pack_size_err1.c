...
Rank 0 will first call MPI_Pack_size with negative input count.
The resulting erro code will then be checked and the corresponding
error class will be verified to make sure it is MPI_ERR_COUNT.

All other rank(s) will simply do nothing.

MPI Calls dependencies for this test:
  MPI_Pack_size(), MPI_Init(), MPI_Finalize()
  MPI_Error_string(), 
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Pack_size_err1: Negative incount");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Pack_size() with negative count */
      ierr2 = MPI_Pack_size(incount, type, MPI_COMM_WORLD, &size);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Pack_size() with negative incount returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Pack_size() with negative incount returned error class %d, expected MPI_ERR_COUNT", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
