...
Rank 0 will first call MPI_Pack_size with MPI_COMM_NULL.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Pack_size_err2: MPI_COMM_NULL");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Pack_size() with MPI_COMM_NULL */
      ierr2 = MPI_Pack_size(incount, type, MPI_COMM_NULL, &size);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Pack_size() with MPI_COMM_NULL returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Pack_size() with MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
