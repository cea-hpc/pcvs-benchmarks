...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_create_hvector_err1: Negative count");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Type_create_hvector() with -ve block length */
      ierr2 = MPI_Type_create_hvector(count, blklen, stride, oldtype, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_create_hvector() with input count 0 returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_create_hvector() with input count 0 returned error class %d, expected MPI_ERR_COUNT", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
