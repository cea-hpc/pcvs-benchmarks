...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Type_create_hvector()

 This test is just the original MPI_Type_hvector test, with 
 MPI_Type_create_hvector replacing MPI_Type_hvector

This test verifies that the correct error is returned if MPI_Type_create_hvector()
is called with an invalid argument.

MPI_Type_create_hvector error tests
-----------------------------------
1)  Call with negative input count.........[MPI_ERR_COUNT]
2)  Call with negative input block length..[MPI_ERR_ARG]
...
MPI Calls dependencies for this test:
  MPI_Type_create_hvector(), MPI_Init(), MPI_Finalize()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_create_hvector_err2: Negative block length");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Type_create_hvector with -ve count */
      ierr2 = MPI_Type_create_hvector(count, blklen, stride, oldtype, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_create_hvector() with negative block length returned MPI_SUCCESS", source);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_create_hvector() with negative block length returned error class %d, expected MPI_ERR_ARG", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
