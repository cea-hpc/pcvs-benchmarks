...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_free_err12: Basic (MPI_UNSIGNED) datatype");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Type_free() with Basic (MPI_UNSIGNED) datatype */
      ierr2 = MPI_Type_free(&type);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_free() with Basic (MPI_UNSIGNED) datatype returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_free() with Basic (MPI_UNSIGNED) datatype returned error class %d, expected MPI_ERR_TYPE", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
