...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Type_hindexed()

This test verifies that the correct error is returned if MPI_Type_hindexed()
is called with an invalid argument.

MPI_Type_hindexed error tests
-----------------------------------
...
MPI Calls dependencies for this test:
  MPI_Type_hindexed(), MPI_Init(), MPI_Finalize()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_hindexed_err3: MPI_DATATYPE_NULL");
	...
	displs[1] = i * extent;
      ...
      /* MPI_Type_hindexed() with MPI_DATATYPE_NULL */
      ierr2 = MPI_Type_hindexed(count, blklen, displs, oldtype, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_hindexed() with MPI_DATATYPE_NULL returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_hindexed() with MPI_DATATYPE_NULL returned error class %d, expected MPI_ERR_TYPE", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
