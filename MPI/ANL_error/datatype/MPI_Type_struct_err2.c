...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Type_struct()

This test verifies that the correct error is returned if MPI_Type_struct()
is called with an invalid argument.

MPI_Type_struct error tests
-----------------------------------
...
MPI Calls dependencies for this test:
  MPI_Type_struct(), MPI_Init(), MPI_Finalize()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_struct_err2: Negative block length");
	...
	types[i] = MPI_INT;
      ...
      /* MPI_Type_struct() with -ve block length */
      ierr2 = MPI_Type_struct(count, blklen, displs, types, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_struct() with negative block length returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_struct() with negative block length returned error class %d (%s), expected MPI_ERR_ARG", errorclass, MPITEST_GetErrName( errorclass ) );
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
