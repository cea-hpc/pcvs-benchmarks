...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Type_vector()

This test verifies that the correct error is returned if MPI_Type_vector()
is called with an invalid argument.

MPI_Type__vector error tests
-----------------------------------
...
MPI Calls dependencies for this test:
  MPI_Type_vector(), MPI_Init(), MPI_Finalize()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_vector_err2: Negative block length");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Type_vector with -ve count */
      ierr2 = MPI_Type_vector(count, blklen, stride, oldtype, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_vector() with negative block length returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_vector() with negative block length returned error class %d (%s), expected MPI_ERR_ARG", errorclass, MPITEST_GetErrName( errorclass ) );
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
