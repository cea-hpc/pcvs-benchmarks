...
-----------------------------------
1)  Call with negative insize.............[MPI_ERR_COUNT/MPI_ERR_ARG]
...
Rank 0 will first call MPI_Unpack with negative insize
...
MPI Calls dependencies for this test:
  MPI_Unpack(), MPI_Init(), MPI_Finalize()
  MPI_Error_string(), 
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Unpack_err1: Negative insize");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Unpack() with negative insize */
      ierr2 = MPI_Unpack(inbuf, insize, &position, outbuf, outcount, type, MPI_COMM_WORLD);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Unpack() with negative input size returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Unpack() with negative input size returned error class %d, expected MPI_ERR_COUNT or MPI_ERR_ARG", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
