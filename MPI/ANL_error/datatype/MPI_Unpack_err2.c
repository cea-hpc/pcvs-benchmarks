...
Rank 0 will first call MPI_Unpack with negative outcount.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Unpack_err2: Negative outcount");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Unpack() with negative outcount */
      ierr2 = MPI_Unpack(inbuf, insize, &position, outbuf, outcount, type, MPI_COMM_WORLD);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Unpack() with negative outcount returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  MPITEST_message(MPITEST_FATAL, error_string);
	   ...
	   fail++;
	  sprintf(info_buf, "MPI_Unpack() with negative outcount returned error class %d, expected MPI_ERR_COUNT", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
