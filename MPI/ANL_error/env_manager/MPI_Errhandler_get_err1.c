...

{
    ...
    sprintf(testname, "MPI_Errhandler_get_err1:  Use MPI_COMM_NULL");
     ...
     */
    sprintf(info_buf, "about to call MPI_Errhandler_get w/ MPI_COMM_NULL");
    MPITEST_message(MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Errhandler_get(MPI_COMM_NULL, &errh);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Errhandler_get /w MPI_COMM_NULL did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Errhandler_get /w MPI_COMM_NULL returned %d, expected MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
