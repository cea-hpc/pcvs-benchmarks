...
Rank 0 will call MPI_Attr_get with MPI_COMM_NULL.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Attr_get_err1: MPI_COMM_NULL");
    ...
    if (MPITEST_me == 0) {
      ...
      sprintf(info_buf, "Calling MPI_Attr_get with MPI_COMM_NULL");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Attr_get(MPI_COMM_NULL, keyval, &attribute_val, &flag);
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Attr_get() with MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
