...
Rank 0 will call MPI_Comm_create with MPI_COMM_NULL.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Comm_create_err1: MPI_COMM_NULL");
    ...
    comm = MPI_COMM_NULL;
    /* group */
    ierr = MPI_Comm_group(MPI_COMM_SELF, &group);
    if (ierr != MPI_SUCCESS) {
       fail++;
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Comm_create with MPI_COMM_NULL */
      sprintf(info_buf, "Calling MPI_Comm_create with MPI_COMM_NULL");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Comm_create() with MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
