...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Comm_split()

This test verifies that the correct error is returned if MPI_Comm_split()
is called with invalid arguments.

MPI_Comm_split error tests
-----------------------------------
1)  Call with MPI_COMM_NULL..............[MPI_ERR_COMM]

In all cases, expect to receive appropriate error.

Rank 0 will call MPI_Comm_split with MPI_COMM_NULL.
...
MPI Calls dependencies for this test:
  MPI_Comm_split(), MPI_Init(), MPI_Finalize()
  MPI_Error_string()
  [MPI_Allreduce(), MPI_Comm_split(), MPI_Comm_split()]
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Comm_split_err1: MPI_COMM_NULL");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Comm_split with MPI_COMM_NULL */
      sprintf(info_buf, "Calling MPI_Comm_split with MPI_COMM_NULL");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Comm_split(MPI_COMM_NULL, 0, 0, &newcomm);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Comm_split() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Comm_split() with MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
