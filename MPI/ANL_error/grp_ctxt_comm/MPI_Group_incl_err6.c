...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Group_incl()

This test verifies that the correct error is returned if MPI_Group_incl()
is called with invalid arguments.

MPI_Group_incl error tests
-----------------------------------
...
6)  Call with duplicate ranks........................[MPI_ERR_OTHER/MPI_ERR_ARG/
MPI_ERR_RANK]

In all cases, expect to receive appropriate error.

Rank 0 will call MPI_Group_incl with duplicate ranks.
The resulting error code will then be checked and the corresponding
error class will be verified to make sure it is MPI_ERR_OTHER, MPI_ERR_ARG,
or MPI_ERR_RANK.

All other rank(s) will simply do nothing.

MPI Calls dependencies for this test:
  MPI_Group_incl(), MPI_Init(), MPI_Finalize()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_incl_err6: call with duplicate ranks");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_incl with duplicate ranks */
      sprintf(info_buf, "Calling MPI_Group_incl with duplicate ranks");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Group_incl(group, n, ranks, &newgroup);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Group_incl() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	else if ((errorclass != MPI_ERR_OTHER) && (errorclass != MPI_ERR_ARG) && (errorclass != MPI_ERR_RANK)) {
	  fail++;
          sprintf(info_buf, "MPI_Group_incl() with duplicate ranks returned error class %d, expected MPI_ERR_OTHER, MPI_ERR_ARG, or MPI_ERR_RANK", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
