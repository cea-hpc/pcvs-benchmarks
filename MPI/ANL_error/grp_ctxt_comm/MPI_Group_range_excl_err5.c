...
Rank 0 will call MPI_Group_range_excl with negative stride and first<last.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_range_excl_err5: negative stride and first<last");
    ...
    n = 1;
    /* ranges */
    ranges[0][RANGES_FIRST] = 0;
    ranges[0][RANGES_LAST] = MPITEST_nump - 1;
    ranges[0][RANGES_STRIDE] = -1;

    if (MPITEST_me == 0) {
      /* Calling MPI_Group_range_excl with negative stride and first<last */
      sprintf(info_buf, "Calling MPI_Group_range_excl with negative stride and first<last");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_range_excl() with negative stride and first<last returned error class %d, expected MPI_ERR_OTHER or MPI_ERR_ARG", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
