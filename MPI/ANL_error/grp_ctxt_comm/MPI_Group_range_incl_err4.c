...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_range_incl_err4: out of rangout of range ranks implied in ranges[]");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_range_incl with out of range ranks implied in ranges[] */
      sprintf(info_buf, "Calling MPI_Group_range_incl with out of range ranks implied in ranges[]");
	...
	/* Since it is the range array that has invalid entries, 
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_range_incl() with out of range ranks implied in ranges[] returned error class %d, expected MPI_ERR_RANK", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
