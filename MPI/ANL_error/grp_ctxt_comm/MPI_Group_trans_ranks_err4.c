...
Rank 0 will call MPI_Group_translate_ranks with a negative argument
in ranks1.  The resulting error code will then be checked and the
corresponding error class will be verified to make sure it is
MPI_ERR_RANK.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_translate_ranks_err4: negative rank");
    ...
    /* ranks1 */
    ranks1[0] = -1;
    /* Make this value distinct from MPI_PROC_NULL (which MPI-2 clarified
       was allowed as an argument to Group_translate_ranks) */
    if (MPI_PROC_NULL == -1) ranks1[0] = -2;

    if (MPITEST_me == 0) {
      /* Calling MPI_Group_translate_ranks with bad arguments */
      sprintf(info_buf, "Calling MPI_Group_translate_ranks with a negative argument in ranks1");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_translate_ranks() with negative rank returned error class %d, expected MPI_ERR_RANK", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
