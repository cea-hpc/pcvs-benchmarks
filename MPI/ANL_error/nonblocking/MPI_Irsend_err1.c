...
*
* This test verifies that the correct error is returned if MPI_Irsend is
...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Irsend_err1:   Destination non-existent rank (rank negative)  ");
     ...
     */
    sprintf(info_buf, "about to call MPI_Irsend w/ Destination rank negative");
    ...
    /* Let all nodes do the invalid Irsend  */
    ...
    ierr = MPI_Irsend(input, 1, MPI_CHAR, dest, 0, MPI_COMM_WORLD, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Irsend /w negative rank destination(%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Irsend /w negative rank destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
