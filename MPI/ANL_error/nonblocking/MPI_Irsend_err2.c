...

{
    ...
    sprintf(testname, "MPI_Irsend_err2: Rank > group size");
     ...
     */
    sprintf(info_buf, "about to call MPI_Irsend w/  Rank larger than group size");
...
/*---------------------------------  Irsend  --------------------------------*/
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Irsend /w Destination rank larger than group size (%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Irsend /w Destination rank larger than group size returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
