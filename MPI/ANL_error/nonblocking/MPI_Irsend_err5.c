...

{
    ...
    sprintf(testname, "MPI_Irsend_err5:  Call with tag < 0");
    ...
    /* We only need 1 node to attempt an invalid Irsend  */
    if (MPITEST_nump < 1)
    {
	sprintf(info_buf, "Not enough nodes to test Irsend");
	MPITEST_message(MPITEST_FATAL, info_buf);
    }
     ...
     */
    sprintf(info_buf, "About to call MPI_Irsend w/  Call with tag less than 0");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Irsend /w   Call with tag less than 0  (%d) did not FAIL", messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Irsend /w  Call with tag less than 0 returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
