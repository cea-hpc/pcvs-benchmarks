...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                          Error test for MPI_Issend
*
*  Buffered Asynchronous(non-blocking) Issend error tests
*  --------------------------------------------------------
...
*
*          This test will hang because Issend is fully blocking, and the Issend 
*          does not return until the receive has started.  If an MPI_Recv is
*          posted (04/96) the program will complete, showing that a message
*          tag greater than allowed is not detected.
******************************************************************************/
...
{
    int
        dest,   	/* Destination of Issend message                     */
        ...
        *maxtag,   	/* maximum tag value                                 */
        messtag;	/* tag value used in Issend                          */
    int
        input[10];	/* input to Issend                                   */
    ...
    sprintf(testname, "MPI_Issend_err6:  Call with tag greater than max allowed");
     ...
     */
    sprintf(info_buf, "About to call MPI_Issend w/  tag greater than MPI_TAG_UB attribute ( > %d)  Tag =  %d  ", *maxtag, messtag);
    MPITEST_message(MPITEST_INFO1, info_buf);


    ierr = 0;

    /* Let all nodes do the invalid Issend  */

    dest = MPITEST_nump - 1;

    /*----------------------------------  Issend  ---------------------------*/

    ierr = MPI_Issend(input, 1, MPI_CHAR, dest, messtag, MPI_COMM_WORLD, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Issend /w   Call with tag greater than MPI_TAG_UB attribute (%d/%d) did not FAIL", *maxtag, messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Issend /w  Call with tag greater than MPI_TAG_UB attribute returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
