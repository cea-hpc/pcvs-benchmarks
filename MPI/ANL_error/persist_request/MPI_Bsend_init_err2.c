...

{
    ...
    sprintf(testname, "MPI_Bsend_init_err2: Rank > group size");
    ...
    sprintf(info_buf, "about to call MPI_Bsend_init w/  Rank larger than group size");
    ...
    /* Let all nodes do the Bsend_init out of range  */
...
/*--------------------------------  Bsend_init  ---------------------------------*/
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Bsend_init /w Destination rank larger than group size (%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Bsend_init /w Destination rank larger than group size returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
