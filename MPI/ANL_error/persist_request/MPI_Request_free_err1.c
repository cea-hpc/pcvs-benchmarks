...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                          Error test for MPI_Request_free
*
* This test verifies that the correct error is returned if MPI_Request_free
* called with an invalid argument.
*
*  MPI_Request_free error tests
*  --------------------------------------------------------
* 1)  Freed request ..................................... [MPI_ERR_REQUEST]
*-------------------------------------------------------------------------
*
...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Request_free_err2:  Request freed");
     ...
     */
    sprintf(info_buf, "about to call MPI_Send_init ");
    ...
    dest = MPITEST_me;
    ierr = MPI_Send_init(input, 20, MPI_INT, dest, 0, MPI_COMM_WORLD, &request);

    if (ierr != MPI_SUCCESS)
    {
        sprintf(info_buf, "Valid MPI_Send_init returned %d", ierr);
    ...
    /*
     *  Do another free; expect failure.
    */
    ierr = MPI_Request_free(&request);
    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Request_free on freed request did not FAIL");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
    }
    else
    {
        ...
        {
	    fail++;
	    sprintf(info_buf, "MPI_Request_free on freed request returned %d, expected MPI_ERR_REQUEST",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
