...
*
* This test verifies that the correct error is returned if MPI_Ssend_initis
...
{
    int
        dest,	        /* Destination of Ssend_initmessage                  */
    ...
    */
    ...
    sprintf(testname, "MPI_Ssend_initerr1:   Destination non-existent rank (rank negative)  ");
     ...
     */
    sprintf(info_buf, "about to call MPI_Ssend_init w/ Destination rank negative");
    ...
    ierr = MPI_Ssend_init(input, 1, MPI_CHAR, dest, 0, MPI_COMM_WORLD, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Ssend_init w/ negative rank destination(%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Ssend_init /w negative rank destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
