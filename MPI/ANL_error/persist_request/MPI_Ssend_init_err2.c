...

{
    ...
    sprintf(testname, "MPI_Ssend_init_err2: Rank > group size");
     ...
     */
    sprintf(info_buf, "about to call MPI_Ssend_init w/  Rank larger than group size");
...
/*---------------------------------  Ssend_init  --------------------------------*/
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Ssend_init /w Destination rank larger than group size (%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Ssend_init /w Destination rank larger than group size returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
