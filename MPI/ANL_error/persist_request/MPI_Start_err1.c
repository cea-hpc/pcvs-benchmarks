...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Start_err1:  Request already active");
    ...
    if (MPITEST_nump < 2)
    {
	sprintf(info_buf, "Not enough ranks to test MPI_Start on already active request");
	MPITEST_message(MPITEST_FATAL, info_buf);
    }
	...
	/*
	 *  Do a valid start; expect success.
	*/
	ierr = MPI_Start(&request);
	if (ierr != MPI_SUCCESS)
	{
	    sprintf(info_buf, "Valid MPI_Start returned %d", ierr);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
	    MPITEST_message(MPITEST_FATAL, error_string);
	}
	...
	*/
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Start on already active request did not FAIL");
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	}
	else
	{
	    ...
	    {
		fail++;
		sprintf(info_buf, "MPI_Start on already active request returned %d, expected MPI_ERR_REQUEST",
			errorclass);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		MPI_Error_string(ierr, error_string, &size);
