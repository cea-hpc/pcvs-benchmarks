...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Iprobe_err5: too large source rank (%d)", source);
    if (MPITEST_me == 0) {
	...
	tag = 1;
       /* MPI_Iprobe call with the too large source rank */
       sprintf(info_buf, "Calling MPI_Iprobe() with too large source rank (%d) ...", source);
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Iprobe() with too large source rank (%d) returned MPI_SUCCESS", source);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
       }
       else {
	  ...
	  if (ierr != MPI_SUCCESS) {
	     ...
	     fail++;
	     sprintf(info_buf, "MPI_Iprobe() /w too large source rank (%d) returned error class %d, expected MPI_ERR_RANK",
		     source, errorclass);
	     MPITEST_message(MPITEST_NONFATAL, info_buf);
