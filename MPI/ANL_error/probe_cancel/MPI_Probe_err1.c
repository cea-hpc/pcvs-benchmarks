...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    source = MPI_ANY_SOURCE * 3; /* bad source rank */
    if (source > 0) source = -1;

    sprintf(testname, "MPI_Probe_err1: source rank negative (%d)", source);
    ...
    if (ierr != MPI_SUCCESS) {
       ...
       tag = 1;
       /* MPI_Probe call with the negative source rank */
       sprintf(info_buf, "Calling MPI_Probe() with negative source rank (%d) ...", source);
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Probe() with negative source rank (%d) returned MPI_SUCCESS", source);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
       }
       else {
	  ...
	  if (ierr != MPI_SUCCESS) {
	     ...
	     fail++;
	     sprintf(info_buf, "MPI_Probe() /w negative source rank (%d) returned error class %d, expected MPI_ERR_RANK",
		     source, errorclass);
	     MPITEST_message(MPITEST_NONFATAL, info_buf);
