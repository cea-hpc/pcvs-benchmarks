...

{
    ...
    MPI_Comm 
      ...
      comm2;      	/* MPI communicators                                 */

    MPI_Status
        stat;           /* MPI_recv status                                   */
/*---------------------------------------------------------------------------*/
    ...
    sprintf(testname, "MPI_Sendrecv_err4:  Call with a freed communicator");
...
/*----------------------  Allocate and Free communicator -------------------*/
     ...
     */
    sprintf(info_buf, "About to call MPI_Sendrecv w/ Call with a freed communicator");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv /w  Call with a freed communicator did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv /w Call with a freed communicator returned %d, expected  MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
