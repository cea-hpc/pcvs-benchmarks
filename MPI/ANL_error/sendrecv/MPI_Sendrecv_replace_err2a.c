...

{
    ...
    sprintf(testname, "MPI_Sendrecv_replace_err2a: SEND Rank > group size");
     ...
     */
    sprintf(info_buf, "about to call MPI_Sendrecv_replace w/ SEND Rank larger than group size");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv_replace /w SEND rank larger than group size (%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv_replace /w SEND rank larger than group size returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
