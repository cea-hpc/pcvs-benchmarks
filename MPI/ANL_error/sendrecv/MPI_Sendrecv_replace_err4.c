...
/*---------------------------------------------------------------------------*/
    ...
    sprintf(testname, "MPI_Sendrecv_replace_err4:  Call with a freed communicator");
...
/*----------------------  Allocate and Free communicator -------------------*/
     ...
     */
    sprintf(info_buf, "About to call MPI_Sendrecv_replace w/ Call with a freed communicator");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv_replace /w  Call with a freed communicator did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv_replace /w Call with a freed communicator returned %d, expected  MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
