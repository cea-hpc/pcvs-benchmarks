...

{
    ...
    sprintf(testname, "MPI_Sendrecv_replace_err5a:  Call with SEND tag < 0");
     ...
     */
    sprintf(info_buf, "About to call MPI_Sendrecv_replace w/  Call with SEND tag less than 0");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv_replace /w   Call with SEND tag less than 0  (%d) did not FAIL", messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv_replace /w  Call with SEND tag less than 0 returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
