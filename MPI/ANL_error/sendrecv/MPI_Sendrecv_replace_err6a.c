...

{
    int
        i,              /* Utility index                                     */
        buffsize,	/* Size, in bytes, of Sendrecv_replace buffer        */
        ...
        *maxtag,   	/* maximum tag value                                 */
        messtag;	/* tag value used in Sendrecv_replace                */
   ...
   /*------------------------------------------------------------------------*/
    ...
    sprintf(testname, "MPI_Sendrecv_replace_err6a:  Call with SEND tag greater than max allowed");
     ...
     * an iobuffer parameter to the MPI_Attr_get function to get the actual upper
     ...
     */
    sprintf(info_buf, "About to call MPI_Sendrecv_replace w/ SEND tag greater than MPI_TAG_UB attribute ( > %d)  Tag =  %d  ", *maxtag, messtag);
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv_replace w/ Call with SEND tag greater than MPI_TAG_UB attribute (%d/%d) did not FAIL", *maxtag, messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv_replace /w  Call with SEND tag greater than MPI_TAG_UB attribute returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
