...

{
    ...
    int
        iobuffer[10];	/* iobuffer for Sendrecv_replace                     */
    ...
    sprintf(testname, "MPI_Sendrecv_replace_err8a, Non-existant SEND data type <MPI_DATATYPE_NULL> ");
    ...
    sprintf(info_buf, "about to call MPI_Sendrecv_replace w/ non-existant data type <MPI_DATATYPE_NULL>");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv_replace /w  non-existant SEND data type <MPI_DATATYPE_NULL>  did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv_replace /w  non-existant SEND data type <MPI_DATATYPE_NULL> returned %d, expected MPI_ERR_TYPE",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
