...
* 3)  Call with Cartesian communicator .................. [COMM/TOPOLOGY]

****************************************************************************/
#define  DIMENSIONS      2
#define  TEST_DESC    "MPI_Graphdims_get called with Cartesian communicator"
...
int main(int argc, char *argv[])
{
   int
      ...
      graph_nodes,		/* number of nes in graph communicator               */
   ...
   sprintf(testname, "Graphdims_get_err3:  called with Cartesian communicator ");
   ...
   if (comm_cart != MPI_COMM_NULL) {
	 ...
	 ierr = MPI_Graphdims_get(comm_cart,
				  &graph_nodes,
				  &nedges);
	 ...
	 if (ierr == MPI_SUCCESS) {
	    ...
	    MPI_Error_class(ierr, &errorclass);
	    /* 10-31-02 - This is the wrong (absent) topology, so ERR_TOPOLOGY
	       is a reasonable error class */
