...
int main(int argc, char *argv[])
{
	...
	char error_string[MPI_MAX_ERROR_STRING];
	  ...
	  recv_stat;	    /* MPI  status structure                         */
        ...
        signed char *buff;  /* Bsend message buffer                          */
    ...
    sprintf(testname, "MPI_Bsend_rtoaq: Root TO All Quality of implementation");
	...
	if (MPITEST_current_rank != MPI_UNDEFINED)
	{
		...
		/*------------- Allocate buffer for BSEND ---------------*/
                ...
                MPI_Pack_size(max_length, MPITEST_mpi_datatypes[test_type],
                        comm, &ps);
		buffsize = 2*(MPI_BSEND_OVERHEAD + ps);
		buff = (signed char *)malloc(buffsize * sizeof (char));
		if(!buff)
		{
			...
			for (root = 0; root < senders; root++)
			{
				...
				for (i = 0; i < send_to; i++)
				{
                                    ...
                                    while (errc == MPI_ERR_BUFFER)
				    {
					  ...
					  /* This code depends on knowing the 
					     error class for buffer space 
					     not available.  This isn't 
					     defined in the MPI standard */
					  if (errc != MPI_ERR_BUFFER) 
					  {
					    sprintf(info_buf, "Non-zero return code (%d,%s) from MPI_Bsend", ierr,MPITEST_GetErrName( errc ));
