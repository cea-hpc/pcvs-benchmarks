...
sleeps for 15 seconds, then begins to read and validate the messages
from rank 1.
...
WDG - 2/2/04 - change sleep to 15 seconds from 90.  The value 90 was chosen
when machines were nearly 100 times slower than they are in 2004.
******************************************************************************/
...
/* 		    if ((flag == 0) && (loop_fail > SPIN)) */
/* 			sleep(1); */
		    if (flag == 1)
			loop_fail = LOOP_SIZE;
		    if (loop_fail == (SPIN + 30))
		    {
	...
	{ /* rank 1 does sends */
		...
		ierr = MPI_Send(send_buffer, 0, MPI_INT, 0, 1, MPI_COMM_WORLD);
		if (ierr != MPI_SUCCESS)
		{
