	 ...
	 for (root = 0; root < test_nump; root++) {
	    ...
	    if (MPITEST_current_rank == root) {
		  ...
		  fail++;
	       ...
	       sprintf(info_buf, "Root begin received post-barrier messages");
	       MPITEST_message(MPITEST_INFO2, info_buf);
	    ...
	    /* to root */
	       ...
	       sprintf(info_buf, "Entering barrier root");
	       MPITEST_message(MPITEST_INFO2, info_buf);
	       ierr = MPI_Barrier(comm);
	       sprintf(info_buf, "Entering barrier done root");
	       MPITEST_message(MPITEST_INFO2, info_buf);
	       if (ierr != MPI_SUCCESS) {
		  sprintf(info_buf, "MPI_Barrier() returned %d", ierr);
		  MPITEST_message(MPITEST_NONFATAL, info_buf);
		  ...
		  fail++;
	       ...
	       ierr = MPI_Send(buffer, count, MPI_INT, root, POST_BARRIER_TAG,
			       comm);
	       if (ierr != MPI_SUCCESS) {
