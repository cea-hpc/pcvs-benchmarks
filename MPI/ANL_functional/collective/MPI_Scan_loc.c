	 ...
	 for (type_count = 0; type_count < MPITEST_num_datatypes(); type_count++) {
	    ...
	    for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++) {
		  ...
		  if (has_op(op_count, test_type) && (length != 0)) {
		     loop_cnt++;
		     ierr = MPI_Scan(send_buffer, recv_buffer, length,
				     localtype,
				     MPITEST_default_ops[op_count],
				     comm);

		     if (ierr != MPI_SUCCESS) {
			sprintf(info_buf, "MPI_Scan() returned %d", ierr);
			MPITEST_message(MPITEST_NONFATAL, info_buf);
			MPI_Error_string(ierr, error_string, &size);
			MPITEST_message(MPITEST_FATAL, error_string);
			fail++;
		      ...
		      */
		     get_reduce_answer(op_count, MPITEST_current_rank + 1,
				       &value);

		     /* error test */
		     error = 0;
		     error = MPITEST_buffer_errors_loc(test_type, length,
						       value, recv_buffer);

		     if (error) {
			if (ierr == MPI_SUCCESS)
			   fail++;
			sprintf(info_buf, "%d errors in buffer, length %d commsize %d commtype %d data_type %d operation %d",
			...
			sprintf(info_buf, "%d errors on operation %d",
				error, op_count);
