...
********************************************************************/
{
  ...
  switch (context)
      ...
      if (comm1 != MPI_COMM_NULL)
	{
	    ...
	    /* printf( "Sizeof comm1 = %d,%d for index %d\n", s1,s2,index ); */
...
/* The following provides a way to convert numeric MPI error classes into a
   convenient string for printing */
typedef struct { int errClass; const char *errName; } ErrNames;

static ErrNames classToName[] = { 
...
/* Some MPI implementations do not include these (e.g., IBM) */
#ifdef MPI_ERR_NAME
...
{
  int i = 0;
  for (i=0; classToName[i].errClass >= 0; i++) {
    if (classToName[i].errClass == errClass) return classToName[i].errName;
  }
