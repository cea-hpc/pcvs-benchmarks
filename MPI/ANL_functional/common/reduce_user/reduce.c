...
/*   fprintf(stderr,"addop type %d size %d\n",*dtype,*len); */
...
{
  int i;
/*   fprintf(stderr,"incop type %d size %d\n",*dtype,*len); */
  for (i=0; i<*len; i++ )  {
	if (*dtype == MPI_INT)
	  {
/* 	    fprintf(stderr,"B incop %d %d->%d\n",i,((int *)invec)[i] ,((int *)inoutvec)[i]); */
	    if ( (int)((int *)inoutvec)[i] > (int)((int *)invec)[i] )
	      ((int *)inoutvec)[i] = ((int *)invec)[i];
	    else
	      ((int *)inoutvec)[i] = (int)(MPITEST_nump+2);
/* 	    fprintf(stderr,"A incop %d %d->%d\n",i,((int *)invec)[i] ,((int *)inoutvec)[i]); */
	  }
	else if (*dtype == MPI_SHORT)
	  {
