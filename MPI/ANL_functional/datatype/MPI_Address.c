   ...
   for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	 ...
	 for (i = 0; i < length; i++) {
	    ...
	    if (ierr != MPI_SUCCESS) {
		  ...
		  fail++;
		  sprintf(info_buf, "Returned address (" MPITEST_AINT_FMT_DEC_SPEC ") from MPI_Address is less than MPI_BOTTOM (" MPITEST_AINT_FMT_DEC_SPEC ")", out_addr, (MPI_Aint)MPI_BOTTOM);
		  MPITEST_message(MPITEST_NONFATAL, info_buf);
	       }
	    }
	 }
