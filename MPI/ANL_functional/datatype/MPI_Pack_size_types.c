...
int main(int argc, char *argv[])
{
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 if (MPITEST_current_rank == 0) {
	    for (type_count = 0; type_count < num_types; type_count++) {
	       sprintf(info_buf, "blklens[%d] = %d, displs[%d] = " MPITEST_AINT_FMT_DEC_SPEC
		       ", types[%d] = %d, type_sizes[%d] = " MPITEST_AINT_FMT_DEC_SPEC,
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    sprintf(info_buf, "Calling MPI_Pack_size on user datatype");
	    MPITEST_message(MPITEST_INFO1, info_buf);
	    ...
	    if (ierr != MPI_SUCCESS) {
	       ...
	       fail++;
	       sprintf(info_buf, "(incount = %d, mpi type = %d, comm = %d) -> size = %d, MPI_Pack_size() returned negative size (%d)", length, newtype2, comm_index, pack_size, pack_size);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	    }
	    else {
	       sprintf(info_buf, "(incount = %d, mpi type = %d, comm = %d) -> size = %d",
