...
All rank will call MPI_Type_create_hvector() and then MPI_Type_commit() to create
...
The input count to MPI_Type_create_hvector() is determined by each message length
in mpitest_cfg.h and the input oldtype to MPI_Type_create_hvector() is determined
...
int main(int argc, char *argv[])
{
   ...
   sprintf(testname, "MPI_Type_create_hvector_basic");
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (type_count=0; type_count < MPITEST_num_datatypes(); type_count++) {
	    ...
	    ierr = MPI_Type_extent(MPITEST_mpi_datatypes[test_type], &stride);
	    if (ierr != MPI_SUCCESS) {
	       fail++;
	    ...
	    if (max_length < stride) max_length = stride;
	    ...
	    for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	       ...
	       ierr = MPI_Type_create_hvector(length, 1, stride, MPITEST_mpi_datatypes[test_type], &newtype);
	       if (ierr != MPI_SUCCESS) {
		  fail++;
