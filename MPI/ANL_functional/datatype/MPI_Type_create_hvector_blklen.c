...
(in mpitest_cfg.h) using MPI_Type_create_hvector() using length as blklen value.
...
/*   struct dataTemplate
   ...
   sprintf(testname, "MPI_Type_create_hvector_blklen");
...
/*********** This code was added by Debbie Swider 9/17/97 ****************/
	    ...
	    /* Because (length)*(length)*(extent) is used to determine
	       ...
	       than the length of the buffer.  If it is - reduce the length
            ...
            if ( ((length)*(length)*(extent))  > (max_length + MPITEST_ADD_BUFSIZE) ) 
...
/****************************************************************************/
	    ...
	    ierr = MPI_Type_create_hvector(length, length, length * extent, oldtype, &newtype);
	    if (ierr != MPI_SUCCESS) {
	       fail++;
