...
All rank will call MPI_Type_create_struct() and then MPI_Type_commit() to create
...
The input count to MPI_Type_create_struct() is determined by each message length
in mpitest_cfg.h and the input oldtype to MPI_Type_create_struct() is determined
...
int main(int argc, char *argv[])
{
   ...
   sprintf(testname, "MPI_Type_create_struct_basic");
	 ...
	 for (type_count=0; type_count < MPITEST_num_datatypes(); type_count++) {
	    ...
	    for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
		  ...
		  MPITEST_message(MPITEST_FATAL, info_buf);
	       ...
	       types = (MPI_Datatype *) calloc(length, sizeof(MPI_Datatype));
	       if (!types) {
		  sprintf(info_buf, "Cannot allocate enough space for types array");
		  MPITEST_message(MPITEST_FATAL, info_buf);
	       }
		  ...
		  types[i] = MPITEST_mpi_datatypes[test_type];
	       ...
	       ierr = MPI_Type_create_struct(length, blklens, displs, types, &newtype);
	       if (ierr != MPI_SUCCESS) {
		  fail++;
