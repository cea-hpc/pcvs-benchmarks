...
(in mpitest_cfg.h) using MPI_Type_create_struct() using length as displacement value.
...
MPI Calls dependencies for this test:
  ...
  MPI_Type_create_struct(), MPI_Type_create_struct(), MPI_Type_commit(),
...
/*   struct dataTemplate
   ...
   sprintf(testname, "MPI_Type_create_struct_blklen");
...
/****************************************************************************/
	    ...
	    ierr = MPI_Type_create_struct(length, blklens1, displs1, types1, &newtype);
	    if (ierr != MPI_SUCCESS) {
	       fail++;
	       sprintf(info_buf, "MPI_Type_create_struct() returned %d", ierr);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
