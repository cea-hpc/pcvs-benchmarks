		...
		for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++)
		{
			...
			for (root = 0; root < receivers; root++)
			{
				...
				for (i = 0; i < get_from; i++)
				{
				    ...
				    while (!flag && !ierr) 
				    {
					ierr = MPI_Test(&recv_request, &flag, &recv_stat);
					recv_loop_cnt++;
				    }

				    if (ierr != MPI_SUCCESS)
				    {
					sprintf(info_buf, "Non-zero return code ierr/flag (%d/%d) from MPI_Test on Irecv, after %d calls to MPI_Test", ierr, flag, recv_loop_cnt);
					MPITEST_message(MPITEST_NONFATAL, info_buf);
					MPI_Error_string(ierr, error_string, &size);
					MPITEST_message(MPITEST_NONFATAL, error_string);
					MPITEST_message(MPITEST_NONFATAL, info_buf);
