...
int main(int argc, char *argv[])
{
	...
	if ((buf2[0] != 1 - MPITEST_me) ||
	    ...
	    (buf2[2] != 1 - MPITEST_me))
	{
	    sprintf(info_buf, "Received data = %d,%d,%d, expected %d,%d,%d",
		buf1[0], buf1[1], buf1[2], buf2[0], buf2[1], buf2[2]);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    fail = 1;
	}
	...
	if (ierr != MPI_SUCCESS)
	{
	    sprintf(info_buf, "Non-zero return code (%d) from MPI_Wait on Issend", ierr);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
