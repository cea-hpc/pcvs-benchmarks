...
int main(int argc, char *argv[])
{
	...
	send_stat[NUMSHORT+1];	  /* MPI  status structure                  */
	...
	{	/* sender */
	    ...
	    if (ierr != MPI_SUCCESS)
	    {
		sprintf(info_buf, "Non-zero return code (%d) from MPI_Startall for send", ierr);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		MPI_Error_string(ierr, error_string, &size);
		MPITEST_message(MPITEST_NONFATAL, error_string);
		fail++;
	    ...
	    ierr = MPI_Waitall(NUMSHORT+1, &send_request[0], &send_stat[0]);
	...
	{	/* receiver */
	    ...
	    for (i = 0; i < NUMSHORT; i++)
	    {
		ierr = MPI_Recv_init(&shortb[i], LONGLENGTH, MPI_CHAR, 0, 1,
	    ...
	    if (ierr != MPI_SUCCESS)
	    {
		sprintf(info_buf, "Non-zero return code (%d) from MPI_Startall for receive", ierr);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		MPI_Error_string(ierr, error_string, &size);
		MPITEST_message(MPITEST_NONFATAL, error_string);
		fail++;
