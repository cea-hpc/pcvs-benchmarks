   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) {
	    ...
	    for (root=0; root<test_nump; root++) {
		  ...
		  sprintf(info_buf, "Constructing receive request from %d, tag: %d", source, tag);
		  MPITEST_message(MPITEST_INFO1, info_buf);

		  ierr = MPI_Recv_init(buffer, length, MPITEST_mpi_datatypes[test_type],
				   source, tag, comm, &recv_request);

		  if (ierr != MPI_SUCCESS) {
		     fail++;
		     sprintf(info_buf, "MPI_Recv_init() returned %d", ierr);
		     MPITEST_message(MPITEST_NONFATAL, info_buf);
		     MPI_Error_string(ierr, error_string, &size);
		     MPITEST_message(MPITEST_FATAL, error_string);
		  }

		  sprintf(info_buf, "Starting receive request from %d, tag: %d", source, tag);
		  MPITEST_message(MPITEST_INFO1, info_buf);
		  ...
		  if (ierr != MPI_SUCCESS) {
		     fail++;
		     sprintf(info_buf, "MPI_Start() returned %d", ierr);
		     MPITEST_message(MPITEST_NONFATAL, info_buf);
		     MPI_Error_string(ierr, error_string, &size);
		     MPITEST_message(MPITEST_FATAL, error_string);
		  }
		     ...
		     sprintf(info_buf, "Cancelling persistent receive request ...");
		     MPITEST_message(MPITEST_INFO1, info_buf);
			...
			sprintf(info_buf, "Waiting for receive request to be cancelled");
			MPITEST_message(MPITEST_INFO1, info_buf);
			...
			sprintf(info_buf, "Checking if the receive request has been cancelled");
			MPITEST_message(MPITEST_INFO1, info_buf);
		  ...
		  sprintf(info_buf, "Receiving message 2 from %d, tag: %d",
			  source, tag);
		  MPITEST_message(MPITEST_INFO1, info_buf);

		  ierr = MPI_Irecv(buffer, length, MPITEST_mpi_datatypes[test_type],
				   source, tag, comm, &recv_request);
		  ...
		  if (ierr != MPI_SUCCESS) {
			...
			if (count != length) {
			   fail++;
			   sprintf(info_buf, "status object returned from MPI_Wait() contains unexpected length."
