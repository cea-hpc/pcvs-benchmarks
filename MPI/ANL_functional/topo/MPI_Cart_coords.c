   ...
   for (comm_count = 0; comm_count < MPITEST_num_comm_sizes(); comm_count++) {
	 ...
	 if (!inter_comm && test_nump >= MPITEST_MINNODE) {
	    ...
	    if (rank_cart == 0) {
		  ...
		  if (ierr != MPI_SUCCESS) {
		     fail++;
		     sprintf(info_buf, "MPI_Cart_coords error (%d)", ierr);
		     MPITEST_message(MPITEST_NONFATAL, info_buf);
		     MPI_Error_string(ierr, error_string, &size);
		     MPITEST_message(MPITEST_FATAL, error_string);
		  }
		  ...
		  for (j = 0; j < ndims; j++) {
			...
			sprintf(info_buf, "ERROR  MPI_Cart_coords:  dimension = %d  (Out of range)   Actual = %3d    Expected  0 - %3d", j, coords[j], dim_size[j]);
			MPITEST_message(MPITEST_FATAL, info_buf);
		     }
		  }
		  sprintf(info_buf, " %5d.   Coords:  %3d%3d%3d%3d ", i, coords[0], coords[1], coords[2], coords[3]);
		  ...
		  for ( k = 0; k < inlist; k++)  {
		    ...
		    for ( m = 0; m < ndims; m++)   {
		      ...
		      while( coords[m] != all_coords[i][k]  && m < ndims )
		      m++;
