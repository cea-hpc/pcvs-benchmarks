...

{
    ...
    sprintf(testname, "MPI_Bsend_err5:  Call with tag < 0");
     ...
     */
    sprintf(info_buf, "About to call MPI_Bsend w/  Call with tag less than 0");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Bsend /w   Call with tag less than 0  (%d) did not FAIL", messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Bsend /w  Call with tag less than 0 returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
