...

{
    ...
    sprintf(testname, "MPI_Bsend_err8, Non_Existant data type ");
    ...
    sprintf(info_buf, "about to call MPI_Bsend w/ non-existant data type");
    ...
    ierr = MPI_Bsend(input, messlength, MPI_DATATYPE_NULL, dest, 0, MPI_COMM_WORLD);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Bsend /w  non-existant data type  did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Bsend /w  non-existant data type returned %d, expected MPI_ERR_TYPE",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
