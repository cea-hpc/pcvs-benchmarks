...
*
* This test verifies that the correct error is returned if MPI_Rsend is
* called with an invalid argument.
*
*
*  Buffered Blocking Rsends error tests
*  -----------------------------------
...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Rsend_err1:   Destination non-existent rank (rank negative)  ");
     ...
     */
    sprintf(info_buf, "about to call MPI_Rsend w/ Destination rank negative");
    ...
    /* Let all nodes do the invalid Rsend  */
    ...
    ierr = MPI_Rsend(input, 1, MPI_CHAR, dest, 0, MPI_COMM_WORLD);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Rsend /w negative rank destination(%d) did not FAIL", dest);
