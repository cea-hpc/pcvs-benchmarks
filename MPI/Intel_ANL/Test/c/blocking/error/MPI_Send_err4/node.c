...
/*---------------------------------------------------------------------------*/
    ...
    sprintf(testname, "MPI_Send_err4:  Call with a freed communicator");
...
/*----------------------  Allocate and Free communicator -------------------*/
     ...
     */
    sprintf(info_buf, "About to call MPI_Send w/ Call with a freed communicator");
    ...
    ierr = MPI_Send(input, 1, MPI_CHAR, dest, 0, comm2);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Send /w  Call with a freed communicator did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Send /w Call with a freed communicator returned %d, expected  MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
