...

{
    ...
    sprintf(testname, "MPI_Send_err8, Non-existant  data type <MPI_DATATYPE_NULL> ");
    ...
    sprintf(info_buf, "about to call MPI_Send w/ non-existant data type <MPI_DATATYPE_NULL>");
    ...
    ierr = MPI_Send(input, messlength, MPI_DATATYPE_NULL, dest, 0, MPI_COMM_WORLD);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Send /w  non-existant data type <MPI_DATATYPE_NULL>  did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Send /w  non-existant data type <MPI_DATATYPE_NULL> returned %d, expected MPI_ERR_TYPE",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
