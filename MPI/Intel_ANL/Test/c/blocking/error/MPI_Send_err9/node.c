...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Send_err9: Mismatched data types between send and receive  ");
    ...
    sprintf(info_buf, "about to call MPI_Send w/  Send/Recv with mismatched data types between send and receive");
	...
	if (MPITEST_me == 0)
	{
	    ...
	    ierr = MPI_Send(float_send, 10, MPI_FLOAT, dest, 1234, MPI_COMM_WORLD);
	...
	if (MPITEST_me == 1)
	{
	    ...
	    {
		fail++;
		sprintf(info_buf, "MPI_Send/Recv /w Mismatched data type on receive did not FAIL");
		...
		{
		    fail++;
		    sprintf(info_buf, "MPI_Send/Recv /w Mismatched data type on receive, returned  %d, expected MPI_ERR_TYPE",
			    errorclass);
		    MPITEST_message(MPITEST_NONFATAL, info_buf);
