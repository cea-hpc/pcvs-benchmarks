...
sprintf( testname, "MPI_Allgather_err2");
...
**  Invalid type:
*/
sprintf(info_buf, "about to call MPI_Allgather w/ invalid type");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Allgather(input, 1, MPI_DATATYPE_NULL, output, 1, MPI_DATATYPE_NULL,
	MPI_COMM_WORLD);

if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Allgather w/ invalid type did not FAIL");
  ...
  {
	fail++;
	sprintf(info_buf, "MPI_Allgather w/ invalid type returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
