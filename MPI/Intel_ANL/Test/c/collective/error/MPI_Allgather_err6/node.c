...
sprintf( testname, "MPI_Allgather_err6");
...
if (MPITEST_nump > 1)
  {
    ...
    sprintf(info_buf, "about to call MPI_Allgather(Intercomm)");
    MPITEST_message( MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Allgather(input, MPITEST_nump, MPI_INT, output, MPITEST_nump, MPI_INT, icomm);
    
    if (ierr == MPI_SUCCESS)
      {
	sprintf(info_buf, "MPI_Allgather(InterComm) did not FAIL");
	...
	if (errorclass != MPI_ERR_COMM)
      {
	    fail++;
	    sprintf(info_buf, "MPI_Allgather(InterComm) returned %d, expected MPI_ERR_COMM",
		errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
