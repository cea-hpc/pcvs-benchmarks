...
sprintf( testname, "MPI_Allgatherv_err1");
...
**  Mis-matched types:
*/
sprintf(info_buf, "about to call MPI_Allgatherv w/ mismatched types");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
if (MPITEST_me != 0)
    ierr = MPI_Allgatherv(input, 1, MPI_CHAR, output, recv_counts,
	recv_displs, MPI_UNSIGNED_CHAR, MPI_COMM_WORLD);
else
    ierr = MPI_Allgatherv(input, 1, MPI_UNSIGNED_CHAR, output, recv_counts,
	recv_displs, MPI_CHAR, MPI_COMM_WORLD);
...
if (MPITEST_nump < 2)
  ...
  {
    fail++;
    sprintf(info_buf, "MPI_Allgatherv w/ mismatched types did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Allgatherv w/ mismatched types returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
