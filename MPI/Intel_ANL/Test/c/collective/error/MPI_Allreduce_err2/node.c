...
sprintf( testname, "MPI_Allreduce_err2");
...
*/
#define yyy(ty, op)  \
sprintf(info_buf, "about to call MPI_Allreduce w/ op %d on invalid %d",op,ty); \
MPITEST_message( MPITEST_INFO1, info_buf); \
ierr = 0; \
ierr = MPI_Allreduce(input, output, 1, ty, op, MPI_COMM_WORLD); \
if (ierr == MPI_SUCCESS) { \
	fail++; \
	sprintf(info_buf, "MPI_Allreduce w/ op %d on invalid %d did not FAIL",op,ty); \
	MPITEST_message(MPITEST_NONFATAL, info_buf); \
} else { \
	MPI_Error_class(ierr, &errorclass); \
	if (errorclass != MPI_ERR_TYPE) { \
		fail++;   \
		sprintf(info_buf, "MPI_Allreduce w/ op %d on invalid %d returned %d, expected MPI_ERR_TYPE", \
			errorclass,op,ty); \
		MPITEST_message(MPITEST_NONFATAL, info_buf); \
