...
sprintf( testname, "MPI_Allreduce_err4");
...
**  Invalid size
*/
sprintf(info_buf, "about to call MPI_Allreduce w/ size = -1");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Allreduce(input, output, -1, MPI_SHORT, MPI_SUM, MPI_COMM_WORLD);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Allreduce w/ size = -1 did not FAIL");
  ...
  {
	fail++;
	sprintf(info_buf, "MPI_Allreduce w/ size = -1 returned %d, expected MPI_ERR_COUNT",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
