...
-----------------------------------------------------------------------------*/
/*******************************************************************************
*                          Error test for MPI_Alltoall()
*
* This test verifies that the correct error is returned if MPI_Alltoall is
...
sprintf( testname, "MPI_Alltoall_err1");
...
**  Mis-matched types:
*/
sprintf(info_buf, "about to call MPI_Alltoall w/ mismatched types");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
if (MPITEST_me == 0)
    ierr = MPI_Alltoall(input, MPITEST_nump, MPI_CHAR, output, MPITEST_nump,
	MPI_UNSIGNED_CHAR, MPI_COMM_WORLD);
else
    ierr = MPI_Alltoall(input, MPITEST_nump, MPI_UNSIGNED_CHAR, output,
	MPITEST_nump, MPI_CHAR, MPI_COMM_WORLD);
...
if (MPITEST_nump < 2)
  ...
  {
    fail++;
    sprintf(info_buf, "MPI_Alltoall w/ mismatched types did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Alltoall w/ mismatched types returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
