...
sprintf( testname, "MPI_Alltoallv_err1");
...
**  Mis-matched types:
*/
sprintf(info_buf, "about to call MPI_Alltoallv w/ mismatched types");
MPITEST_message( MPITEST_INFO1, info_buf);
if (MPITEST_me == 0)
    ierr = MPI_Alltoallv(input, send_counts, send_displs, MPI_CHAR, output,
	recv_counts, recv_displs, MPI_UNSIGNED_CHAR, MPI_COMM_WORLD);
else
    ierr = MPI_Alltoallv(input, send_counts, send_displs, MPI_UNSIGNED_CHAR,
	output, recv_counts, recv_displs, MPI_CHAR, MPI_COMM_WORLD);
...
if (MPITEST_nump < 2)
  ...
  {
    fail++;
    sprintf(info_buf, "MPI_Alltoallv w/ mismatched types did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Alltoallv w/ mismatched types returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
