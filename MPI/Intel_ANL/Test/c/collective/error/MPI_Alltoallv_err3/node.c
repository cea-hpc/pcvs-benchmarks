...
sprintf( testname, "MPI_Alltoallv_err3");
...
**  Invalid size
*/
sprintf(info_buf, "about to call MPI_Alltoallv w/ size = -1");
...
ierr = MPI_Alltoallv(input, send_counts, send_displs, MPI_SHORT, output,
		recv_counts, recv_displs, MPI_SHORT, MPI_COMM_WORLD);
for (size=0; size <= MPITEST_nump; size++)
  {
    send_counts[size]=1;
  }
...
if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Alltoallv w/ size = -1 did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COUNT)
      {
	fail++;
	sprintf(info_buf, "MPI_Alltoallv w/ size = -1 returned %d, expected MPI_ERR_COUNT",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
