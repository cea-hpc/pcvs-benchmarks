...
sprintf( testname, "MPI_Alltoallv_err6");
...
if (MPITEST_nump > 1)
  {
    ...
    sprintf(info_buf, "about to call MPI_Alltoallv(Intercomm)");
    MPITEST_message( MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Alltoallv(input, send_counts, send_displs, MPI_SHORT, output,
		recv_counts, recv_displs, MPI_SHORT, icomm);

    if (ierr == MPI_SUCCESS)
      {
	sprintf(info_buf, "MPI_Alltoallv(InterComm) did not FAIL");
	...
	if (errorclass != MPI_ERR_COMM)
	  {
	    fail++;
	    sprintf(info_buf, "MPI_Alltoallv(InterComm) returned %d, expected MPI_ERR_COMM",
		errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
