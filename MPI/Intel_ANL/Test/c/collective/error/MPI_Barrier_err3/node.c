...
sprintf( testname, "MPI_Barrier_err3");
...
if (MPITEST_nump > 1)
  {
    ...
    sprintf(info_buf, "about to call MPI_Barrier(Intercomm)");
    MPITEST_message( MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Barrier(icomm);

    if (ierr == MPI_SUCCESS)
      {
	sprintf(info_buf, "MPI_Barrier(InterComm) did not FAIL");
	...
	if (errorclass != MPI_ERR_COMM)
	  {
	    fail++;
	    sprintf(info_buf, "MPI_Barrier(InterComm) returned %d, expected MPI_ERR_COMM",
	        errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
