...
sprintf( testname, "MPI_Bcast_err8");
...
if (MPITEST_nump > 1)
  {
    ...
    sprintf(info_buf, "about to call MPI_Bcast(Intercomm)");
    MPITEST_message( MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Bcast(input, 1, MPI_INT, 0, icomm);

    if (ierr == MPI_SUCCESS)
      {
        sprintf(info_buf, "MPI_Bcast(InterComm) did not FAIL");
    ...
    else
      {
        ...
        if (errorclass != MPI_ERR_COMM)
          {
	    fail++;
	    sprintf(info_buf, "MPI_Bcast(InterComm) returned %d, expected MPI_ERR_COMM",
	        errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
