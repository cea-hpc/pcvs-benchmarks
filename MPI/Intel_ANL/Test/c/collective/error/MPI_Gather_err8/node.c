...
sprintf( testname, "MPI_Gather_err8");
...
if (MPITEST_nump > 1)
  {
    ...
    sprintf(info_buf, "about to call MPI_Gather(Intercomm)");
    MPITEST_message( MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Gather(input, MPITEST_nump, MPI_INT, output, MPITEST_nump,
		MPI_INT, 0, icomm);

    if (ierr == MPI_SUCCESS)
      {
	sprintf(info_buf, "MPI_Gather(InterComm) did not FAIL");
	...
	if (errorclass != MPI_ERR_COMM)
	  {
	    fail++;
	    sprintf(info_buf, "MPI_Gather(InterComm) returned %d, expected MPI_ERR_COMM",
		errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
