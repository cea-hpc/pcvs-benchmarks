...
sprintf( testname, "MPI_Gatherv_err8");
...
if (MPITEST_nump > 1)
  {
    ...
    sprintf(info_buf, "about to call MPI_Gatherv(Intercomm)");
    MPITEST_message( MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Gatherv(input, MPITEST_nump, MPI_INT, output, recv_counts,
	    recv_displs, MPI_INT, 0, icomm);

    if (ierr == MPI_SUCCESS)
      {
	sprintf(info_buf, "MPI_Gatherv(InterComm) did not FAIL");
	...
	if (errorclass != MPI_ERR_COMM)
	  {
	    fail++;
	    sprintf(info_buf, "MPI_Gatherv(InterComm) returned %d, expected MPI_ERR_COMM",
		errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
