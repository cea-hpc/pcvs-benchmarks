...
sprintf( testname, "MPI_Reduce_scatter_err7");
...
if (MPITEST_nump > 1)
  {
    ...
    sprintf(info_buf, "about to call MPI_Reduce_scatter(Intercomm)");
    MPITEST_message( MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Reduce_scatter(input, output, count, MPI_INT, MPI_SUM, icomm);

    if (ierr == MPI_SUCCESS)
      {
	sprintf(info_buf, "MPI_Reduce_scatter(InterComm) did not FAIL");
	...
	if (errorclass != MPI_ERR_COMM)
	  {
	    fail++;
	    sprintf(info_buf, "MPI_Reduce_scatter(InterComm) returned %d, expected MPI_ERR_COMM",
		errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
