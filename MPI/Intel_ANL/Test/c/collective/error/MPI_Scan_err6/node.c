...
sprintf( testname, "MPI_Scan_err6");
...
sprintf(info_buf, "about to call MPI_Scan(Free comm)");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Scan(input, output, count, MPI_INT, MPI_SUM, comm);

if (ierr == MPI_SUCCESS)
  {
    sprintf(info_buf, "MPI_Scan(Free Comm) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Scan(Free Comm) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
