...
-----------------------------------------------------------------------------*/
/*******************************************************************************
*                          Error test for MPI_Scatterv()
*
* This test verifies that the correct error is returned if MPI_Scatterv is
...
int main( int argc, char *argv[])
{
  ...
  double
    *input,          /* input to scatter */
    *output;         /* results of scatter */
...
sprintf( testname, "MPI_Scatterv_err1");
...
**  Mis-matched types:
*/
sprintf(info_buf, "about to call MPI_Scatterv w/ mismatched types");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
if (MPITEST_me != 0)
    ierr = MPI_Scatterv(input, send_counts, send_displs, MPI_CHAR, output,
	1, MPI_UNSIGNED_CHAR, 0, MPI_COMM_WORLD);
else
    ierr = MPI_Scatterv(input, send_counts, send_displs, MPI_UNSIGNED_CHAR,
	output, 1, MPI_CHAR, 0, MPI_COMM_WORLD);


if (MPITEST_nump < 2)
  {
    sprintf(info_buf, "Not enough ranks to test w/ mismatched types");
    MPITEST_message(MPITEST_INFO0, info_buf);
    }
else if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Scatterv w/ mismatched types did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Scatterv w/ mismatched types returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
