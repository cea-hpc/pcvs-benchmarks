...
sprintf( testname, "MPI_Scatterv_err2");
...
**  Invalid type:
*/
sprintf(info_buf, "about to call MPI_Scatterv w/ invalid type");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Scatterv(input, send_counts, send_displs, MPI_DATATYPE_NULL, output,
	1, MPI_DATATYPE_NULL, 0, MPI_COMM_WORLD);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Scatterv w/ invalid type did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Scatterv w/ invalid type returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
