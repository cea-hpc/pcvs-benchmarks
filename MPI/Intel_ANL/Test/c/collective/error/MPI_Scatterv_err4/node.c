...
sprintf( testname, "MPI_Scatterv_err4");
...
**  Invalid root = -1
*/
sprintf(info_buf, "about to call MPI_Scatterv w/ root = -1");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Scatterv(input, send_counts, send_displs, MPI_SHORT, output,
	MPITEST_nump, MPI_SHORT, -1, MPI_COMM_WORLD);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Scatterv w/ root = -1 did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_ROOT)
      {
	fail++;
	sprintf(info_buf, "MPI_Scatterv w/ root = -1 returned %d, expected MPI_ERR_ROOT",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
