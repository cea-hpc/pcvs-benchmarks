...
#if MPITEST_long_double_def
      case MPITEST_long_double:
	 if (((MPITEST_long_double_int_struct *) buffer)[i].LongDouble != value.LongDouble) {
	    error++;
	    if (error <= 2) {
...
int main(int argc, char *argv[])
{
   int
      ...
      max_length = 0,		/* maximum buffer length specified in config.
				 * file   */
   ...
   for (comm_count = 0; comm_count < MPITEST_num_comm_sizes(); comm_count++) {
	 ...
	 if (!counts)
	    MPITEST_message(MPITEST_NONFATAL, "Could not allocate counts array");
	 ...
	 for (type_count = 0; type_count < MPITEST_num_datatypes(); type_count++) {
	    ...
	    switch (test_type) {
	     case MPITEST_float:
		  ...
		  max_length = MPITEST_nump;
	       send_buffer = malloc((MPITEST_nump * max_length + 1) * sizeof(MPITEST_float_int_struct));
	     ...
	     case MPITEST_double:
	       ...
	       if (max_length < MPITEST_nump)
		  max_length = MPITEST_nump;
	       send_buffer = malloc((MPITEST_nump * max_length + 1) * sizeof(MPITEST_double_int_struct));
	     ...
	     case MPITEST_long:
	       ...
	       if (max_length < MPITEST_nump)
		  max_length = MPITEST_nump;
	       send_buffer = malloc((MPITEST_nump * max_length + 1) * sizeof(MPITEST_long_int_struct));
	     ...
	     case MPITEST_int:
	       ...
	       if (max_length < MPITEST_nump)
		  max_length = MPITEST_nump;
	       send_buffer = malloc((MPITEST_nump * max_length + 1) * sizeof(MPITEST_2int_struct));
	     ...
	     case MPITEST_short_int:
	       ...
	       if (max_length < MPITEST_nump)
		  max_length = MPITEST_nump;
	       send_buffer = malloc((MPITEST_nump * max_length + 1) * sizeof(MPITEST_short_int_struct));
	     ...
	     case MPITEST_long_double:
	       ...
	       if (max_length < MPITEST_nump)
		  max_length = MPITEST_nump;
	       send_buffer = malloc((MPITEST_nump * max_length + 1) * sizeof(MPITEST_long_double_int_struct));
	    ...
	    for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++) {
		  ...
		  MPITEST_init_buffer_loc(test_type, test_nump * length,
					  value, send_buffer);
		  ...
		  if (has_op(op_count, test_type) && (length != 0)) {
		     loop_cnt++;
		     ierr = MPI_Reduce_scatter(send_buffer, recv_buffer,
					       counts, localtype,
				       MPITEST_default_ops[op_count], comm);

		     if (ierr != MPI_SUCCESS) {
			sprintf(info_buf, "MPI_Reduce_scatter() return %d", ierr);
			MPITEST_message(MPITEST_NONFATAL, info_buf);
			MPI_Error_string(ierr, error_string, &size);
			MPITEST_message(MPITEST_FATAL, error_string);
			fail++;
		     ...
		     /* error test */
		     error = 0;
		     error = MPITEST_buffer_errors_loc(test_type, length, value, recv_buffer);

		     if (error) {
			if (ierr == MPI_SUCCESS)
			   fail++;
			sprintf(info_buf, "%d errors found in buffer, len %d commsize %d commtype %d data_type %d op %d",
