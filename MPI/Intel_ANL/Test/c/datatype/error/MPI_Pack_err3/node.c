...
Rank 0 will first call MPI_Pack with MPI_COMM_NULL.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Pack_err3: MPI_COMM_NULL");
	...
	incount = 1; /* Ensure that the *only* error is COMM_NULL */
      outsize = MPITEST_BUFSIZE;
      type = MPI_INT;
      position = 0;

      /* MPI_Pack() with MPI_COMM_NULL */
      ierr2 = MPI_Pack(inbuf, incount, type, outbuf, outsize, &position, MPI_COMM_NULL);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Pack() with MPI_COMM_NULL returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Pack() with MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
