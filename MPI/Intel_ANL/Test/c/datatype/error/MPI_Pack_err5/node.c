...
5)  Call with uncommitted datatype........[MPI_ERR_TYPE/MPI_ERR_ARG]
...
Rank 0 will first call MPI_Pack with uncomiited datatype.
...
MPI Calls dependencies for this test:
  MPI_Type_contiguous(), MPI_Pack(), MPI_Init(), MPI_Finalize()
  MPI_Error_string(), 
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Pack_err5: uncommitted datatype");
	...
	MPITEST_message(MPITEST_FATAL, error_string);
      ...
      /* MPI_Pack() with uncommitted datatype */
      ierr2 = MPI_Pack(inbuf, incount, newtype, outbuf, outsize, &position, MPI_COMM_WORLD);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Pack() with uncommitted datatype returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Pack() with uncommitted datatype returned error class %d, expected MPI_ERR_TYPE", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
