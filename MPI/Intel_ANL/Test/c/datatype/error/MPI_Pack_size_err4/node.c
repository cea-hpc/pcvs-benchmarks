...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Pack_size()

This test verifies that the correct error is returned if MPI_Pack_size()
is called with an invalid argument.

MPI_Pack_size error tests
-----------------------------------
1)  Call with negative input incount......[MPI_ERR_COUNT]
2)  Call with MPI_COMM_NULL...............[MPI_ERR_COMM]
3)  Call with MPI_DATATYPE_NULL...........[MPI_ERR_TYPE]
4)  Call with uncommitted datatype........[MPI_ERR_TYPE]

In all cases, expect to receive appropriate error.

Rank 0 will first call MPI_Pack_size with uncomiited datatype.
...
MPI Calls dependencies for this test:
  MPI_Type_contiguous(), MPI_Pack_size(), MPI_Init(), MPI_Finalize()
  MPI_Error_string(), 
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Pack_size_err4: uncommitted datatype");
    ...
    if (MPITEST_me == source) {
      ...
      ierr= MPI_Type_contiguous(incount, oldtype, &newtype);
      if (ierr != MPI_SUCCESS) {
	fail++;
	...
	MPITEST_message(MPITEST_FATAL, error_string);
      ...
      /* MPI_Pack_size() with uncommitted datatype */
      ierr2 = MPI_Pack_size(incount, newtype, MPI_COMM_WORLD, &size);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Pack_size() with uncommitted datatype returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Pack_size() with uncommitted datatype returned error class %d, expected MPI_ERR_TYPE", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
