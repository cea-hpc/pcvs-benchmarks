...
Rank 0 will first call MPI_Type_contiguous with negative input count.
...
MPI Calls dependencies for this test:
  MPI_Type_contiguous(), MPI_Init(), MPI_Finalize()
  MPI_Error_string(), 
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_contiguous_err1: Negative count");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Type_contiguous() with negative count */
      ierr2 = MPI_Type_contiguous(count, oldtype, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_contiguous() with negative input count returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_contiguous() with negative input count returned error class %d, expected MPI_ERR_COUNT", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
