...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Type_create_struct()

 This test is just the original MPI_Type_struct test, with 
 MPI_Type_create_struct replacing MPI_Type_struct

This test verifies that the correct error is returned if MPI_Type_create_struct()
is called with an invalid argument.

MPI_Type_create_struct error tests
-----------------------------------
...
3)  Call with input array of types
    which contains MPI_DATATYPE_NULL.......[MPI_ERR_TYPE]

In all cases, expect to receive appropriate error.

MPI Calls dependencies for this test:
  MPI_Type_create_struct(), MPI_Init(), MPI_Finalize()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_create_struct_err3: MPI_DATATYPE_NULL");
	...
	types[i] = MPI_INT;
      ...
      types[count - 1] = MPI_DATATYPE_NULL;

      /* MPI_Type_create_struct() with MPI_DATATYPE_NULL */
      ierr2 = MPI_Type_create_struct(count, blklen, displs, types, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_create_struct() with MPI_DATATYPE_NULL returned MPI_SUCCESS", source);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_create_struct() with MPI_DATATYPE_NULL returned error class %d, expected MPI_ERR_TYPE", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
