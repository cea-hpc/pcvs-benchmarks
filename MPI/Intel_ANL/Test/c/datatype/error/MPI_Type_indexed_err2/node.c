...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_indexed_err2: Negative block length");
	...
	displs[1] = i * extent;
      ...
      /* MPI_Type_indexed() with -ve block length */
      ierr2 = MPI_Type_indexed(count, blklen, displs, MPI_INT, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_indexed() with negative block length returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_indexed() with negative block length returned error class %d (%s), expected MPI_ERR_ARG", errorclass, MPITEST_GetErrName( errorclass ) );
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
