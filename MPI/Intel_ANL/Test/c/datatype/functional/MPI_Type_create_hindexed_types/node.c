...
-----------------------------------------------------------------------------*/
/******************************************************************************
                          Test for MPI_Type_create_hindexed()
...
(in mpitest_cfg.h) using MPI_Type_create_hindexed().  The root's rank will then send
...
MPI Calls dependencies for this test:
  ...
  MPI_Type_create_hindexed(), MPI_Type_create_struct(), MPI_Type_commit(),
...
int main(int argc, char *argv[])
{
   ...
   signed char *buffer; /* message buffer (test assumed signed)               */
   ...
   sprintf(testname, "MPI_Type_create_hindexed_types");
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    if (!displs1) {
	       sprintf(info_buf, "Cannot allocate enough memory for displs1 array");
	       MPITEST_message(MPITEST_FATAL, info_buf);
	    }

	    blklens1 = (int *) calloc(length, sizeof(int));
	    if (!blklens1) {
	       sprintf(info_buf, "Cannot allocate enough memory for blklens1 array");
	       MPITEST_message(MPITEST_FATAL, info_buf);
	    }
	    ...
	    ierr = MPI_Type_create_hindexed(length, blklens1, displs1, oldtype, &newtype);
	    if (ierr != MPI_SUCCESS) {
	       fail++;
	       sprintf(info_buf, "MPI_Type_create_hindexed() returned %d", ierr);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	       MPI_Error_string(ierr, error_string, &size);
	       MPITEST_message(MPITEST_FATAL, error_string);
	    }
