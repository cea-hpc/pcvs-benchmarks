...
-----------------------------------------------------------------------------*/
/******************************************************************************
                          Test for MPI_Type_create_hvector()

 This test is just the original MPI_Type_hvector test, with 
 MPI_Type_create_hvector replacing MPI_Type_hvector

All ranks will first do a MPI_Type_create_struct() of all types in the type config
...
(in mpitest_cfg.h) using MPI_Type_create_hvector() using MPITEST_STRIDE + extent as
...
MPI Calls dependencies for this test:
  ...
  MPI_Type_create_hvector(), MPI_Type_create_struct(), MPI_Type_commit(),
...
/*   struct dataTemplate
   ...
   sprintf(testname, "MPI_Type_create_hvector_stride");
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 ierr = MPI_Type_create_struct(basic_types, blklens, displs, types, &oldtype);
	 if (ierr != MPI_SUCCESS) {
	    sprintf(info_buf, "MPI_Type_create_struct() returned %d", ierr);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
	    MPITEST_message(MPITEST_FATAL, error_string);
	 }
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    ierr = MPI_Type_create_hvector(length, 1, extent + MPITEST_STRIDE, oldtype, &newtype);
	    if (ierr != MPI_SUCCESS) {
	       fail++;
	       sprintf(info_buf, "MPI_Type_create_hvector() returned %d", ierr);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	       MPI_Error_string(ierr, error_string, &size);
	       MPITEST_message(MPITEST_FATAL, error_string);
	    }
	    ...
	    for (root=0; root<test_nump; root++) {
		  ...
		  for (i = 0; i < length; i++) {
			...
			for (k = 0; k < type_sizes[j]; k++) {
			   ...
			   pos = i * (extent + MPITEST_STRIDE) + acc + k;
		  ...
		  if (buffer[length * (extent + MPITEST_STRIDE)] != -1 * MPITEST_current_rank) {
		     fail++;
		     error2++;
#ifdef MPITEST_1ST_ERR
		     if (error2 <= 1) {
			...
			sprintf(info_buf,
				...
				" Actual buffer[%d]: %d",
				length * (MPITEST_STRIDE + extent), -1 * MPITEST_current_rank,
				length * (MPITEST_STRIDE + extent), buffer[length * (MPITEST_STRIDE + extent)]);
			MPITEST_message(MPITEST_NONFATAL, info_buf);
#ifdef MPITEST_1ST_ERR
		     }
#endif
