...
int main(int argc, char *argv[])
{
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 if (extent != 0) {
	    fail++;
	    sprintf(info_buf, "Unexpected return calling MPI_Type_extent() w/ MPI_LB Expected: %d, Actual: " MPITEST_AINT_FMT_DEC_SPEC, 0, extent);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	 }
	 ...
	 if (extent != 0) {
	    fail++;
	    sprintf(info_buf, "MPI_Type_extent() w/ MPI_UB returned unexpected Expected: %d, Actual: " MPITEST_AINT_FMT_DEC_SPEC, 0, extent);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	 }
      }
