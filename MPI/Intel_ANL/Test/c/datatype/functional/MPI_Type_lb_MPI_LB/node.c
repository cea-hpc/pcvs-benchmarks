...
int main(int argc, char *argv[])
{
   ...
   MPI_Aint *displs, displ, tmp_lb, type_lb = 0;

   int inter_flag, type_lb_flag = 0;
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    for (type_count = 0; type_count < num_types - 1; type_count++) {
		  ...
		  MPITEST_message(MPITEST_FATAL, error_string);
	       ...
	       /* record smallest sticky lb value (taking displs into account) */
	       ierr = MPITEST_datatype_has_sticky_lb(types[type_count], &tmp_lb);
	       if (ierr == 1) {
		  if (!type_lb_flag || (tmp_lb + displs[type_count] < type_lb)) {
		     type_lb_flag = 1;
		     type_lb = tmp_lb + displs[type_count];
		  }
	       }
	    }
		...
		/* if there was already a type with a sticky lb, let's make sure this one
		 * is the one that remains
		 */
		displs[num_types - 1] = type_lb - 10;
	    }
	    ...
	    else if (displ != displs[num_types - 1]) {
	       fail++;
	       sprintf(info_buf, "MPI_Type_lb() returned unexpected displacement  Expected: " MPITEST_AINT_FMT_DEC_SPEC ", Actual: " MPITEST_AINT_FMT_DEC_SPEC, displs[num_types - 1], displ);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	    }
