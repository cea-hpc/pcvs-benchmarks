...
int main(int argc, char *argv[])
{
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) {
	    ...
	    /* If we didn't see a sticky lb, then length is our lower bound */
	    if (!type_lb_flag) type_lb = length;
	    ...
	    else if (displ != type_lb) {
	       fail++;
	       sprintf(info_buf, "MPI_Type_lb() returned unexpected displacement  Expected: " MPITEST_AINT_FMT_DEC_SPEC ", Actual: " MPITEST_AINT_FMT_DEC_SPEC, type_lb, displ);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	    }
