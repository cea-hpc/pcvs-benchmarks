...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Attr_put()

This test verifies that the correct error is returned if MPI_Attr_put()
is called with invalid arguments.

MPI_Attr_put error tests
-----------------------------------
...
Rank 0 will call MPI_Attr_put with MPI_KEYVAL_INVALID.
...
MPI Calls dependencies for this test:
  MPI_Attr_put(), MPI_Init(), MPI_Finalize()
  MPI_Error_string()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Attr_put_err2: MPI_KEYVAL_INVALID");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Attr_put with MPI_KEYVAL_INVALID */
      sprintf(info_buf, "Calling MPI_Attr_put with MPI_KEYVAL_INVALID");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Attr_put(MPI_COMM_SELF, MPI_KEYVAL_INVALID, &attribute_val);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Attr_put() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
...
#  endif
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Attr_put() with MPI_KEYVAL_INVALID returned error class %d, expected MPI_ERR_ARG or MPI_ERR_OTHER", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
