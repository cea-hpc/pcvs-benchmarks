...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Comm_create()

This test verifies that the correct error is returned if MPI_Comm_create()
is called with invalid arguments.

MPI_Comm_create error tests
-----------------------------------
1)  Call with MPI_COMM_NULL..............[MPI_ERR_COMM]
2)  Call with MPI_GROUP_NULL.............[MPI_ERR_GROUP]

In all cases, expect to receive appropriate error.

Rank 0 will call MPI_Comm_create with MPI_GROUP_NULL.
...
MPI Calls dependencies for this test:
  MPI_Comm_create(), MPI_Init(), MPI_Finalize()
  MPI_Error_string()
  [MPI_Allreduce(), MPI_Comm_create(), MPI_Comm_create()]
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Comm_create_err2: MPI_GROUP_NULL");
    ...
    /* Init the variables used in the bad call */
    /* comm */
    comm = MPI_COMM_SELF;
    /* group */
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Comm_create with MPI_GROUP_NULL */
      sprintf(info_buf, "Calling MPI_Comm_create with MPI_GROUP_NULL");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Comm_create(comm, group, &newcomm);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Comm_create() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Comm_create() with MPI_GROUP_NULL returned error class %d, expected MPI_ERR_GROUP", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
