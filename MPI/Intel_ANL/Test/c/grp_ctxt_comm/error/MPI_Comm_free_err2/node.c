...
Rank 0 will call MPI_Comm_free with MPI_COMM_WORLD.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Comm_free_err2: MPI_COMM_WORLD");
    ...
    if (MPITEST_me == 0) {
      ...
      sprintf(info_buf, "Calling MPI_Comm_free with MPI_COMM_NULL");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Comm_free() with MPI_COMM_WORLD returned error class %d, expected MPI_ERR_COMM, MPI_ERR_ARG, or MPI_ERR_OTHER", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
