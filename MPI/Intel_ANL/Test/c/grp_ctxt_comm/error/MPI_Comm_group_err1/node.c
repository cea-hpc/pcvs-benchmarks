...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Comm_group()

This test verifies that the correct error is returned if MPI_Comm_group()
is called with invalid arguments.

MPI_Comm_group error tests
-----------------------------------
1)  Call with MPI_COMM_NULL..............[MPI_ERR_COMM]

In all cases, expect to receive appropriate error.

Rank 0 will call MPI_Comm_group with MPI_COMM_NULL.
...
MPI Calls dependencies for this test:
  MPI_Comm_group(), MPI_Init(), MPI_Finalize()
  MPI_Error_string()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Comm_group_err1: MPI_COMM_NULL");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Comm_group with MPI_COMM_NULL */
      sprintf(info_buf, "Calling MPI_Comm_group with MPI_COMM_NULL");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Comm_group(MPI_COMM_NULL, &group);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Comm_group() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Comm_group() with MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
