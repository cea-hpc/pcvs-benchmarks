...
Rank 0 will call MPI_Comm_remote_size with MPI_COMM_NULL.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Comm_remote_size_err1: MPI_COMM_NULL");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Comm_remote_size with MPI_COMM_NULL */
      sprintf(info_buf, "Calling MPI_Comm_remote_size with MPI_COMM_NULL");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Comm_remote_size(MPI_COMM_NULL, &size);
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Comm_remote_size() with MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
