...
Rank 0 will call MPI_Group_excl with an out of range (posative) rank.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_excl_err5: call with out of range (posative) rank");
    ...
    n = 1;
    /* ranks */
    ranks[0] = MPITEST_nump;

    if (MPITEST_me == 0) {
      /* Calling MPI_Group_excl with out of range (posative) rank */
      sprintf(info_buf, "Calling MPI_Group_excl with out of range (posative) rank");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_excl() with out of range (posative) rank returned error class %d, expected MPI_ERR_RANK", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
