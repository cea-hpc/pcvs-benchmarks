...
Rank 0 will call MPI_Group_excl with duplicate ranks.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_excl_err6: call with duplicate ranks");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_excl with duplicate ranks */
      sprintf(info_buf, "Calling MPI_Group_excl with duplicate ranks");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_excl() with duplicate ranks returned error class %d, expected MPI_ERR_OTHER, MPI_ERR_ARG, or MPI_ERR_RANK", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
