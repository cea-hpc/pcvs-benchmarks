...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Group_free()

This test verifies that the correct error is returned if MPI_Group_free()
is called with invalid arguments.

MPI_Group_free error tests
-----------------------------------
1)  Call with MPI_GROUP_NULL..............[MPI_ERR_GROUP]

In all cases, expect to receive appropriate error.

Rank 0 will call MPI_Group_free with MPI_GROUP_NULL.
...
MPI Calls dependencies for this test:
  MPI_Group_free(), MPI_Init(), MPI_Finalize()
  MPI_Error_string()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_free_err1: MPI_GROUP_NULL");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_free with MPI_GROUP_NULL */
      sprintf(info_buf, "Calling MPI_Group_free with MPI_GROUP_NULL");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Group_free(&group);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Group_free() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_free() with MPI_GROUP_NULL returned error class %d, expected MPI_ERR_GROUP", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
