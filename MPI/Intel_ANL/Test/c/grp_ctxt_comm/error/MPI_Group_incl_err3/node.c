...
Rank 0 will call MPI_Group_incl with n greater then the size of group.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_incl_err3: n greater then size of group");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_incl with n greater than size of group */
      sprintf(info_buf, "Calling MPI_Group_incl with n greater then size of group");
	...
	/* 10-31-02 Since an invalid rank is passed as input, MPI_ERR_RANK is
	   an acceptable error class */
	else if ((errorclass != MPI_ERR_OTHER) && 
		 (errorclass != MPI_ERR_ARG) &&
	         (errorclass != MPI_ERR_RANK)) {
	  fail++;
          sprintf(info_buf, "MPI_Group_incl() with n greater then size of group returned error class %d, expected MPI_ERR_OTHER or MPI_ERR_ARG", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
