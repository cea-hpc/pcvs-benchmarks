...
Rank 0 will call MPI_Group_range_excl with negative n.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_range_excl_err2: negative n");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_range_excl with negative n */
      sprintf(info_buf, "Calling MPI_Group_range_excl with negative n");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_range_excl() with negative n returned error class %d, expected MPI_ERR_OTHER or MPI_ERR_ARG", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
