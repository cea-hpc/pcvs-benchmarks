...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Group_union()

This test verifies that the correct error is returned if MPI_Group_union()
is called with invalid arguments.

MPI_Group_union error tests
-----------------------------------
...
Rank 0 will call MPI_Group_union with MPI_GROUP_NULL.
...
MPI Calls dependencies for this test:
  MPI_Group_union(), MPI_Comm_group(), MPI_Init(), MPI_Finalize()
  MPI_Error_string()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_union_err2: group2=MPI_GROUP_NULL");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_union with group2=MPI_GROUP_NULL */
      sprintf(info_buf, "Calling MPI_Group_union with group2=MPI_GROUP_NULL");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Group_union(group1, group2, &newgroup);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Group_union() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_union() with group2=MPI_GROUP_NULL returned error class %d, expected MPI_ERR_GROUP", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
