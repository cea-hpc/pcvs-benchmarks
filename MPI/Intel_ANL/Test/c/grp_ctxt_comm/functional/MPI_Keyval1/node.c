...
int main(int argc, char *argv[])
{
	...
	*attr,		/* Attribute returned		   */
	...
	if (MPITEST_current_rank != MPI_UNDEFINED)
	{
	    ...
	    if (extra1 != 3)
	    {
		sprintf(info_buf, "callback functions called " MPITEST_AINT_FMT_DEC_SPEC " times for key1,  expected 3 (comm_index %d)", extra1, comm_index);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		fail++;
	    }
	    if (extra2 != 1)
	    {
		sprintf(info_buf, "callback functions called " MPITEST_AINT_FMT_DEC_SPEC " times for key2, expected 1 (comm_index %d)", extra2, comm_index);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		fail++;
	    }
	    if (extra3 != 2)
	    {
		sprintf(info_buf, "callback functions called " MPITEST_AINT_FMT_DEC_SPEC " times for key3, expected 2 (comm_index %d)", extra3, comm_index);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		fail++;
	    }
	    if (extra4 != 1)
	    {
		sprintf(info_buf, "callback functions called " MPITEST_AINT_FMT_DEC_SPEC " times for key4, expected 1 (comm_index %d)", extra4, comm_index);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		fail++;
	    }
