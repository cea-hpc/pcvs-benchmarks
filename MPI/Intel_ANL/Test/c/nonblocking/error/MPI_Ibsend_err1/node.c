...
*
* This test verifies that the correct error is returned if MPI_Ibsend is
...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Ibsend_err1:   Destination non-existent rank (rank negative)  ");
     ...
     */
    sprintf(info_buf, "about to call MPI_Ibsend w/ Destination rank negative");
    ...
    ierr = MPI_Ibsend(input, 1, MPI_CHAR, dest, 0, MPI_COMM_WORLD, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Ibsend /w negative rank destination(%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Ibsend /w negative rank destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
