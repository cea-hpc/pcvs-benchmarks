...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                          Error test for MPI_Irecv
*
* This test verifies that the correct error is returned if MPI_Irecv is
* called with an invalid argument.
*
*
*  Nonlocking Receive error tests
*  -----------------------------------
...
* 9)  Try to receive a message larger than Irecv count .. [MPI_ERR_TRUNCATE]
*
*
* In all cases, expect to receive appropriate error.
*
* Test history:
*    1  04/22/96     jh   Created
*
* NOTES:

******************************************************************************/
#define  TEST_TYPE   "MPI_Irecv"
#define  PROG_NAME   "MPI_Irecv_err01"
#define  MESS01      "Source non-existent rank (rank negative)"
#define  ERR_NAM     "MPI_ERR_RANK"
...
{
    int
        source,  	/* Source of Irecv message                           */
    ...
    /* Let all nodes do the invalid Irecv  */

      source = -1 * (MPITEST_me + 1);

      if (source == MPI_PROC_NULL)  source = source - 1;
      if (source == MPI_ANY_SOURCE) source = source - 1;
      if (source == MPI_PROC_NULL)  source = source - 1;
      if (source == MPI_ANY_SOURCE) source = source - 1;

    ierr = MPI_Irecv(output, 1, MPI_CHAR, source, MPI_ANY_TAG, \
		    MPI_COMM_WORLD, &recv_request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "%s /w %s (%d) did not FAIL", TEST_TYPE, MESS01, source);
