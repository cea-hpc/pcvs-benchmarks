...
#define  PROG_NAME   "MPI_Irecv_err07"
#define  MESS01      "Message length count argument negative"
#define  MESS02      "Count < 0 "
...

{
   ...
   /*-------------------------------  Receive ------------------------------*/
    ...
    ierr = MPI_Irecv(output, messlength , MPI_CHAR, source, MPI_ANY_TAG, \
		     MPI_COMM_WORLD, &recv_request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "%s /w %s (%d) did not FAIL", TEST_TYPE, MESS02, messlength);
