...

{
    ...
    sprintf(testname, "MPI_Irsend_err8, Non-existant  data type <MPI_DATATYPE_NULL> ");
    ...
    sprintf(info_buf, "about to call MPI_Irsend w/ non-existant data type <MPI_DATATYPE_NULL>");
    ...
    ierr = MPI_Irsend(input, messlength, MPI_DATATYPE_NULL, dest, 0, MPI_COMM_WORLD, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Irsend /w  non-existant data type <MPI_DATATYPE_NULL>  did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Irsend /w  non-existant data type <MPI_DATATYPE_NULL> returned %d, expected MPI_ERR_TYPE",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
