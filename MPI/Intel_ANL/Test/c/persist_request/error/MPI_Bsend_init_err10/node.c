...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Bsend_init_err10: Message larger than attached buffer ");
    ...
    sprintf(info_buf, "about to call MPI_Bsend_init w/ Message larger than attached buffer ");
    ...
    if (ierr == MPI_SUCCESS)
    {
	    ...
	    /* WDG - 10/8/03 - ERR_IN_STATUS is incorrect for Wait; 
	       it is only for the multiple completion operations */
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Bsend_init /w  Message larger than attached buffer (%d) did not FAIL", buffsize);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Bsend_init /w  Message larger than attached buffer returned %d (%s), expected MPI_ERR_BUFFER",
