...

{
    ...
    sprintf(testname, "MPI_Bsend_init_err3:  Use  MPI_COMM_NULL communicator");
     ...
     */
    sprintf(info_buf, "about to call MPI_Bsend_init w/ MPI_COMM_NULL");
    ...
    ierr = MPI_Bsend_init(input, 1, MPI_CHAR, dest, 0, MPI_COMM_NULL, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Bsend_init /w MPI_COMM_NULL communicator did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Bsend_init /w MPI_COMM_NULL communicator returned %d, expected MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
