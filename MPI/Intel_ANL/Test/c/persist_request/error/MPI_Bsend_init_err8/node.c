/*-----------------------------------------------------------------------------
MESSAGE PASSING INTERFACE TEST CASE Copyright - 1996 Intel Corporation
...

{
    ...
    sprintf(testname, "MPI_Bsend_init_err8, Non_Existant data type ");
    ...
    sprintf(info_buf, "about to call MPI_Bsend_init w/ non-existant data type");
    ...
    ierr = MPI_Bsend_init(input, messlength, MPI_DATATYPE_NULL, dest, 0, MPI_COMM_WORLD, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Bsend_init /w  non-existant data type  did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Bsend_init /w  non-existant data type returned %d, expected MPI_ERR_TYPE",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
