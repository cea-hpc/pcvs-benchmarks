...
*
* This test verifies that the correct error is returned if MPI_Rsend_init is
...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Rsend_init_err1:   Destination non-existent rank (rank negative)  ");
     ...
     */
    sprintf(info_buf, "about to call MPI_Rsend_init w/ Destination rank negative");
    ...
    ierr = MPI_Rsend_init(input, 1, MPI_CHAR, dest, 0, MPI_COMM_WORLD, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Rsend_init /w negative rank destination(%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Rsend_init /w negative rank destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
