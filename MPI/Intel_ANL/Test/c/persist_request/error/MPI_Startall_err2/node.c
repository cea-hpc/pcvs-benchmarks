...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Startall_err2:  Request freed");
    ...
    /*
     *  Do a startall; expect failure.
    */
    ierr = MPI_Startall(1, &request);
    if (ierr == MPI_SUCCESS)
    {
        fail++;
        sprintf(info_buf, "MPI_Startall on freed request did not FAIL");
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    }
    else
    {
        ...
        {
	    fail++;
	    sprintf(info_buf, "MPI_Startall on freed request returned %d, expected MPI_ERR_REQUEST",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
	    MPITEST_message(MPITEST_NONFATAL, error_string);
