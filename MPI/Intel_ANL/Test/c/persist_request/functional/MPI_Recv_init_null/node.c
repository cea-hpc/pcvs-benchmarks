...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Recv_init_null: all persistent Recv from MPI_PROC_NULL");
	...
	if (MPITEST_current_rank != MPI_UNDEFINED)
	{
		...
		for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++)
		{
		    ...
		    if (ierr != MPI_SUCCESS)
		    {
			sprintf(info_buf, "(%d,%d,%d) length %d commsize %d commtype %d data_type %d", length_count, comm_count, type_count, length,
				test_nump, comm_type, test_type);
		    ...
		    if (ierr != MPI_SUCCESS)
		    {
			sprintf(info_buf, "Non-zero return code (%d) from MPI_Start", ierr);
			MPITEST_message(MPITEST_NONFATAL, info_buf);
			MPI_Error_string(ierr, error_string, &size);
			MPITEST_message(MPITEST_NONFATAL, error_string);
			loop_fail++;
		    ...
		    {
			sprintf(info_buf, "Unexpected source value=%d, expected MPI_PROC_NULL(%d)", recv_stat.MPI_SOURCE, MPI_PROC_NULL);
			MPITEST_message(MPITEST_NONFATAL, info_buf);
			loop_fail++;
		    }
