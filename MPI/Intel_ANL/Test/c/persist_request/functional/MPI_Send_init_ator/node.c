...
int main(int argc, char *argv[])
{
	...
	sprintf(info_buf, "ierr = %d  from Errhandler_set:  comm_count  %d    comm_index %d   comm_type  %d   test_nump %d", 
...
/*  Start the Receive */
				...
				if (ierr != MPI_SUCCESS)
				{
				    ...
				    loop_fail++;
         			...
         			if (ierr != MPI_SUCCESS)
				{
				    sprintf(info_buf, "Non-zero return code  ierr =(%d) from MPI_Wait on receive", ierr);
				    MPITEST_message(MPITEST_NONFATAL, info_buf);
				    MPI_Error_string(ierr, error_string, &size);
				    MPITEST_message(MPITEST_NONFATAL, error_string);
				    loop_fail++;
	...
	if (ierr != MPI_SUCCESS)
	{
	    sprintf(info_buf, "Non-zero return code (%d) from MPI_Request_free on send", ierr);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
	    MPITEST_message(MPITEST_NONFATAL, error_string);
	    fail++;
