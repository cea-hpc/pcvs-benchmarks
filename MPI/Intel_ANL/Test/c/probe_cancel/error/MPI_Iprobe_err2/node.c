...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Iprobe_err2: tag negative (%d)", bad_tag);
    ...
    source = 0;
       ...
       /* MPI_Iprobe call with the negative tag */
       sprintf(info_buf, "Calling MPI_Iprobe() with negative tag (%d)", bad_tag);
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Iprobe() with negative tag (%d) returned MPI_SUCCESS", bad_tag);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
       }
       else {
	  ...
	  if (ierr != MPI_SUCCESS) {
	     ...
	     fail++;
	     sprintf(info_buf, "MPI_Iprobe() /w negative tag (%d) returned error class %d, expected MPI_ERR_TAG",
		     bad_tag, errorclass);
	     MPITEST_message(MPITEST_NONFATAL, info_buf);
       ...
       /* Skip receving or cancelling to receive the message since the */
       /* MPI state is undefined after an error                        */
