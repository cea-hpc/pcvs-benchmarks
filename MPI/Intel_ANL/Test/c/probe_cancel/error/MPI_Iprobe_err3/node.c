...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Iprobe_err3: tag (%d) > MPI_TAG_UB (%d)", bad_tag, *attr_ub);
    if (*attr_ub < INT_MAX) {
	...
	/* 10-31-02 - Do not indicate an error when the test isn't
	   applicable */
       sprintf(info_buf, "MPI_Iprobe_err3: MPI_TAG_UB = INT_MAX (%d) Test not applicable, bailing out...", INT_MAX);
    ...
    if (MPITEST_me == 0) {
       ...
       source = 0;
       /* MPI_Iprobe call with too large tag */
       sprintf(info_buf, "Calling MPI_Iprobe() with too large tag (%d) ...", bad_tag);
       MPITEST_message(MPITEST_INFO1, info_buf);

       ierr2 = MPI_Iprobe(source, bad_tag, MPI_COMM_WORLD, &flag, &status);

       if (ierr2 == MPI_SUCCESS) {
	  fail++;
	  sprintf(info_buf, "MPI_Iprobe() with too large tag (%d) returned MPI_SUCCESS", bad_tag);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
       }
       else {
	  ...
	  if (ierr != MPI_SUCCESS) {
	     ...
	     fail++;
	     sprintf(info_buf, "MPI_Iprobe() /w too large tag (%d) returned error class %d, expected MPI_ERR_TAG",
		     bad_tag, errorclass);
	     MPITEST_message(MPITEST_NONFATAL, info_buf);
