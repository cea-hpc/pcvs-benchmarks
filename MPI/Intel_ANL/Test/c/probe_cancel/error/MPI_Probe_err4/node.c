...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Probe_err4: MPI_COMM_NULL");
    ...
    if (ierr != MPI_SUCCESS) {
       ...
       /* MPI_Probe call with the MPI_COMM_NULL */
       sprintf(info_buf, "Calling MPI_Probe() with MPI_COMM_NULL (%d) ...", MPI_COMM_NULL);
       MPITEST_message(MPITEST_INFO1, info_buf);

       ierr2 = MPI_Probe(source, tag, MPI_COMM_NULL, &status);

       if (ierr2 == MPI_SUCCESS) {
	  fail++;
	  sprintf(info_buf, "MPI_Probe() with MPI_COMM_NULL returned MPI_SUCCESS");
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
       }
       else {
	  ...
	  if (ierr != MPI_SUCCESS) {
	     ...
	     fail++;
	     sprintf(info_buf, "MPI_Probe() /w MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM",
		     errorclass);
	     MPITEST_message(MPITEST_NONFATAL, info_buf);
