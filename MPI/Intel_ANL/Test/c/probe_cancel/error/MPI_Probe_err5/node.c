...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    source = MPITEST_nump; /* bad source rank */
    sprintf(testname, "MPI_Probe_err5: too large source rank (%d)", source);
    if (MPITEST_me == 0) {
    ...
    tag = 1;
       ...
       /* MPI_Probe call with the too large source rank */
       sprintf(info_buf, "Calling MPI_Probe() with too large source rank (%d) ...", source);
       MPITEST_message(MPITEST_INFO1, info_buf);

       ierr2 = MPI_Probe(source, tag, MPI_COMM_WORLD, &status);

       if (ierr2 == MPI_SUCCESS) {
	  fail++;
	  sprintf(info_buf, "MPI_Probe() with too large source rank (%d) returned MPI_SUCCESS", source);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
       }
       else {
	  ...
	  if (ierr != MPI_SUCCESS) {
	     ...
	     fail++;
	     sprintf(info_buf, "MPI_Probe() /w too large source rank (%d) returned error class %d, expected MPI_ERR_RANK",
		     source, errorclass);
	     MPITEST_message(MPITEST_NONFATAL, info_buf);
