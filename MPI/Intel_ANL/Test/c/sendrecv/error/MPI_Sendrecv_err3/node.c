...

{
    ...
    sprintf(testname, "MPI_Sendrecv_err3:  Use  MPI_COMM_NULL communicator");
     ...
     */
    sprintf(info_buf, "about to call MPI_Sendrecv w/ MPI_COMM_NULL");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv /w MPI_COMM_NULL communicator did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv /w MPI_COMM_NULL communicator returned %d, expected MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
