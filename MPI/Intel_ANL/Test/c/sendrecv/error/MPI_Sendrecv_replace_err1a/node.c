...
*
* This test verifies that the correct error is returned if 
* MPI_Sendrecv_replace is called with an invalid argument.
*
*
...

{
    ...
    int
        iobuff[10];	/* input to Sendrecv_replace                         */
    ...
    */
    ...
    sprintf(testname, "MPI_Sendrecv_replace_err1a:  Send Destination non-existent rank (rank negative)  ");
     ...
     */
    sprintf(info_buf, "about to call MPI_Sendrecv_replace w/ SEND Destination rank negative");
	...
	dest--;
        ...
        ierr = MPI_Sendrecv_replace(iobuff,
				    1,  
				    MPI_CHAR, 
				    dest,
				    0, 
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv_replace /w negative rank SEND destination(%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv_replace /w negative rank SEND destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
