...

{
    ...
    sprintf(testname, "MPI_Sendrecv_replace_err7, message length count < 0");
     ...
     */
    sprintf(info_buf, "about to call MPI_Sendrecv_replace w/ message length count < 0 ");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv_replace /w negative message length count (%d) did not FAIL", messlength);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv_replace /w negative message length count returned %d, expected MPI_ERR_COUNT",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
