...
int main(int argc, char *argv[])
{
    ...
    float
        iobuffer[100]  ;/* Array of values to send and receive               */
    ...
    sprintf(testname, "MPI_Sendrecv_replace_err9: Mismatched data types between send and receive  ");
    ...
    if (MPITEST_nump < 2)
    {
	sprintf(info_buf, "Not enough ranks to test Sendrecv_replace with mismatched data types");
	MPITEST_message(MPITEST_FATAL, info_buf);
    }

    sprintf(info_buf, "about to call MPI_Sendrecv_replace with mismatched data types between send and receive");
	...
	if (MPITEST_me == 0)
	{
	    ...
	    {
		fail++;
		sprintf(info_buf, "MPI_Sendrecv_replace /w Mismatched data type on receive did not FAIL");
		...
		{
		    fail++;
		    sprintf(info_buf, "MPI_Sendrecv_replace /w Mismatched data type on receive, returned  %d, expected MPI_ERR_TYPE",
			    errorclass);
		    MPITEST_message(MPITEST_NONFATAL, info_buf);
	...
	}	/* End of node 0 Sendrecv_replace  */

	/*--------------------  Node 1:  Sendrecv_replace  ---------------- */
