...
int main(int argc, char *argv[])
{
    ...
    int
        ...
        length_count = 0,   /* loop counter for message length loop          */
        ...
        test_type = 0,      /* the index of the current buffer type          */
    ...
    sprintf(testname, "MPI_Sendrecv_replace_self: all Sendrecv_replace to SELF");
            ...
            (MPITEST_inter == MPITEST_NOT_INTER))
	{
		...
		for (type_count = 0; type_count < MPITEST_num_datatypes(); type_count++)
		{
			...
			MPITEST_init_buffer(test_type, length + 1, value, iobuffer);

			MPITEST_dataTemplate_init(&value, MPITEST_current_rank);
			MPITEST_init_buffer(test_type, length , value, iobuffer);
			...
			if (ierr != MPI_SUCCESS)
			{
				...
				test_nump, comm_type, test_type);
			    MPITEST_message(MPITEST_NONFATAL, info_buf);
			    sprintf(info_buf, "Non-zero return code (%d) from MPI_Sendrecv", ierr);
			    MPITEST_message(MPITEST_NONFATAL, info_buf);
			    MPI_Error_string(ierr, error_string, &size);
		...
		if (MPITEST_current_rank == 1)
		{
			...
			test_nump, comm_type, test_type);
			MPITEST_message(MPITEST_NONFATAL, info_buf);
			sprintf(info_buf, "Non-zero return code (%d) from MPI_Send signal", ierr);
			MPITEST_message(MPITEST_NONFATAL, info_buf);
			MPI_Error_string(ierr, error_string, &size);
			MPITEST_message(MPITEST_FATAL, error_string);
		    }	/* Error Test  */
