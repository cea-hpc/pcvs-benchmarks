...
****************************************************************************/
#define  DIMENSIONS      1
#define  TEST_DESC       "MPI_Cart_rank:  call with communicator without topology"
...
int main(int argc, char *argv[])
{
   ...
   sprintf(testname, "MPI_Cart_rank_err1:  Call with communicator without topology");
   ...
   if (MPITEST_me == 0) {
      ...
      ierr = MPI_Cart_rank(MPI_COMM_WORLD,
			   coords,
			   &rank_cart);
	 ...
	 if (errorclass != MPI_ERR_TOPOLOGY && errorclass != MPI_ERR_OTHER && errorclass != MPI_ERR_COMM) {
	    fail++;
	    sprintf(info_buf, "%s  returned %d(%s), expected MPI_ERR_COMM(%d) or MPI_ERR_ARG(%d) or MPI_ERR_TOPOLOGY(%d) or MPI_ERR_OTHER(%d)", TEST_DESC, errorclass, MPITEST_GetErrName( errorclass ), MPI_ERR_COMM, MPI_ERR_ARG, MPI_ERR_TOPOLOGY, MPI_ERR_OTHER);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
