...
****************************************************************************/
#define  MPITEST_DIMS    1
#define  TEST_DESC       "MPI_Cartdim_get:  call with communicator without topology"
...
int main(int argc, char *argv[])
{
   int
      ...
      test_nump,		/* The number of processors in current
				 * communicator  */
      cart_dims,		/* Number of dimensions in cartesian
				 * structure       */
      rank_cart,		/* rank in graph communicator                        */
      reorder;			/* allow rank reorder flag                           */

   int
      dim_size[MPITEST_DIMS],	/* number of processes in each dimension     */
      periods[MPITEST_DIMS];	/* periodic flag for each dimension          */
   ...
   sprintf(testname, "MPI_Cartdim_get_err1:  Call with communicator without topology");
   ...
   if (test_nump < 2) {
      sprintf(info_buf, "Not enough ranks to test MPI_Cartdim_get, need at least 2");
      MPITEST_message(MPITEST_FATAL, info_buf);
   }
   ...
   if (MPITEST_me == 0) {
      ...
      ierr = MPI_Cartdim_get(MPI_COMM_WORLD,
			     &cart_dims);


      if (ierr == MPI_SUCCESS) {
         if (cart_dims != MPI_UNDEFINED) {
	    fail++;
