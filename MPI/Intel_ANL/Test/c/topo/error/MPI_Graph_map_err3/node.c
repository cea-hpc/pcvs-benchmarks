...
#define  PRNT_NODE       0
#define  DIMENSIONS      1
#define  TEST_DESC       "MPI_Graph_map call with nnodes = 0"
...
int main(int argc, char *argv[])
{
   ...
   sprintf(testname, "Graph_map_err3:  call with nnodes = 0");
   ...
   if (comm_graph != MPI_COMM_NULL) {
	 ...
	 if (ierr == MPI_SUCCESS) {
	    ...
	    MPI_Error_class(ierr, &errorclass);
	    if (errorclass != MPI_ERR_ARG && errorclass != MPI_ERR_RANK && errorclass != MPI_ERR_OTHER) {
	       fail++;
	       sprintf(info_buf, "%s  returned %d, expected MPI_ERR_ARG(%d) or MPI_ERR_RANK(%d) or MPI_ERR_OTHER(%d)", TEST_DESC, errorclass, MPI_ERR_ARG, MPI_ERR_RANK, MPI_ERR_OTHER);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	       MPI_Error_string(ierr, error_string, &size);
	       MPITEST_message(MPITEST_NONFATAL, error_string);
	    }
