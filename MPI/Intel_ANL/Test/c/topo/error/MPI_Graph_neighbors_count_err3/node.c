...
/******************************************************************************
                          ...
                          MPI Graph neighbors_count_err3
...
#define  PRNT_NODE       0
#define  DIMENSIONS      1
#define  TEST_DESC    "MPI Graph neighbors_count: call with Cartesian communicator"
...
int main(int argc, char *argv[])
{
   ...
   sprintf(testname, "Graph_neighbors_count_err3:  call with Cartesian communicator");
   ...
   if (test_nump < 2) {
      sprintf(info_buf, "Not enough ranks to test MPI_Graph_neighbors_count, need at least 2");
      MPITEST_message(MPITEST_FATAL, info_buf);
   }
   ...
   if (comm_cart != MPI_COMM_NULL) {
	 ...
	 ierr = MPI_Graph_neighbors_count(comm_cart,
					  rank_cart,
					  &neighbors);
