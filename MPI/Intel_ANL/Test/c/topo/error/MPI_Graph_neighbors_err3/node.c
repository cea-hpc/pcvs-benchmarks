...
#define  PRNT_NODE       0
#define  DIMENSIONS      1
#define  TEST_DESC    "MPI Graph neighbors: call with Cartesian communicator"
...
int main(int argc, char *argv[])
{
   int
      ...
      edge_cnt,			/* edge count for building index and edge
				 * vectors    */
      neighbors,		/* neighbor count returned from neighbors            */
      maxneighbors,		/* length of graph_neighbors vector                  */
      nnodes,			/* number of nodes in graph                          */
      rank_cart,		/* rank in the Cartesian communicator                */
      cart_nump;		/* number of processes in Cartesian
				 * communicator     */

   int
      graph_neighbors[4],	/* neighbor node vector                         */
      dim_size[2],		/* processes in each Cartesian dimension             */
      periods[2],		/* all periodic assignment by Cartesian
				 * dimension    */
      index[4],			/* cumulative neighbor count for nodes in
				 * graph      */
      edges[8],			/* node neighbor list vector                         */
      nx[4][2],			/* node connection array                             */
      nx_tot[4];		/* node connetion total for buiding index
				 * vector     */
   ...
   sprintf(testname, "Graph_neighbors_err3:  call with Cartesian communicator");
   ...
   if (comm_cart != MPI_COMM_NULL) {
	 ...
	 ierr = MPI_Graph_neighbors(comm_cart,
				    rank_cart,
				    maxneighbors,
				    graph_neighbors);
