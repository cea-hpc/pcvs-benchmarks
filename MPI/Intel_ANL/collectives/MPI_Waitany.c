...

{
    ...
    MPI_Status
        one_stat,               /* Single status variable                    */
...
, ierr);
	...
	for (i = 0; i < 4*NUMMESG; i++)
	{
		...
		/* 10-31-02. The MPI_ERROR field has meaning only when ierr has
		   class MPI_ERR_IN_STATUS.  MPI_Waitany must not return
		   this value (test below).  Since the code below tests 
		   the MPI_ERROR value, clear it here (since the value is
		   undefined, it may not be MPI_SUCCESS in the one_stat
		   status.
		*/
		if (!ierr) 
		    recv_stat[index].MPI_ERROR = MPI_SUCCESS;
	    }
	    total_cnt++;
	    if (ierr != MPI_SUCCESS)
	    {
...
*/
	...
	if (ierr != MPI_SUCCESS)
	{
	    sprintf(info_buf, "Non-zero return code (%d) from second MPI_Waitany on receive", ierr);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	...
	{    
	    sprintf(info_buf, "Second Waitany:  index  not equal to  MPI_UNDEFINED (%d)", index);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    fail++;
	}
	...
	for (i = 0; i < 4*NUMMESG; i++)
	{
	    ...
	    {
		sprintf(info_buf, "recv_request[%d] not equal to  MPI_REQUEST_NULL(%d/%d) after second Waitany", i, recv_request[i], MPI_REQUEST_NULL);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		MPI_Error_string(ierr, error_string, &size);
		MPITEST_message(MPITEST_NONFATAL, error_string);
		fail++;
