   ...
   for (comm_count = 0; comm_count < MPITEST_num_comm_sizes(); comm_count++) {
	 ...
	 if (!inter_comm && test_nump >= MPITEST_MINNODE) {
	    ...
	    for (i = 0; i < maxindex; i++) {
		  ...
		  sprintf(info_buf, "Error in Graph_get for node (%3d):  Actual = (%d)   Expected (%d)", i, graph_index[i], index[i]);
		  MPITEST_message(MPITEST_FATAL, info_buf);
	       }
	    }
	    ...
	    for (j = 0; j < maxedges; j++) {
		  ...
		  sprintf(info_buf, "Error in Graph_get for edge (%3d):  Actual = (%d)   Expected (%d)", j, graph_edges[j], edges[j]);
		  MPITEST_message(MPITEST_FATAL, info_buf);
	       }
	    }
