...
int main(int argc, char *argv[])
{
   ...
   for (comm_count = 0; comm_count < MPITEST_num_comm_sizes(); comm_count++) {
	 ...
	 if (!inter_comm && test_nump >= MPITEST_MINNODE) {
	    ...
	    ierr = MPI_Graphdims_get(comm_graph,
				     &gdnodes,
				     &gdedges);
	    ...
	    if (gdnodes != nnodes) {
	       sprintf(info_buf, "Error in Graphdims_get for nodes:  Actual = (%d)   Expected (%d)", gdnodes, nnodes);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	    }
	    if (gdedges != inp_edges) {
	       sprintf(info_buf, "Error in Graphdims_get for edge count:  Actual = (%d)   Expected (%d)", gdedges, inp_edges);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	    }
