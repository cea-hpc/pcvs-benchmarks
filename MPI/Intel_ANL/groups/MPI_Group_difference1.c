...
int main(int argc, char *argv[])
{
	...
	if (MPITEST_current_rank != MPI_UNDEFINED)
	{
	    ...
	    if (ierr != MPI_SUCCESS)
	    {
		sprintf(info_buf, "Non-zero return code (%d) from MPI_Group_difference(group, EMPTY)  (comm_index %d)", ierr, comm_index);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		MPI_Error_string(ierr, error_string, &size);
		MPITEST_message(MPITEST_FATAL, error_string);
		fail++;
	    ...
	    {
		fail++;
		sprintf(info_buf, "MPI_Group_difference(group, EMPTY) did not return identical group (comm_index %d)", comm_index);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		MPI_Error_string(ierr, error_string, &size);
	    }
