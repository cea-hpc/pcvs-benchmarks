...

{
	...
	recv_request[4 * NUMMESG+1];	/* MPI request structure             */
    ...
    MPI_Comm
	comm[4];	/* A number of communicators to sort over	     */
    int
	...
	*/
	...
	if (ierr != MPI_SUCCESS)
	{
	    sprintf(info_buf, "Non-zero return code (%d) from MPI_Start on receive", ierr);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
	    MPITEST_message(MPITEST_NONFATAL, error_string);
	    fail++;
