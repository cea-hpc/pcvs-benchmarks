...
int main(int argc, char *argv[])
{
    ...
    MPI_Status
	sendrecv_stat;	/* MPI status structure			         */
    ...
    sprintf(testname, "MPI_Sendrecv_null: all Sendrecv from MPI_PROC_NULL");
	...
	if (MPITEST_current_rank != MPI_UNDEFINED)
	{
		...
		for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++)
		{
		    ...
		    sprintf(info_buf, "(%d,%d,%d) length %d commsize %d commtype %d data_type %d", length_count, comm_count, type_count, send_length, test_nump, comm_type, test_type);
			...
			sprintf(info_buf, "(%d,%d,%d) length %d commsize %d commtype %d data_type %d",
			  length_count, comm_count, type_count, send_length,
