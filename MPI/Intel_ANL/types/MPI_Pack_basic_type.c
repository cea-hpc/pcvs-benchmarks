...
int main(int argc, char *argv[])
{
   ...
   signed char *in_buffer, *packed_buffer, *buf_ptr;  
                        /* message buffer (declare as signed because the 
			   test assumes signed)                               */
   ...
   int       sextent;   /* sextent is used in MPI_Type_size, which takes
			   an int, not an MPI_Aint.  This matters on 
			   64-bit platforms */
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 /*
	  * The original Intel test used MPI_Type_extent here.  However, 
	  * what is really needed is the SIZE, since this test is 
	  * not unpacking multiple copies of a datatype with a single
	  * Unpack call (or packing them); rather, it is unpacking
	  * them one after the other.  It may be that True extent
	  * rather than the MPI-1 extent is needed here.
	  */
	 ierr = MPI_Type_size(newtype, &sextent);
	 if (ierr != MPI_SUCCESS) {
	    sprintf(info_buf, "MPI_Type_size() returned %d", ierr);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
	 ...
	 /* Now that we have the size, put it into the extent variable
	    for the rest of the code */
	 extent = (MPI_Aint)sextent;
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    for (root=0; root<test_nump; root++) {
		  ...
		  for (i = 0; i < length; i++) {
			...
			ierr = MPI_Pack(buf_ptr, 1, type, packed_buffer,
		  ...
		  for (i = 0; i < pack_size; i++) {
		     sprintf(info_buf, "packed_buffer[%d] = %d", i, packed_buffer[i]);
		     MPITEST_message(MPITEST_INFO2, info_buf);
		  }
#endif
		  ...
		  if (error + error2) {
			...
			MPITEST_message(MPITEST_INFO2, info_buf);
		     ...
		     sprintf(info_buf, "Displaying PACKED received data buffer:");
		     MPITEST_message(MPITEST_INFO2, info_buf);
