...
int main(int argc, char *argv[])
{
   ...
   /* The sequence ??) is the C trigraph for ], which is not what is 
      intended (and an excellent example of why trigraphs were a 
      bad idea) */
   printf("DEBUG ( ?? ): Step 1\n");
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    /* to continue */
	    MPITEST_get_buffer(MPITEST_byte, length * extent + MPITEST_ADD_BUFSIZE, (void *) &buffer);
	    ...
	    for (root=0; root<test_nump; root++) {
		  ...
		  for (i = 0; i < length * extent + MPITEST_ADD_BUFSIZE; i++) {
		  ...
		  for (i = length * extent; i < length * extent + MPITEST_ADD_BUFSIZE; i++) {
		     buffer[i] = -1 * MPITEST_current_rank;
		  }
	       }
