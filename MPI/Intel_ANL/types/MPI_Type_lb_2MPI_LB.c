...
int main(int argc, char *argv[])
{
   ...
   MPI_Aint xt, *displs1, *displs2, displ, tmp_lb, type1_lb, type2_lb;
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    if (type1_lb_flag) {
		sprintf(info_buf, "type1 has sticky lb at %d", type1_lb);
		MPITEST_message(MPITEST_INFO1, info_buf);
	    }

	    if (type2_lb_flag) {
		sprintf(info_buf, "type2 has sticky lb at %d", type2_lb);
		MPITEST_message(MPITEST_INFO1, info_buf);
	    }
	    ...
	    if (MPITEST_current_rank==0) {
		  ...
		  sprintf(info_buf, "blklens1[%d] = %d, displs1[%d] = %d, "
		  ...
		  MPITEST_message(MPITEST_INFO2, info_buf);
	       ...
	       for (type_count = 0; type_count < num_types; type_count++) {
		  sprintf(info_buf, "blklens2[%d] = %d, displs2[%d] = %d, "
	    ...
	    if (ierr != MPI_SUCCESS) {
	       ...
	       fail++;
	       sprintf(info_buf, "MPI_Type_lb() returned unexpected displacement (type1) Expected: %d, Actual: %d",
		       type1_lb, displ);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	    }
	    ...
	    if (ierr != MPI_SUCCESS) {
	       ...
	       fail++;
	       sprintf(info_buf, "MPI_Type_lb() returned unexpected displacement (type2) Expected: %d, Actual: %d",
		       type2_lb, displ);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	    }
	    ...
	    if (ierr != MPI_SUCCESS) {
	       ...
	       fail++;
	       sprintf(info_buf, "MPI_Type_lb() returned unexpected displacement  Expected: %d, Actual: %d", tmp_lb, displ);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	    }
