...
int main(int argc, char *argv[])
{
	...
	if(myrank == 0)
	fprintf(stderr,"Lancement test Gatherv ...\n");
	for (count = 1; count < 65000; count = 2 * count) 
	{
	    ...
	    if (color == 0) 
	    {
			...
			err = MPI_Gatherv( NULL, 0, datatype, buf, recvcounts, recvdispls, datatype, (rank == 0) ? MPI_ROOT : MPI_PROC_NULL, intercomm );
			if (err)
				errs++;
	    ...
	    else 
	    {
			...
			err = MPI_Gatherv( buf, count, datatype, NULL, 0, 0, datatype, 0, intercomm );
