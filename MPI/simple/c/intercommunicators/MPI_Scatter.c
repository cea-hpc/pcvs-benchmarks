...
int main(int argc, char *argv[])
{
	...
	for (count = 1; count < 65000; count = 2 * count)
	{
		...
		if (color == 0)
		{
			...
			err = MPI_Scatter(buf, count, datatype, NULL, 0, datatype, (rank == 0) ? MPI_ROOT : MPI_PROC_NULL,
				intercomm);
		...
		else
		{
			...
			err = MPI_Scatter(NULL, 0, datatype, buf, count, datatype, 0, intercomm);
			if (err)
			{
				errs++;
			}
