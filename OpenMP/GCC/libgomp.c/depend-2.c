...
#pragma omp task depend(out : x)
      ...
      for (i = 0; i < 8; i++)
#pragma omp task depend(in : a[i * 8 : 4]) depend(                             \
    inout : a[i * 8 + 4 : 2]) depend(in : a[0 : 4]) depend(in : x)
      {
        if (a[0] != 8 + 2 * 9 || x != 1)
          printf("%d -- x : %d - %d - %d -%d\n", __LINE__, a[0], 8 + 2 * 9,
                 a[0] != 8 + 2 * 9, x != 1);
...
#pragma omp task depend(inout : a[i * 8 : 4])                                  \
                            depend(in : p[i * 8 : 2]) depend(in : p[0 : 2], x)
      {
