...
int test_omp_taskyield(FILE * logFile){
...
int main()
{
	...
	printf("##############################################\n");
	printf("Testing omp taskyield\n\n");

	fprintf(stdout,"######## OpenMP Validation Suite V %s ######\n", OMPTS_VERSION );
	fprintf(stdout,"## Repetitions: %3d                       ####\n",REPETITIONS);
	fprintf(stdout,"## Loop Count : %6d                    ####\n",LOOPCOUNT);
	fprintf(stdout,"##############################################\n");
	fprintf(stdout,"Testing omp taskyield\n\n");

	for ( i = 0; i < REPETITIONS; i++ ) {
		fprintf (stdout, "\n\n%d. run of test_omp_taskyield out of %d\n\n",i+1,REPETITIONS);
		if(test_omp_taskyield(stdout)){
