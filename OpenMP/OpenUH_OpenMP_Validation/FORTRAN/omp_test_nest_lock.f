      ...
      INTEGER FUNCTION test_omp_test_nest_lock()
        IMPLICIT NONE
        INTEGER result
!result is:
!      0 -- if the test fails
...
!               print *, result, nr_iterations
        IF(result.EQ.0 .AND. nr_iterations .EQ. LOOPCOUNT) THEN
              test_omp_test_nest_lock=1
        ELSE
              test_omp_test_nest_lock=0
        ENDIF
      END FUNCTION
!This is the main driver to invoke different test functions
      ...
      INTEGER test_omp_test_nest_lock
...
!      READ  (*,*) logfilename
      ...
      result=1
      WRITE (*,*) "--------------------------------------------------"
      WRITE (*,*) "Testing omp_test_nest_lock"
      WRITE (*,*) "--------------------------------------------------"
      WRITE (*,*) 
      WRITE (*,*) "testname: test_omp_test_nest_lock"
      WRITE (*,*) "(Crosstests should fail)"
      WRITE (*,*)
      
      DO j = 1, N
        temp =  test_omp_test_nest_lock()
        IF (temp .EQ. 1) THEN
