      ...
      INTEGER FUNCTION test_section_lastprivate()
        INTEGER known_sum
...
!$omp end parallel
        ...
        IF ( known_sum .EQ. sum .AND. i0 .EQ. 999 ) THEN
           test_section_lastprivate = 1
        ELSE
           test_section_lastprivate = 0
        END IF
      END
!This is the main driver to invoke different test functions
      ...
      INTEGER test_section_lastprivate
...
!      READ  (*,*) logfilename
      ...
      result=1
      WRITE (*,*) "--------------------------------------------------"
      WRITE (*,*) "Testing omp section lastprivate"
      WRITE (*,*) "--------------------------------------------------"
      WRITE (*,*) 
      WRITE (*,*) "testname: test_section_lastprivate"
      WRITE (*,*) "(Crosstests should fail)"
      WRITE (*,*)
      
      DO j = 1, N
        temp =  test_section_lastprivate()
        IF (temp .EQ. 1) THEN
