...
void * threaded ( void * arg )
{
	...
	ret = pthread_setschedparam( pthread_self(), SCHED_OTHER, &sp );
	 ...
	 */
	check_param( pthread_self(), SCHED_OTHER, sp.sched_priority );
	...
	if ( ret != 0 )
	{
		/* check the thread attributes have been applied
		  (we only check what is reported, not the real behavior) 
		 */
		check_param( pthread_self(), SCHED_OTHER, sp.sched_priority - 1 );
#if VERBOSE > 0
