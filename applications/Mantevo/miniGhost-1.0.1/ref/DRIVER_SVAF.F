...
CONTAINS
   ...
   SUBROUTINE DRIVER_SVAF ( GRID, IERR )
   ...
   IMPLICIT NONE
         ...
         DO TSTEP = STARTING_TSTEP, NTSTEPS
            DO IVAR = 1, NVARS
               ...
               IF ( STENCIL == STENCIL_2D5PT .OR. STENCIL == STENCIL_3D7PT ) THEN
                  CALL MG_SVAF ( GRID(0,0,0,IVAR), IERR )
                  CALL MG_ASSERT ( IERR, 'DRIVER_SVAF:MG_SVAF', IVAR )
               ELSE IF ( STENCIL == STENCIL_2D9PT .OR. STENCIL == STENCIL_3D27PT ) THEN
                  CALL MG_SVAF_DIAGS ( GRID(0,0,0,IVAR), IERR )
                  CALL MG_ASSERT ( IERR, 'DRIVER_SVAF:MG_SVAF_DIAG', IVAR )
               ELSE
                  IERR = -1
      ...
      DO IVAR = 1, NVARS
         CALL MG_SUM_GRID ( GRID(0,0,0,IVAR), IVAR, GSUM, IERR )
         CALL MG_ASSERT ( IERR, 'DRIVER_SVAF:MG_SUM_GRID(Final check)', IVAR )
         MG_PERF%TIME_SUMGRID_PE = MG_PERF%TIME_SUMGRID_PE + MG_TIMER () - TIME_START_2
 ...
 99   FORMAT ( 'Time step ', I4, ' for spike ', I3, ' for variable ', &
               I4, ' the error is ', 1PE12.5 '; error tolerance is ', 1PE12.5, '.' )
