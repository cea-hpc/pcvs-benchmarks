<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="comparison">
		<diffTestSuite name="{checked/@name}" >
		<xsl:apply-templates select="checked/testcase"/>
		</diffTestSuite>
	</xsl:template>

	<xsl:template match="checked/testcase">
		<diffTestCase>
	...
	<xsl:template name="loopReference">
		...
		<xsl:for-each select="/comparison/reference/testcase">
