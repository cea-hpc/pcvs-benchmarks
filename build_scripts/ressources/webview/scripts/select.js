...
{
	var tabMessage = new Array("success","error","failed","unchanged_success","unchanged_failure", "fixed", "regress_failure", "regress_perf");
	for(var cpt = 0; cpt<tabMessage.length; cpt++){
		if(message == tabMessage[cpt] || message == "*"){
			...
			{
				if (Obj1[i].tagName == 'TR') Obj1[i].style.display = '';
			}
