...
C
       ...
       integer packbuf1(1000), packbuf2(1000)
       integer packbuf3(1000), packbuf4(1000)
       integer pbufsize, intsize
       integer max_asizev
...
C We use aintv(3) to detect and report this specific error
     ...
     &               MPI_CHARACTER, packbuf3, aintv(1), 
     &               aintv(2), ierr )
       if (aintv(2) .le. aintv(3)) then
...
C We could try sending this with MPI_BYTE...
       aintv(2) = 0
       call mpi_unpack_external( "external32", packbuf1, aintv(1),
     &  aintv(2), ioutbuf, insize, MPI_INTEGER, ierr )
       call mpi_unpack_external( "external32", packbuf2, aintv(1),
     &  aintv(2), routbuf, rsize, MPI_DOUBLE_PRECISION, ierr )
       call mpi_unpack_external( "external32", packbuf3, aintv(1),
     &  aintv(2), coutbuf, csize, MPI_CHARACTER, ierr )
       call mpi_unpack_external( "external32", packbuf4, aintv(1),
