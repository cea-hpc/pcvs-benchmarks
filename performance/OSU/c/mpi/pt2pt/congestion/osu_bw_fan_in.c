...
int main(int argc, char *argv[])
{
    ...
    OMB_CHECK_NULL_AND_EXIT(s_buf, "Unable to allocate send buffer");
    OMB_CHECK_NULL_AND_EXIT(r_buf, "Unable to allocate receive buffer");
    ...
    MPI_CHECK(MPI_Comm_split(omb_comm, 0 == fan_in_out_info.is_parent, 0,
                             &barrier_comm));
    ...
    for (mpi_type_itr = 0; mpi_type_itr < options.omb_dtype_itr;
        ...
        for (size = options.min_message_size; size <= options.max_message_size;
             size *= options.message_size_incr) {
            ...
            if (options.buf_num == MULTIPLE) {
                for (i = 0; i < window_size; i++) {
                    if (allocate_memory_pt2pt_size(
                            &s_buf[i], &r_buf[i],
                            !(0 == fan_in_out_info.is_parent), size)) {
                        /* Error allocating memory */
                        omb_mpi_finalize(omb_init_h);
                        exit(EXIT_FAILURE);
                    }
                ...
                set_buffer_pt2pt(s_buf[0], !(0 == fan_in_out_info.is_parent),
                                 options.accel, 'a', size);
                set_buffer_pt2pt(r_buf[0], !(0 == fan_in_out_info.is_parent),
                                 options.accel, 'b', size);
            }
            ...
            omb_graph_allocate_and_get_data_buffer_cus_rank(
                &omb_graph_data, &omb_graph_options, size, options.iterations,
                myid);
            if (options.graph) {
                OMB_CHECK_NULL_AND_EXIT(omb_graph_data,
            ...
            if (options.omb_tail_lat) {
                if (0 == myid) {
                    MPI_CHECK(MPI_Recv(omb_lat_arr, options.iterations,
                                       MPI_DOUBLE, MPI_ANY_SOURCE, 102,
                                       omb_comm, MPI_STATUS_IGNORE));
                } else if (0 == fan_in_out_info.cur_node_info->local_rank &&
                           2 == fan_in_out_info.cur_node_info->node_id) {
                    MPI_CHECK(MPI_Send(omb_lat_arr, options.iterations,
                                       MPI_DOUBLE, 0, 102, omb_comm));
                }
            }
            if (options.graph) {
                if (0 == myid) {
                    MPI_CHECK(MPI_Recv(omb_graph_data->data, options.iterations,
                                       MPI_DOUBLE, MPI_ANY_SOURCE, 103,
                                       omb_comm, MPI_STATUS_IGNORE));
                } else if (0 == fan_in_out_info.cur_node_info->local_rank &&
                           2 == fan_in_out_info.cur_node_info->node_id) {
                    MPI_CHECK(MPI_Send(omb_graph_data->data, options.iterations,
                                       MPI_DOUBLE, 0, 103, omb_comm));
                }
            }
    ...
    if (options.buf_num == SINGLE) {
        free_memory(s_buf[0], r_buf[0], !(0 == fan_in_out_info.is_parent));
    }
