...
static void print_usage(int rank, const char * prog, int has_size)
{
  ...
  if (has_size) {
    ...
    fprintf(stdout,
            "  -i : Set number of iterations per message size to ITER.\n");
    fprintf(
        stdout,
        "       By default, the value of ITER is 1000 for small messages\n");
    fprintf(stdout, "       and 100 for large messages.\n");
    fprintf(stdout,
            "  -M : Set maximum memory consumption (per process) to SIZE. \n");
  ...
  fprintf(stdout,
          "  -r : Set the compute target. Use 1 (default) to execute on\n");
...
enum accel_type { none, cuda, openacc, managed };
