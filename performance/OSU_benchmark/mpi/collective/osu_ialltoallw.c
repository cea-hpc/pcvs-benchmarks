...
int main(int argc, char *argv[]) {
  ...
  if (allocate_buffer((void **)&stypes, numprocs * sizeof(MPI_Datatype),
                      none)) {
    fprintf(stderr, "Could Not Allocate Memory [rank %d]\n", rank);
    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
  }

  if (allocate_buffer((void **)&rtypes, numprocs * sizeof(MPI_Datatype),
                      none)) {
    fprintf(stderr, "Could Not Allocate Memory [rank %d]\n", rank);
    MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
  }
  ...
  for (size = options.min_message_size; size <= options.max_message_size;
    ...
    for (i = 0; i < options.iterations + options.skip; i++) {
      t_start = MPI_Wtime();
      MPI_Ialltoallw(sendbuf, sendcounts, sdispls, stypes, recvbuf, recvcounts,
                     rdispls, rtypes, MPI_COMM_WORLD, &request);
