  ...
  if (rank == 0) {
    ...
    if (full) {
      fprintf(stdout, "%*.*f%*.*f%*d\n", FIELD_WIDTH, FLOAT_PRECISION, min_time,
              FIELD_WIDTH, FLOAT_PRECISION, max_time, 12, iterations);
    }
...
print_help_message (int rank)
{
    ...
    if (accel_enabled) {
        ...
        printf("                use accelerator managed device buffers which "
               "can be of TYPE `managed' or\n");
        printf("                `openacc' (uses standard host buffers if not specified)\n");
    }

    if (options.show_size) {
      printf("  -m [MIN:]MAX  set the minimum and/or the maximum message size "
             "to MIN and/or MAX\n"
             "                bytes respectively. Examples:\n"
             "                -m 128      // min = default, max = 128\n"
             "                -m 2:128    // min = 2, max = 128\n"
             "                -m 2:       // min = 2, max = default\n");
      printf("  -M SIZE       set per process maximum memory consumption to "
             "SIZE bytes\n");
      printf("                (default %d)\n", MAX_MEM_LIMIT);
    }

    printf("  -i ITER       set iterations per message size to ITER (default "
           "1000 for small\n");
    printf("                messages, 100 for large messages)\n");
    printf("  -x ITER       set number of warmup iterations to skip before "
           "timing (default 200)\n");

    printf("  -f            print full format listing (MIN/MAX latency and ITERATIONS\n");
    printf("                displayed in addition to AVERAGE latency)\n");

    printf("  -t CALLS      set the number of MPI_Test() calls during the "
           "dummy computation, \n");
    ...
    if (CUDA_KERNEL_ENABLED) {
      ...
      printf("                on the GPU only, and set to both for compute on "
             "both.\n");

      printf("  -s SIZE       set the size of arrays to be allocated on device "
             "(GPU) \n");
      printf(
          "                for dummy compute on device (GPU) (default 32) \n");
    }
...
void calculate_and_print_stats(int rank, int size, int numprocs, double timer,
  ...
  print_stats_nbc(rank, size, overall_time, tcomp_total, comm_time, wait_total,
                  init_total, test_total);
}
...
void print_stats_nbc(int rank, int size, double overall_time, double cpu_time,
  ...
  if (options.show_full) {
    fprintf(stdout, "%*.*f%*.*f%*.*f%*.*f%*.*f%*.*f\n", FIELD_WIDTH,
            ...
            FLOAT_PRECISION, init_time, FIELD_WIDTH, FLOAT_PRECISION, test_time,
            FIELD_WIDTH, FLOAT_PRECISION, wait_time, FIELD_WIDTH,
            FLOAT_PRECISION, comm_time, FIELD_WIDTH, FLOAT_PRECISION, overlap);
  } else {
...
void print_stats(int rank, int size, double avg_time, double min_time,
  ...
  if (options.show_full) {
    fprintf(stdout, "%*.*f%*.*f%*lu\n", FIELD_WIDTH, FLOAT_PRECISION, min_time,
            FIELD_WIDTH, FLOAT_PRECISION, max_time, 12, options.iterations);
  }
