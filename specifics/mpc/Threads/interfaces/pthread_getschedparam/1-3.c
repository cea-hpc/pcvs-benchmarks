...
void * threaded ( void * arg )
{
	 ...
	 */
	check_param( pthread_self(), SCHED_OTHER, sched_get_priority_min( SCHED_OTHER ) );
...
int main( int argc, char *argv[] )
{
	...
	ret = pthread_attr_setschedpolicy( &ta, SCHED_OTHER );
	...
	/* Wait while the thread checks its policy
	  (we only check what is reported, not the real behavior) 
	 */
	check_param( child, SCHED_OTHER, sched_get_priority_min( SCHED_OTHER ) );
