...
* NOTES:
*
*  03/20/96  Data mismatch between send and receive NOT being detected
******************************************************************************/
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Bsend_err9: Mismatched data types between send and receive  ");
    ...
    if (MPITEST_nump < 2)
    {
	sprintf(info_buf, "Not enough ranks to test Bsend/Recv with mismatched data types");
	MPITEST_message(MPITEST_FATAL, info_buf);
    }

    sprintf(info_buf, "about to call MPI_Bsend w/  Bsend/Recv with mismatched data types between send and receive");
	...
	if (MPITEST_me == 0)
	{
	    ...
	    ierr = MPI_Bsend(float_send, 10, MPI_FLOAT, dest, 1234, MPI_COMM_WORLD);
	}
	...
	if (MPITEST_me == 1)
	{
	    ...
	    {
		fail++;
		sprintf(info_buf, "MPI_Bsend/Recv /w Mismatched data type on receive did not FAIL");
		...
		{
		    fail++;
		    sprintf(info_buf, "MPI_Bsend/Recv /w Mismatched data type on receive, returned  %d, expected MPI_ERR_TYPE",
			    errorclass);
		    MPITEST_message(MPITEST_NONFATAL, info_buf);
