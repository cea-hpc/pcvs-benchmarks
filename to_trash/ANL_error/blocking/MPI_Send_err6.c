...
{
    int
        buffsize,	/* Size, in bytes, of send buffer                    */
    ...
    sprintf(testname, "MPI_Send_err6:  Call with tag greater than max allowed");
     ...
     */
    sprintf(info_buf, "About to call MPI_Send w/  tag greater than MPI_TAG_UB attribute ( > %d)  Tag =  %d  ", *maxtag, messtag);
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Send w/ Call with tag greater than MPI_TAG_UB attribute (%d/%d) did not FAIL", *maxtag, messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Send /w  Call with tag greater than MPI_TAG_UB attribute returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
