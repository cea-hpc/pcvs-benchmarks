...
sprintf( testname, "MPI_Allgatherv_err2");
...
**  Invalid type:
*/
sprintf(info_buf, "about to call MPI_Allgatherv w/ invalid type");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
if (MPITEST_me != 0)
    ierr = MPI_Allgatherv(input, 1, MPI_DATATYPE_NULL, output, recv_counts,
	recv_displs, MPI_SHORT, MPI_COMM_WORLD);
else
    ierr = MPI_Allgatherv(input, 1, MPI_DATATYPE_NULL, output, recv_counts,
	recv_displs, MPI_REAL, MPI_COMM_WORLD);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Allgatherv w/ invalid type did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Allgatherv w/ invalid type returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
