...
sprintf( testname, "MPI_Allgatherv_err4");
...
*/
sprintf(info_buf, "about to call MPI_Allgatherv(MPI_COMM_NULL)");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Allgatherv(input, MPITEST_nump, MPI_INT, output, recv_counts, recv_displs, MPI_INT, MPI_COMM_NULL);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Allgatherv(MPI_COMM_NULL) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Allgatherv(MPI_COMM_NULL) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
