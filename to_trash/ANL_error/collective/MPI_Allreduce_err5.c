...
sprintf( testname, "MPI_Allreduce_err5");
...
*/
sprintf(info_buf, "about to call MPI_Allreduce(MPI_COMM_NULL)");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Allreduce(input, output, 1, MPI_INT, MPI_SUM, MPI_COMM_NULL);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Allreduce(MPI_COMM_NULL) did not FAIL");
  ...
  {
	fail++;
	sprintf(info_buf, "MPI_Allreduce(MPI_COMM_NULL) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
