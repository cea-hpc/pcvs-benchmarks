...
sprintf( testname, "MPI_Allreduce_err6");
...
sprintf(info_buf, "about to call MPI_Allreduce(Free comm)");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Allreduce(input, output, 1, MPI_INT, MPI_SUM, comm);

if (ierr == MPI_SUCCESS)
  {
    sprintf(info_buf, "MPI_Allreduce(Free Comm) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Allreduce(Free Comm) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
