...
sprintf( testname, "MPI_Alltoall_err2");
...
**  Invalid type:
*/
sprintf(info_buf, "about to call MPI_Alltoall w/ invalid type");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Alltoall(input, MPITEST_nump, MPI_DATATYPE_NULL, output,
	MPITEST_nump, MPI_DATATYPE_NULL, MPI_COMM_WORLD);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Alltoall w/ invalid type did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Alltoall w/ invalid type returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
