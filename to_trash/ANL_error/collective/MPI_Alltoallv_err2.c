...
sprintf( testname, "MPI_Alltoallv_err2");
...
**  invalid type:
*/
sprintf(info_buf, "about to call MPI_Alltoallv w/ invalid type");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Alltoallv(input, send_counts, send_displs, MPI_DATATYPE_NULL, output,
	recv_counts, recv_displs, MPI_DATATYPE_NULL, MPI_COMM_WORLD);

if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Alltoallv w/ invalid type did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_TYPE)
      {
	fail++;
	sprintf(info_buf, "MPI_Alltoallv w/ invalid type returned %d, expected MPI_ERR_TYPE",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
