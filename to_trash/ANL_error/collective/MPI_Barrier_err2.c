...
sprintf( testname, "MPI_Barrier_err2");
...
sprintf(info_buf, "about to call MPI_Barrier(Free comm)");
...
if (ierr == MPI_SUCCESS)
  {
    sprintf(info_buf, "MPI_Barrier(Free Comm) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Barrier(Free Comm) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
