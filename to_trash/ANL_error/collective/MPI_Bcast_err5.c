...
sprintf( testname, "MPI_Bcast_err5");
...
**  Invalid root
*/
sprintf(info_buf, "about to call MPI_Bcast w/ root = MPITEST_nump");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Bcast(input, 1, MPI_INT, MPITEST_nump, MPI_COMM_WORLD);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Bcast w/ root = %d did not fail", MPITEST_nump);
...
else
  {
    ...
    if (errorclass != MPI_ERR_ROOT)
      {
	fail++;
	sprintf(info_buf, "MPI_Bcast w/ root = %d returned %d, expected MPI_ERR_ROOT",
	    MPITEST_nump, errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
