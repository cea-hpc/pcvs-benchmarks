...
sprintf( testname, "MPI_Gather_err5");
...
**  Invalid root > # nodes
*/
sprintf(info_buf, "about to call MPI_Gather w/ root > # nodes");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Gather(input, 1, MPI_SHORT, output, MPITEST_nump, MPI_SHORT,
...
if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Gather w/ root > # nodes did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_ROOT)
      {
	fail++;
	sprintf(info_buf, "MPI_Gather w/ root > # nodes returned %d, expected MPI_ERR_ROOT",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
