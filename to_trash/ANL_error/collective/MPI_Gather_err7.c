...
sprintf( testname, "MPI_Gather_err7");
...
sprintf(info_buf, "about to call MPI_Gather(Free comm)");
...
if (ierr == MPI_SUCCESS)
  {
    sprintf(info_buf, "MPI_Gather(Free Comm) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Gather(Free Comm) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
