...
sprintf( testname, "MPI_Reduce_err1");
...
*/
#define xxx(ty, op)  \
sprintf(info_buf, "about to call MPI_Reduce w/ invalid op %d on %d",op,ty); \
MPITEST_message( MPITEST_INFO1, info_buf); \
ierr = 0; \
ierr = MPI_Reduce(input, output, 1, ty, op, 0, MPI_COMM_WORLD); \
if (ierr == MPI_SUCCESS) { \
    fail++; \
    sprintf(info_buf, "MPI_Reduce w/ invalid op %d on %d(%x) did not FAIL",op,ty,ty); \
    MPITEST_message(MPITEST_NONFATAL, info_buf); \
} else { \
    MPI_Error_class(ierr, &errorclass); \
    if (errorclass != MPI_ERR_OP) { \
	fail++; \
	sprintf(info_buf, "MPI_Reduce w/ invalid op %d on %d returned %d, expected MPI_ERR_OP", \
	    errorclass,op,ty); \
	MPITEST_message(MPITEST_NONFATAL, info_buf); \
