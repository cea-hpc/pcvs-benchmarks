...
sprintf( testname, "MPI_Reduce_err4");
...
**  Invalid size
*/
sprintf(info_buf, "about to call MPI_Reduce w/ size = -1");
...
if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Reduce w/ size = -1 did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COUNT)
      {
	fail++;
	sprintf(info_buf, "MPI_Reduce w/ size = -1 returned %d, expected MPI_ERR_COUNT",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
