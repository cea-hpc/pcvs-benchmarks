...
sprintf( testname, "MPI_Reduce_err5");
...
**  Invalid root
*/
sprintf(info_buf, "about to call MPI_Reduce w/ root = -1");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Reduce(input, output, 1, MPI_INT, MPI_SUM, -1, MPI_COMM_WORLD);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Reduce w/ root = -1 did not fail");
...
else
  {
    ...
    if (errorclass != MPI_ERR_ROOT)
      {
	fail++;
	sprintf(info_buf, "MPI_Reduce w/ root = -1 returned %d, expected MPI_ERR_ROOT",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
