...
sprintf( testname, "MPI_Reduce_err6");
...
**  Invalid root
*/
sprintf(info_buf, "about to call MPI_Reduce w/ root = MPITEST_nump");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Reduce(input, output, 1, MPI_INT, MPI_SUM, MPITEST_nump, MPI_COMM_WORLD);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Reduce w/ root = %d did not FAIL", MPITEST_nump);
...
else
  {
    ...
    if (errorclass != MPI_ERR_ROOT)
      {
	fail++;
	sprintf(info_buf, "MPI_Reduce w/ root = %d returned %d, expected MPI_ERR_ROOT",
