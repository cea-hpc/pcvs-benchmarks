...
sprintf( testname, "MPI_Reduce_err7");
...
*/
sprintf(info_buf, "about to call MPI_Reduce(MPI_COMM_NULL)");
MPITEST_message( MPITEST_INFO1, info_buf);

ierr = 0;
ierr = MPI_Reduce(input, output, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_NULL);


if (ierr == MPI_SUCCESS)
  {
    fail++;
    sprintf(info_buf, "MPI_Reduce(MPI_COMM_NULL) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Reduce(MPI_COMM_NULL) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
