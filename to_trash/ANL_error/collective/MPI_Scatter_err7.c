...
sprintf( testname, "MPI_Scatter_err7");
...
sprintf(info_buf, "about to call MPI_Scatter(Free comm)");
...
if (ierr == MPI_SUCCESS)
  {
    sprintf(info_buf, "MPI_Scatter(Free Comm) did not FAIL");
...
else
  {
    ...
    if (errorclass != MPI_ERR_COMM)
      {
	fail++;
	sprintf(info_buf, "MPI_Scatter(Free Comm) returned %d, expected MPI_ERR_COMM",
	    errorclass);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
