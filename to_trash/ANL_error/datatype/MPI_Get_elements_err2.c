...
sent.  It will then creat a new datatype using MPI_Type_contiguous()
followed by calling MPI_Get_elements() using the newly created but
not committed datatype.
...
MPI Calls dependencies for this test:
  ...
  MPI_Error_string(), MPI_Type_contiguous, MPI_Send(), MPI_Recv()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Get_elements_err2: uncommitted datatype");
	...
	MPITEST_message(MPITEST_FATAL, error_string);
      ...
      ierr = MPI_Type_contiguous(MPITEST_BUFSIZE, MPI_INT, &type);
      if (ierr != MPI_SUCCESS) {
	fail++;
	...
	MPITEST_message(MPITEST_FATAL, error_string);
      ...
      /* MPI_Get_elements() with uncommitted datatype */
      ierr2 = MPI_Get_elements(&status, type, &elmt_count);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Get_elements() with uncommitted datatype returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Get_elements() with uncommitted datatype returned error class %d, expected MPI_ERR_COUNT", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
