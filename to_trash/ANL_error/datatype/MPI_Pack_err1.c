...
Rank 0 will first call MPI_Pack with negative input count.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Pack_err1: Negative incount");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Pack() with negative count */
      ierr2 = MPI_Pack(inbuf, incount, type, outbuf, outsize, &position, MPI_COMM_WORLD);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Pack() with negative input count returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Pack() with negative input count returned error class %d, expected MPI_ERR_COUNT", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
