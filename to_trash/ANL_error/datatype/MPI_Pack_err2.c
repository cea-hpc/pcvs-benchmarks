...
Rank 0 will first call MPI_Pack with negative outsize.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Pack_err2: Negative outsize");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Pack() with negative outsize */
      ierr2 = MPI_Pack(inbuf, incount, type, outbuf, outsize, &position, MPI_COMM_WORLD);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Pack() with negative outsize returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  MPITEST_message(MPITEST_FATAL, error_string);
		 ...
		 (errorclass != MPI_ERR_ARG) &&
	         (errorclass != MPI_ERR_COUNT)) {
	  fail++;
	  sprintf(info_buf, "MPI_Pack() with negative outsize returned error class %d, expected MPI_ERR_OTHER or MPI_ERR_ARG", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
