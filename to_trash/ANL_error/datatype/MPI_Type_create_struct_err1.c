...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_create_struct_err1: Negative count");
	...
	types[i] = MPI_INT;
      ...
      /* MPI_Type_create_struct() with -ve count */
      ierr2 = MPI_Type_create_struct(count, blklen, displs, types, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_create_struct() with negative count returned MPI_SUCCESS", source);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_create_struct() with negative count returned error class %d, expected MPI_ERR_COUNT", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
