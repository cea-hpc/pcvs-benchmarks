...
4)  Call with input array of types
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_create_struct_err2: Negative block length");
	...
	types[i] = MPI_INT;
      ...
      types[count - 1] = MPI_INT;

      /* MPI_Type_create_struct() with -ve block length */
      ierr2 = MPI_Type_create_struct(count, blklen, displs, types, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_create_struct() with negative block length returned MPI_SUCCESS", source);
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_create_struct() with negative block length returned error class %d, expected MPI_ERR_ARG", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
