...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_hindexed_err2: Negative block length");
	...
	displs[1] = i * extent;
      ...
      /* MPI_Type_hindexed() with -ve block length */
      ierr2 = MPI_Type_hindexed(count, blklen, displs, oldtype, &newtype);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_hindexed() with negative block length returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_hindexed() with negative block length returned error class %d (%s), expected MPI_ERR_ARG", errorclass, MPITEST_GetErrName( errorclass) );
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
