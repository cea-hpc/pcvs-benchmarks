...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Type_ub()

This test verifies that the correct error is returned if MPI_Type_ub()
is called with an invalid argument.

MPI_Type_ub error tests
-----------------------------------
1)  Call with MPI_DATTYPE_NULL..................[MPI_ERR_TYPE]

In all cases, expect to receive appropriate error.

Rank 0 will first call MPI_Type_ub() with MPI_DATATYPE_NULL.
...
MPI Calls dependencies for this test:
  MPI_Type_ub(), MPI_Init(), MPI_Finalize(), MPI_Error_string(), 
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Type_ub_err1: MPI_DATATYPE_NULL");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Type_ub() with MPI_DATATYPE_NULL */
      ierr2 = MPI_Type_ub(oldtype, &sizeub);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Type_ub() with MPI_DATATYPE_NULL returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Type_ub() with MPI_DATATYPE_NULL returned error class %d, expected MPI_ERR_TYPE", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
