...
Rank 0 will first call MPI_Unpack with MPI_DATATYPE_NULL.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Unpack_err4: MPI_DATATYPE_NULL");
    ...
    if (MPITEST_me == source) {
      ...
      /* MPI_Unpack() with MPI_DATATYPE_NULL */
      ierr2 = MPI_Unpack(inbuf, insize, &position, outbuf, outcount, type, MPI_COMM_WORLD);

      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Unpack() with MPI_DATATYPE_NULL returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Unpack() with MPI_DATATYPE_NULL returned error class %d, expected MPI_ERR_TYPE", errorclass);
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
