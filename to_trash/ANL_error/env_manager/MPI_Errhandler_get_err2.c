...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                          Error test for MPI_Errhandler_get
*
*  Get Errhandler error tests
*  -----------------------------------
...

{
    ...
    sprintf(testname, "MPI_Errhandler_get_err2:  Use freed comm");
...
/*----------------------  Allocate and Free communicator -------------------*/
    ...
    /*
     * *  Attempt to Use freed communicator
     */
    sprintf(info_buf, "about to call MPI_Errhandler_get w/ freed communicator");
    MPITEST_message(MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Errhandler_get(comm, &errh);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Errhandler_get /w freed comm. did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Errhandler_get /w freed comm. returned %d, expected MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
