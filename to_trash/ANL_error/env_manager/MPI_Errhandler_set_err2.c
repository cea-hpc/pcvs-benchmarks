...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                          Error test for MPI_Errhandler_set
*
*  Set Errhandler error tests
*  -----------------------------------
* 1)  Call with MPI_COMM_NULL ........................... [MPI_ERR_COMM]
* 2)  Call with a freed communicator .................... [MPI_ERR_COMM]
* 3)  Call with MPI_ERRHANDLER_NULL...................... [MPI_ERR_ARG]
*-------------------------------------------------------------------------
*
...

{
    ...
    sprintf(testname, "MPI_Errhandler_set_err2:  Use freed comm.");
    ...
    /*
     * * Create an Errhandler
     */
    ierr = MPI_Errhandler_create(err1, &errh);
    if (ierr != MPI_SUCCESS)
    {
        sprintf(info_buf, "Non-zero return code (%d) from MPI_Errhandler_create", ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, error_string, &size);
        MPITEST_message(MPITEST_FATAL, error_string);
    }

/*----------------------  Allocate and Free communicator -------------------*/
    ...
    /*
     * *  Attempt to Use freed comm. as communicator:
     */
    sprintf(info_buf, "about to call MPI_Errhandler_set w/ freed comm.");
    MPITEST_message(MPITEST_INFO1, info_buf);

    ierr = 0;
    ierr = MPI_Errhandler_set(comm, errh);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Errhandler_set /w freed comm. did not FAIL");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
    }
    else
    {
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Errhandler_set /w freed comm. returned %d, expected MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
