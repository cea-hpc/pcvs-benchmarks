...
Rank 0 will call MPI_Attr_delete with MPI_KEYVAL_INVALID.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Attr_delete_err2: MPI_KEYVAL_INVALID");
    ...
    if (MPITEST_me == 0) {
      ...
      sprintf(info_buf, "Calling MPI_Attr_delete with MPI_KEYVAL_INVALID");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Attr_delete(MPI_COMM_SELF, MPI_KEYVAL_INVALID);
...
#  endif
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Attr_delete() with MPI_KEYVAL_INVALID returned error class %d, expected MPI_ERR_ARG or MPI_ERR_OTHER", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
