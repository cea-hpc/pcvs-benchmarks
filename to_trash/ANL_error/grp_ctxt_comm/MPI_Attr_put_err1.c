...
Rank 0 will call MPI_Attr_put with MPI_COMM_NULL.
...
Test history:
   ...
   2  08/21/96     brdavis      Forgot to change MPI_Attr_get to MPI_Attr_put
                                in original version.  Fixed.
******************************************************************************/
#include <limits.h>
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Attr_put_err1: MPI_COMM_NULL");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Attr_put with MPI_COMM_NULL */
      sprintf(info_buf, "Calling MPI_Attr_put with MPI_COMM_NULL");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Attr_put(MPI_COMM_NULL, keyval, &attribute_val);
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Attr_put() with MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
