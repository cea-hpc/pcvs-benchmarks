...
2)  Call with MPI_COMM_WORLD.............[MPI_ERR_COMM]
3)  Call with MPI_COMM_SELF..............[MPI_ERR_COMM]

In all cases, expect to receive appropriate error.

Rank 0 will call MPI_Comm_free with MPI_COMM_NULL.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Comm_free_err1: MPI_COMM_NULL");
	...
	if (ierr != MPI_SUCCESS) {
	  fail++;
	    ...
	    /* */ printf( "Error class was %d\n", errorclass );
	  fail++;
          sprintf(info_buf, "MPI_Comm_free() with MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
