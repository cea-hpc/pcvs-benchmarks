...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Comm_free()

This test verifies that the correct error is returned if MPI_Comm_free()
is called with invalid arguments.

MPI_Comm_free error tests
-----------------------------------
1)  Call with MPI_COMM_NULL.........[MPI_ERR_COMM]
2)  Call with MPI_COMM_WORLD........[MPI_ERR_COMM/MPI_ERR_ARG/MPI_ERR_OTHER]
3)  Call with MPI_COMM_SELF.........[MPI_ERR_COMM/MPI_ERR_ARG/MPI_ERR_OTHER]

In all cases, expect to receive appropriate error.

Rank 0 will call MPI_Comm_free with MPI_COMM_SELF.
...
MPI_ERR_COMM, or MPI_ERR_OTHER.

All other rank(s) will simply do nothing.

MPI Calls dependencies for this test:
  MPI_Comm_free(), MPI_Init(), MPI_Finalize()
  MPI_Error_string()
  [MPI_Allreduce(), MPI_Comm_free(), MPI_Comm_free()]
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Comm_free_err3: MPI_COMM_SELF");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Comm_free with MPI_COMM_SELF */
      sprintf(info_buf, "Calling MPI_Comm_free with MPI_COMM_SELF");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Comm_free(&comm);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Comm_free() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	else if ((errorclass != MPI_ERR_COMM) && (errorclass != MPI_ERR_ARG) && (errorclass != MPI_ERR_OTHER)) {
	  fail++;
          sprintf(info_buf, "MPI_Comm_free() with MPI_COMM_SELF returned error class %d, expected MPI_ERR_COMM, MPI_ERR_ARG, or MPI_ERR_OTHER", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
