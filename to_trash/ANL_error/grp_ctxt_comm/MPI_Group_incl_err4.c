...
Rank 0 will call MPI_Group_incl with negative rank.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_incl_err4: call with negative rank");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_incl with negative rank */
      sprintf(info_buf, "Calling MPI_Group_incl with negative rank");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_incl() with negative rank returned error class %d, expected MPI_ERR_RANK", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
