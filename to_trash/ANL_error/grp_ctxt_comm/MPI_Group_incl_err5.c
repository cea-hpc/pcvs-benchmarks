...
Rank 0 will call MPI_Group_incl with an out of range (posative) rank.
The resulting error code will then be checked and the corresponding
error class will be verified to make sure it is MPI_ERR_RANK.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_incl_err5: call with out of range (posative) rank");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_incl with out of range (posative) rank */
      sprintf(info_buf, "Calling MPI_Group_incl with out of range (posative) rank");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_incl() with out of range (posative) rank returned error class %d, expected MPI_ERR_RANK", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
