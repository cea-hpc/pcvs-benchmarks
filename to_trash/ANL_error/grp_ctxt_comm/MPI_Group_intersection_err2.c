...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Group_intersection()

This test verifies that the correct error is returned if MPI_Group_intersection()
is called with invalid arguments.

MPI_Group_intersection error tests
-----------------------------------
1)  Call with group1=MPI_GROUP_NULL..............[MPI_ERR_GROUP]
1)  Call with group2=MPI_GROUP_NULL..............[MPI_ERR_GROUP]

In all cases, expect to receive appropriate error.

Rank 0 will call MPI_Group_intersection with MPI_GROUP_NULL.
The resulting error code will then be checked and the corresponding
error class will be verified to make sure it is MPI_ERR_GROUP.

All other rank(s) will simply do nothing.

MPI Calls dependencies for this test:
  MPI_Group_intersection(), MPI_Comm_group(), MPI_Init(), MPI_Finalize()
  MPI_Error_string()
  [MPI_Allreduce(), MPI_Comm_rank(), MPI_Comm_size()]

Test history:
   1  08/02/96     brdavis      Original version
******************************************************************************/
#include <limits.h>
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_intersection_err2: group2=MPI_GROUP_NULL");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_intersection with group2=MPI_GROUP_NULL */
      sprintf(info_buf, "Calling MPI_Group_intersection with group2=MPI_GROUP_NULL");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Group_intersection(group1, group2, &newgroup);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Group_intersection() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	else if (errorclass != MPI_ERR_GROUP) {
	  fail++;
          sprintf(info_buf, "MPI_Group_intersection() with group2=MPI_GROUP_NULL returned error class %d, expected MPI_ERR_GROUP", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
