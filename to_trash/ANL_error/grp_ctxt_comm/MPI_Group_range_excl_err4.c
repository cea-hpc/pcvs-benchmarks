...
Rank 0 will call MPI_Group_range_excl with negative ranks implied in ranges.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_range_excl_err4: ranges[] implies out range ranks");
    ...
    ranges[0][RANGES_LAST] = MPITEST_nump;
    ranges[0][RANGES_STRIDE] = 1;

    if (MPITEST_me == 0) {
      /* Calling MPI_Group_range_excl with ranges[] implies out range ranks */
      sprintf(info_buf, "Calling MPI_Group_range_excl with ranges[] implies out range ranks");
	...
	/* Since it is the range array that has invalid entries, 
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_range_excl() with ranges[] implies out range ranks returned error class %d, expected MPI_ERR_RANK", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
