...
-----------------------------------------------------------------------------*/
/******************************************************************************
                     Error test for MPI_Group_range_excl()

This test verifies that the correct error is returned if MPI_Group_range_excl()
is called with invalid arguments.

MPI_Group_range_excl error tests
-----------------------------------
1)  Call with group=MPI_GROUP_NULL...................[MPI_ERR_GROUP]
2)  Call with negative n.............................[MPI_ERR_OTHER/MPI_ERR_ARG]
3)  Call with ranges[] implies negative ranks........[MPI_ERR_RANK]
4)  Call with ranges[] implies out range ranks.......[MPI_ERR_RANK]
...
Rank 0 will call MPI_Group_range_excl with posative stride and first>last.
...
MPI Calls dependencies for this test:
  MPI_Group_range_excl(), MPI_Init(), MPI_Finalize()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_range_excl_err6: posative stride and first>last");
    ...
    n = 1;
    /* ranges */
    ranges[0][RANGES_FIRST] = MPITEST_nump - 1;
    ranges[0][RANGES_LAST] = 0;
    ranges[0][RANGES_STRIDE] = 1;

    if (MPITEST_me == 0) {
      /* Calling MPI_Group_range_excl with posative stride and first>last */
      sprintf(info_buf, "Calling MPI_Group_range_excl with posative stride and first>last");
      MPITEST_message(MPITEST_INFO1, info_buf);

      ierr2 = MPI_Group_range_excl(group, n, ranges, &newgroup);
      if (ierr2 == MPI_SUCCESS) {
	fail++;
	sprintf(info_buf, "MPI_Group_range_excl() returned MPI_SUCCESS");
	MPITEST_message(MPITEST_NONFATAL, info_buf);
      }
      else {
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_range_excl() with posative stride and first>last returned error class %d, expected MPI_ERR_OTHER or MPI_ERR_ARG", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
