...
Rank 0 will call MPI_Group_range_incl with MPI_GROUP_NULL.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_range_incl_err1: MPI_GROUP_NULL");
    ...
    if (MPITEST_me == 0) {
      /* Calling MPI_Group_range_incl with MPI_GROUP_NULL */
      sprintf(info_buf, "Calling MPI_Group_range_incl with MPI_GROUP_NULL");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_range_incl() with MPI_GROUP_NULL returned error class %d, expected MPI_ERR_GROUP", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
