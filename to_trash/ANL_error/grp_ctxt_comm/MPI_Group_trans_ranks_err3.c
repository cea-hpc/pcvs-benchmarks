...
Rank 0 will call MPI_Group_translate_ranks with negative n.
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Group_translate_ranks_err3: negative n");
    ...
    if (MPITEST_me == 0) {
      ...
      sprintf(info_buf, "Calling MPI_Group_translate_ranks with n=-1");
	...
	if (ierr != MPI_SUCCESS) {
	  ...
	  fail++;
          sprintf(info_buf, "MPI_Group_translate_ranks() with negative n returned error class %d, expected MPI_ERR_OTHER or MPI_ERR_ARG", errorclass);
          MPITEST_message(MPITEST_NONFATAL, info_buf);
