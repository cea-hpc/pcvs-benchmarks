...
3)  removed
4)  removed
5)  Call with peer_comm not containing local and remote leaders...[MPI_ERR_ARG/MPI_ERR_COMM/MPI_ERR_OTHER/MPI_ERR_RANK]
6)  removed
...
#define TEST_NAME "MPI_Intercomm_create_err5"
#define TEST_DESCRIPTION "peer_comm not containing local and remote leaders"
#define EXPECTED_ERRORS "MPI_ERR_ARG or MPI_ERR_COMM or MPI_ERR_OTHER or MPI_ERR_RANK"
    ...
    */
    ...
    if(MPITEST_me < 2)
    {
      ...
      if (ierr2 == MPI_SUCCESS) {
        ...
        else if ((errorclass != MPI_ERR_ARG) && (errorclass != MPI_ERR_COMM) && (errorclass != MPI_ERR_OTHER) && (errorclass != MPI_ERR_RANK)){
          fail++;
