...
* NOTES:
*
*  03/20/96  NOTE:   MPI_Ibsend_OVERHEAD is the minimum buffer size
******************************************************************************/
...
int main(int argc, char *argv[])
{
    ...
    char
       *buff;     	/* Buffer pointer for malloc for Ibsend buffer       */
    ...
    sprintf(testname, "MPI_Ibsend_err10: Message larger than attached Ibsend buffer ");
    ...
    sprintf(info_buf, "about to call MPI_Ibsend w/ Message larger than attached Ibsend buffer ");
    ...
    ierr = MPI_Ibsend(float_send, 200 , MPI_FLOAT, dest, 1234, MPI_COMM_WORLD, &request);
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Ibsend /w  Message larger than attached Ibsend buffer (%d) did not FAIL", buffsize);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Ibsend /w  Message larger than attached Ibsend buffer returned %d (%s), expected MPI_ERR_BUFFER",
		    errorclass, MPITEST_GetErrName( errorclass ) );
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
