...

{
    ...
    sprintf(testname, "MPI_Ibsend_err2: Rank > group size");
    ...
    sprintf(info_buf, "about to call MPI_Ibsend w/  Rank larger than group size");
    ...
    /* Let all nodes do the Ibsend out of range  */
...
/*--------------------------------  Ibsend  ---------------------------------*/
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Ibsend /w Destination rank larger than group size (%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Ibsend /w Destination rank larger than group size returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
