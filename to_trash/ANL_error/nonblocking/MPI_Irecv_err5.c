...
* 9)  Try to receive a message larger than Irecv count .. [MPI_ERR_BUFFER]
*
*
...
*
* NOTES:
*       04/22/96  Avoid -1 for tag value, that is MPI_ANY_TAG
******************************************************************************/
#define  TEST_TYPE   "MPI_Irecv"
#define  PROG_NAME   "MPI_Irecv_err05"
#define  MESS01      "Call with Invalid TAG ( < 0 )"
#define  MESS02      "Call with TAG < 0 "
...

{
    ...
    */
    ...
    if(messtag == MPI_ANY_TAG) messtag = messtag -1;

    ierr = MPI_Irecv(output, 1, MPI_CHAR, source, messtag, \
		     MPI_COMM_WORLD, &recv_request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "%s /w %s (%d) did not FAIL", TEST_TYPE, MESS02, messtag);
