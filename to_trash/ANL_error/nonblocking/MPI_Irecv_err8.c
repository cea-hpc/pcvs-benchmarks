...
#define  PROG_NAME   "MPI_Irecv_err08"
#define  MESS01      "Use non-existant data type <MPI_DATATYPE_NULL>"
#define  MESS02      "Use non-existant data type <MPI_DATATYPE_NULL>"
...

{
   ...
   /*-------------------------------  Receive ------------------------------*/
    ...
    ierr = MPI_Irecv(output, messlength , MPI_DATATYPE_NULL , source, MPI_ANY_TAG, MPI_COMM_WORLD, &recv_request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "%s /w %s  did not FAIL", TEST_TYPE, MESS02);
