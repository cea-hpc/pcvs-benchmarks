...
*
* This test verifies that the correct error is returned if MPI_Bsend_init is
...
*-------------------------------------------------------------------------
*10)  Send a message larger than the attached buffer......[MPI_ERR_BUFFER]
...

{
    int
        ...
        dest,	        /* Destination of Bsend_init message                 */
    ...
    */
    ...
    sprintf(testname, "MPI_Bsend_init_err1:  Destination non-existent rank (rank negative)");
     ...
     */
    sprintf(info_buf, "about to call MPI_Bsend_init w/ Destination rank negative");
    MPITEST_message(MPITEST_INFO1, info_buf);

    /*--------------------- Allocate buffer for Bsend_init ----------------------*/
    buffsize = 1024;
    ...
    /* Let all nodes do the invalid Bsend_init  */

    dest = -1 * (MPITEST_me + 1);
    if (dest == MPI_PROC_NULL)	/* This is a valid value and may be = -1 */
    {
	dest = dest - 1;
    }

    ierr = MPI_Bsend_init(input, 1, MPI_CHAR, dest, 0, MPI_COMM_WORLD, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Bsend_init /w negative rank destination(%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Bsend_init /w negative rank destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
