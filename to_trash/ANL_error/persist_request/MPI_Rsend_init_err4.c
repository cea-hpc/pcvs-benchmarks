...
/*---------------------------------------------------------------------------*/
    ...
    sprintf(testname, "MPI_Rsend_init_err4:  Call with a freed communicator");
    ...
    */
     ...
     */
    sprintf(info_buf, "About to call MPI_Rsend_init w/ Call with a freed communicator");
    ...
    ierr = MPI_Rsend_init(input, 1, MPI_CHAR, dest, 0, comm, &request);

    if (ierr == MPI_SUCCESS)
    {
	fail++;
	sprintf(info_buf, "MPI_Rsend_init /w  Call with a freed communicator did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Rsend_init /w Call with a freed communicator returned %d, expected  MPI_ERR_COMM",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
