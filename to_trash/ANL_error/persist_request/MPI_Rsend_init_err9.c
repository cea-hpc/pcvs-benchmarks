...
* NOTES:
*
*  03/20/96  Data mismatch between Rsend_init and receive NOT being detected
******************************************************************************/
...
int main(int argc, char *argv[])
{
    ...
    float
        float_send[100];	/* Array of values to Rsend_init                 */
    ...
    sprintf(testname, "MPI_Rsend_init_err9: Mismatched data types between send and receive  ");
    ...
    if (MPITEST_nump < 2)
    {
	sprintf(info_buf, "Not enough ranks to test Rsend_init/Recv with mismatched data types");
	MPITEST_message(MPITEST_FATAL, info_buf);
    }

    sprintf(info_buf, "about to call MPI_Rsend_init w/  Rsend/Recv mismatched data types between send and receive");
	...
	if (MPITEST_me == 0)
	{
	    ...
	    ierr = MPI_Rsend_init(float_send, 10, MPI_FLOAT, dest, 1234, MPI_COMM_WORLD, &request);
	...
	if (MPITEST_me == 1)
	{
	    ...
	    {
		fail++;
		sprintf(info_buf, "MPI_Rsend_init/Recv /w Mismatched data type on receive did not FAIL");
		...
		{
		    fail++;
		    sprintf(info_buf, "MPI_Rsend_init/Recv /w Mismatched data type on receive, returned  %d, expected MPI_ERR_TYPE",
			    errorclass);
		    MPITEST_message(MPITEST_NONFATAL, info_buf);
