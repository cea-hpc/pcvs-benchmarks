...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                          Error test for MPI_Start
*
* This test verifies that the correct error is returned if MPI_Start
* called with an invalid argument.
*
*  MPI_Start error tests
*  --------------------------------------------------------
...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Start_err2:  Request freed");
    ...
    /*
     *  Do a start; expect failure.
    */
    ierr = MPI_Start(&request);
    if (ierr == MPI_SUCCESS)
    {
        fail++;
        sprintf(info_buf, "MPI_Start on freed request did not FAIL");
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    }
    else
    {
        ...
        {
	    fail++;
	    sprintf(info_buf, "MPI_Start on freed request returned %d, expected MPI_ERR_REQUEST",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    MPI_Error_string(ierr, error_string, &size);
	    MPITEST_message(MPITEST_NONFATAL, error_string);
