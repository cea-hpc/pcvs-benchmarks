...
Rank 0 will call MPI_Cancel() with MPI_REQUEST_NULL.

All other rank(s) will simply do nothing.

MPI Calls dependencies for this test:
  MPI_Cancel(), MPI_Init(), MPI_Finalize(), MPI_Error_string()
  MPI_Error_class()
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Cancel_err2: MPI_REQUEST_NULL");
    ...
    if (MPITEST_me == 0) {
       ...
       sprintf(info_buf, "Cancelling request MPI_REQUEST_NULL ...");
	  ...
	  fail++;
	  sprintf(info_buf, "MPI_Cancel() returned MPI_SUCCESS with MPI_REQUEST_NULL");
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
       }
       else {
	  ...
	  if (errorclass != MPI_ERR_REQUEST) {
	     fail++;
	     sprintf(info_buf, "MPI_Cancel() /w MPI_COMM_NULL returned error class %d, Expecting MPI_ERR_REQUEST (%d)",
