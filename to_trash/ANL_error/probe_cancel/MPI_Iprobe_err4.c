...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    sprintf(testname, "MPI_Iprobe_err4: MPI_COMM_NULL");
    ...
    if (ierr != MPI_SUCCESS) {
       ...
       source = 0;
       /* MPI_Iprobe call with the MPI_COMM_NULL */
       sprintf(info_buf, "Calling MPI_Iprobe() with MPI_COMM_NULL (%d) ...", MPI_COMM_NULL);
       MPITEST_message(MPITEST_INFO1, info_buf);

       /* Must pass with a valid tag */
       tag = 10;
       ierr2 = MPI_Iprobe(source, tag, MPI_COMM_NULL, &flag, &status);

       if (ierr2 == MPI_SUCCESS) {
	  fail++;
	  sprintf(info_buf, "MPI_Iprobe() with MPI_COMM_NULL returned MPI_SUCCESS");
	  MPITEST_message(MPITEST_NONFATAL, info_buf);
       }
       else {
	  ...
	  if (ierr != MPI_SUCCESS) {
	     ...
	     fail++;
	     sprintf(info_buf, "MPI_Iprobe() /w MPI_COMM_NULL returned error class %d, expected MPI_ERR_COMM",
		     errorclass);
	     MPITEST_message(MPITEST_NONFATAL, info_buf);
