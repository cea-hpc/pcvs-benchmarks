...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                   Error test for MPI_Sendrecv
*
* This test verifies that the correct error is returned if MPI_Sendrecv is
* called with an invalid argument.
*
*
*  Sendrecvs error tests
*  -----------------------------------
...
*-----------------------------------------------------------------------------
*  NOTE:
*       For the Sendrecv tests there are two versions of this test.
...
{
    int
        dest,	        /* Destination of Sendrecv message                   */
    ...
    int
        input[10],	/* input to Sendrecv                                 */
        output[10];     /* output from Sendrecv                              */
    ...
    */
    ...
    sprintf(testname, "MPI_Sendrecv_err1a: SEND Destination non-existent rank (rank negative)  ");
    ...
    /*
     * *  Destination rank negative:
     */
    sprintf(info_buf, "about to call MPI_Sendrecv w/ SEND Destination rank negative");
    ...
    /* Let all nodes do the invalid Sendrecv  */

    dest = -1 * (MPITEST_me + 1);

/* These are  valid values and may be = -1/-2, respectively */

     while(dest == MPI_PROC_NULL || dest == MPI_ANY_SOURCE )	
    {
	dest = dest - 1;
    }

    ierr = MPI_Sendrecv(input,
          		1, 
		        MPI_CHAR, 
		        dest, 
	                0, 
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv /w negative SEND rank  destination(%d) did not FAIL", dest);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv /w negative SEND rank destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
