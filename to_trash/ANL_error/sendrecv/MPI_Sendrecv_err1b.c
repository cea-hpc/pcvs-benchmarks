...
{
    int
        src,	        /* Source of Sendrecv message                        */
    ...
    */
    ...
    sprintf(testname, "MPI_Sendrecv_err1b: RECV Destination non-existent rank (rank negative)  ");
     ...
     */
    sprintf(info_buf, "about to call MPI_Sendrecv w/ RECV Destination rank negative");
...
/* These are  valid values and may be = -1/-2, respectively */
     ...
     while(src == MPI_PROC_NULL || src == MPI_ANY_SOURCE )	
    {
	src = src - 1;
    }
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv /w negative RECV rank  destination(%d) did not FAIL", src);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv /w negative RECV rank destination returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
