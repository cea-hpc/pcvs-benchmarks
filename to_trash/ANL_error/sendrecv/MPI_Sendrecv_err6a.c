...

{
    int
        i,              /* Utility index                                     */
        buffsize,	/* Size, in bytes, of Sendrecv buffer                */
        ...
        *maxtag,   	/* maximum tag value                                 */
        messtag;	/* tag value used in Sendrecv                        */
    ...
    MPI_Comm 
        comm;      	/* MPI communicators                                 */

    MPI_Status
        stat;           /* MPI Recv status                                   */
   /*------------------------------------------------------------------------*/
    ...
    sprintf(testname, "MPI_Sendrecv_err6a:  Call with SEND tag greater than max allowed");
    ...
    /*
     * *  Attempt to  Call with SEND tag greater than Attr_get<MPI_TAG_UB>:
     */
    sprintf(info_buf, "About to call MPI_Sendrecv w/ SEND tag greater than MPI_TAG_UB attribute ( > %d)  Tag =  %d  ", *maxtag, messtag);
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv w/ Call with SEND tag greater than MPI_TAG_UB attribute (%d/%d) did not FAIL", *maxtag, messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv /w  Call with SEND tag greater than MPI_TAG_UB attribute returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
