...

{
   ...
   /*------------------------------------------------------------------------*/
    ...
    sprintf(testname, "MPI_Sendrecv_err6b:  Call with RECV tag greater than max allowed");
     ...
     */
    sprintf(info_buf, "About to call MPI_Sendrecv w/  RECV tag greater than MPI_TAG_UB attribute ( > %d)  Tag =  %d  ", *maxtag, messtag);
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv w/ Call with RECV tag greater than MPI_TAG_UB attribute (%d/%d) did not FAIL", *maxtag, messtag);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv /w  Call with RECV tag greater than MPI_TAG_UB attribute returned %d, expected  MPI_ERR_TAG",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
