...

{
    ...
    sprintf(testname, "MPI_Sendrecv_err7a, message SEND length count < 0");
     ...
     */
    sprintf(info_buf, "about to call MPI_Sendrecv w/ SEND message length count < 0 ");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv /w negative message SEND length count (%d) did not FAIL", messlength);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv /w negative message SEND length count returned %d, expected MPI_ERR_COUNT",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
