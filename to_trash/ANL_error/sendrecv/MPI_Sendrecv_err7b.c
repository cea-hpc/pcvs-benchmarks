...

{
    ...
    sprintf(testname, "MPI_Sendrecv_err7b, message RECV length count < 0");
     ...
     */
    sprintf(info_buf, "about to call MPI_Sendrecv w/  RECV message length count < 0 ");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv /w negative message RECV length count (%d) did not FAIL", messlength);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv /w negative message RECV length count returned %d, expected MPI_ERR_COUNT",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
