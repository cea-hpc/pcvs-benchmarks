...

{
    ...
    sprintf(testname, "MPI_Sendrecv_err8b, Non-existant RECV data type <MPI_DATATYPE_NULL> ");
    ...
    sprintf(info_buf, "about to call MPI_Sendrecv w/ non-existant RECV data type <MPI_DATATYPE_NULL>");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv /w  non-existant RECV data type <MPI_DATATYPE_NULL>  did not FAIL");
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv /w  non-existant RECV data type <MPI_DATATYPE_NULL> returned %d, expected MPI_ERR_TYPE",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
