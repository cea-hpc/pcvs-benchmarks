...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Sendrecv_err9: Mismatched data types between send and receive  ");
    ...
    if (MPITEST_nump < 2)
    {
	sprintf(info_buf, "Not enough ranks to test Sendrecv with mismatched data types");
	MPITEST_message(MPITEST_FATAL, info_buf);
    }

    sprintf(info_buf, "about to call MPI_Sendrecv with mismatched data types between send and receive");
	...
	if (MPITEST_me == 0)
	{
	    ...
	    {
		fail++;
		sprintf(info_buf, "MPI_Sendrecv /w Mismatched data type did not FAIL");
		...
		{
		    fail++;
		    sprintf(info_buf, "MPI_Sendrecv /w Mismatched data type on receive, returned  %d, expected MPI_ERR_TYPE",
			    errorclass);
		    MPITEST_message(MPITEST_NONFATAL, info_buf);
