...

{
    ...
    */
    ...
    sprintf(testname, "MPI_Sendrecv_replace_err1b:  RECV Source non-existent rank (rank negative)  ");
    ...
    /*  Source rank negative   */

    sprintf(info_buf, "about to call MPI_Sendrecv_replace w/ RECV Source rank negative");
    ...
    {
	fail++;
	sprintf(info_buf, "MPI_Sendrecv_replace /w negative rank RECV Source(%d) did not FAIL", src);
	...
	{
	    fail++;
	    sprintf(info_buf, "MPI_Sendrecv_replace /w negative rank RECV Source returned %d, expected MPI_ERR_RANK",
		    errorclass);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
