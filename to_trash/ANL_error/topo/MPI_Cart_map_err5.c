...
#define  DIMENSIONS  1
#define  TEST_DESC   "MPI_Cart_map: specify more ranks than available "
...
int main(int argc, char *argv[])
{
   ...
   sprintf(testname, "MPI_Cart_map_err5:  Specify more ranks than available");
   ...
   if (comm_cart != MPI_COMM_NULL) {
	 ...
	 if (ierr == MPI_SUCCESS) {
	    ...
	    MPI_Error_class(ierr, &errorclass);
	    if (errorclass != MPI_ERR_DIMS && errorclass != MPI_ERR_RANK && errorclass != MPI_ERR_OTHER) {
	       fail++;
	       sprintf(info_buf, "%s  returned %d(%s), expected MPI_ERR_DIMS(%d) or MPI_ERR_RANK(%d) or MPI_ERR_OTHER(%d)", TEST_DESC, errorclass, MPITEST_GetErrName( errorclass ) , MPI_ERR_DIMS, MPI_ERR_RANK, MPI_ERR_OTHER);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	       MPI_Error_string(ierr, error_string, &size);
	    }
	    else {
