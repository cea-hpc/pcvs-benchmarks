...
****************************************************************************/
#define  DIMENSIONS      1
#define  TEST_DESC       "MPI_Cartdim_get:  call with graph communicator"
...
int main(int argc, char *argv[])
{
   ...
   int
      index[4],			/* cumulative graph node connetion vector            */
      edges[6],			/* node connection vector                            */
   ...
   MPI_Comm
      comm_cart,		/* Cartesin communicator                             */
   ...
   sprintf(testname, "MPI_Cartdim_get_err3:  Call with graph communicator");
   ...
   if (comm_graph != MPI_COMM_NULL) {
	 ...
	 ierr = MPI_Cartdim_get(comm_graph,
				&cart_dims);
