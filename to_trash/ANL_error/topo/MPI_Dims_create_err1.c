...
/******************************************************************************
 ...
 Reference:  MPI Standard,  Chapter 6:  Process Topologies
                            Section 6.5.2:  Cartesian Convenience Function
...
#define  DIMENSIONS     4
#define  MAXNODE      120
#define  TEST_DESC     "MPI_Dims_create_err1:  nodes not a multiple of dimensions"
...
int main(int argc, char *argv[])
{
   ...
   sprintf(testname, "MPI_Dims_create_err1:  nodes not a multiple of dimensions");
   ...
   if (ierr == MPI_SUCCESS) {
	 ...
	 fail++;
	 sprintf(info_buf, "%s  returned %d, expected MPI_ERR_DIMS(%d) or MPI_ERR_OTHER(%d) ", TEST_DESC, errorclass, MPI_ERR_DIMS, MPI_ERR_OTHER);
	 MPITEST_message(MPITEST_NONFATAL, info_buf);
	 MPI_Error_string(ierr, error_string, &size);
