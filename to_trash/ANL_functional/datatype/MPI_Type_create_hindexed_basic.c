...
All rank will call MPI_Type_create_hindexed() and then MPI_Type_commit() to create
...
The input count to MPI_Type_create_hindexed() is determined by each message length
in mpitest_cfg.h and the input oldtype to MPI_Type_create_hindexed() is determined
...
MPI Calls dependencies for this test:
  ...
  MPI_Type_create_hindexed(), MPI_Type_commit(), MPI_Type_free(),
...
int main(int argc, char *argv[])
{
   ...
   sprintf(testname, "MPI_Type_create_hindexed_basic");
	 ...
	 for (type_count=0; type_count < MPITEST_num_datatypes(); type_count++) {
	    ...
	    for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
		  ...
		  displs[i] = i * extent;
	       ...
	       ierr = MPI_Type_create_hindexed(length, blklens, displs, MPITEST_mpi_datatypes[test_type], &newtype);
	       if (ierr != MPI_SUCCESS) {
		  fail++;
