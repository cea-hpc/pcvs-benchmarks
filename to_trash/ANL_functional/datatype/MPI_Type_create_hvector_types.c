...
(in mpitest_cfg.h) using MPI_Type_create_hvector().  The root's rank will then send
...
/*   struct dataTemplate
   ...
   sprintf(testname, "MPI_Type_create_hvector_types");
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    ierr = MPI_Type_create_hvector(length, 1, extent, oldtype, &newtype);
	    if (ierr != MPI_SUCCESS) {
	       fail++;
	       sprintf(info_buf, "MPI_Type_hvector() returned %d", ierr);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	       MPI_Error_string(ierr, error_string, &size);
	       MPITEST_message(MPITEST_FATAL, error_string);
	    }
