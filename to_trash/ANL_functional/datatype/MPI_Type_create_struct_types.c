...
-----------------------------------------------------------------------------*/
/******************************************************************************
                          Test for MPI_Type_create_struct()
...
/*   struct dataTemplate
   ...
   signed char *buffer; /* message buffer (not all compilers make signed the
                           default, and the tests assumed signed */
   ...
   sprintf(testname, "MPI_Type_create_struct_types");
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 ierr = MPI_Type_create_struct(basic_types, blklens, displs, types, &newtype);
	 if (ierr != MPI_SUCCESS) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    for (root=0; root<test_nump; root++) {
		  ...
		  /* The original test assumed that char was signed.
		     Under some compilers, such as IRIX, this is not true,
		     and the test must work in this case.  This was fixed
		     by making buffer a signed char */
