...
int main(int argc, char *argv[])
{
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    ub = displs[num_types - 1] + 
			type_sizes[num_types - 1];
	    ...
	    if (ierr != MPI_SUCCESS) {
	       ...
	       fail++;
	       sprintf(info_buf, "MPI_Type_ub() returned unexpected displacement  Expected: >= %d, Actual: " MPITEST_AINT_FMT_DEC_SPEC, ub, displ);
	       MPITEST_message(MPITEST_NONFATAL, info_buf);
	    }
