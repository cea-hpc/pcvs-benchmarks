...
void makebuf(void)
{
	...
	if ((buffer1 == 0) || (buffer2 == 0)) {
		sprintf(info_buf, "not enough memory for buffer %lu",
			maxlength + ALIGN + 1);
		MPITEST_message(MPITEST_FATAL, info_buf);
		MPI_Abort(MPI_COMM_WORLD, 1);
		exit(1);
	}
	buffer1 = (char *)((((unsigned long)buffer1) + ALIGN-1) & ~(ALIGN-1));
	buffer2 = (char *)((((unsigned long)buffer2) + ALIGN-1) & ~(ALIGN-1));
	...
	sprintf(info_buf, "Buffer address %p  length %lu",
		buffer1, maxlength);
	MPITEST_message(MPITEST_INFO1, info_buf);
}
