   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) {
	    ...
	    for (root=0; root<test_nump; root++) {
		  ...
		  sprintf(info_buf, "Constructing persistent send request from  source %d to %d, tag: %d", root, dest, tag);
		  MPITEST_message(MPITEST_INFO1, info_buf);

		  ierr = MPI_Send_init(buffer, length, MPITEST_mpi_datatypes[test_type],
				       dest, tag, comm, &send_request);
		  if (ierr != MPI_SUCCESS) {
		     fail++;
		     sprintf(info_buf, "MPI_Send_init() returned %d", ierr);
		     MPITEST_message(MPITEST_NONFATAL, info_buf);
		     MPI_Error_string(ierr, error_string, &size);
		     MPITEST_message(MPITEST_FATAL, error_string);
		  }
		  else {
		     sprintf(info_buf, "Starting send request from source %d to %d, tag: %d", root, dest, tag);
		     MPITEST_message(MPITEST_INFO1, info_buf);
			...
			MPITEST_message(MPITEST_FATAL, error_string);
		     ...
		     sprintf(info_buf, "Cancelling persistent send request ...");
		     MPITEST_message(MPITEST_INFO1, info_buf);
		  ...
		  ierr = MPI_Isend(buffer2, length, MPITEST_mpi_datatypes[test_type],
				   dest, tag, comm, &send_request);
