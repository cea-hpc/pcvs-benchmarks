...
root_stat;
							 ...
							 * communicator */
	{
		...
		MPITEST_get_buffer(test_type, max_length, &node_recv_buffer);
		MPITEST_get_buffer(test_type, max_length, &node_send_buffer);
		MPITEST_get_buffer(test_type, max_length, &root_recv_buffer);
		MPITEST_get_buffer(test_type, max_length, &root_send_buffer);
		...
		for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++)
		{
		    ...
		    root_send_length = MPITEST_byte_to_element(test_type, byte_length);
			...
			for (root = 0; root < senders; root++)
			{
			    ...
			    if (print_node_flag)
			    {
				sprintf(info_buf, "(%d,%d,%d) node_recv_length %d commsize %d commtype %d data_type %d root %d",length_count, comm_count, type_count, node_send_length, test_nump, comm_type,
					test_type, root);
				...
				MPITEST_init_buffer(test_type, node_recv_length + 1,node_value, node_recv_buffer);

				MPITEST_dataTemplate_init(&node_value, MPITEST_current_rank);
				MPITEST_init_buffer_inc(test_type, node_send_length, node_value, node_send_buffer);
				loop_cnt++;
				loop_fail = 0;
				...
				error = MPITEST_buffer_errors(test_type, node_recv_length, node_value, node_recv_buffer);

				/* check for receive buffer overflow */
				MPITEST_dataTemplate_init(&node_value, -1);
				error += MPITEST_buffer_errors_ov(test_type, node_recv_length, node_value, node_recv_buffer);

				if (error)
				{
				    sprintf(info_buf, "%d errors in buffer (%d,%d,%d) len %d commsize %d commtype %d data_type %d root %d", error, length_count, comm_count, type_count, node_recv_length, test_nump, comm_type, test_type, root);
				    MPITEST_message(MPITEST_NONFATAL, info_buf);
				    loop_fail++;
				}
				else
				...
				if (error)
				{
				    sprintf(info_buf, "Send/Receive lengths differ - Sender(node/length)=%d/%d,  Receiver(node/length)=%d/%d", node_stat.MPI_SOURCE, node_recv_length, MPITEST_current_rank, node_cnt_len);
				    MPITEST_message(MPITEST_NONFATAL, info_buf);
				    loop_fail++;
				}
				...
				MPITEST_dataTemplate_init(&root_value, MPITEST_current_rank);

				/* Initialize send buffer */
				MPITEST_init_buffer(test_type, root_send_length+1, root_value, root_send_buffer);
				/* Initialize the receive buffer to -1 */

				MPITEST_dataTemplate_init(&root_value, -1);
				MPITEST_init_buffer(test_type, root_recv_length + 1, root_value, root_recv_buffer);
				send_to = test_nump;
				if (inter_comm)
				    send_to = receivers;
				for (i = 0; i < send_to; i++)
				{
				    ...
				    loop_fail = 0;
				      ...
				      root_recv_buffer,
        			    ...
        			    if (ierr != MPI_SUCCESS)
				    {
	       			        sprintf(info_buf, "Non-zero return code (%d) from root MPI_Sendrecv", ierr);
				    ...
				    error = MPITEST_buffer_errors_inc(test_type,
						  root_recv_length, root_value, root_recv_buffer);
				    ...
				    if (error)
				    {
					sprintf(info_buf, "%d errors in buffer (%d,%d,%d) len %d commsize %d commtype %d data_type %d root %d", error, length_count, comm_count, type_count, root_recv_length, test_nump, comm_type, test_type, root);
					MPITEST_message(MPITEST_NONFATAL, info_buf);
					loop_fail++;
				    }
				    else
				     ...
				     */
				    error = root_recv_length - root_cnt_len;

				    if (error)
				    {
					sprintf(info_buf, "Isend/Irecv lengths differ - Sender(node/length)=%d/%d,  Receiver(node/length)=%d/%d", root_stat.MPI_SOURCE, root_recv_length, MPITEST_current_rank, root_cnt_len);
					MPITEST_message(MPITEST_NONFATAL, info_buf);
					loop_fail++;
				    }
