   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    for (root=0; root<test_nump; root++) {
		  ...
		  sprintf(info_buf, "Sending from source: %d, tag: %d", MPITEST_current_rank, tag);
		  MPITEST_message(MPITEST_INFO1, info_buf);
		  ...
		  do {
		     ierr = MPI_Iprobe(MPI_ANY_SOURCE, tag, comm, &flag, &status);
		     if (ierr != MPI_SUCCESS) {
			fail++;
			sprintf(info_buf, "MPI_Iprobe() returned %d, status.MPI_ERROR = %d",
