	    ...
	    for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++) {
		  ...
		  if (has_op(op_count, test_type) && (length != 0)) {
		     ...
		     ierr = MPI_Reduce_scatter(send_buffer, recv_buffer,
				   counts, MPITEST_mpi_datatypes[test_type],
				       MPITEST_default_ops[op_count], comm);

		     if (ierr != MPI_SUCCESS) {
			sprintf(info_buf, "MPI_Reduce_scatter() returned %d", ierr);
			MPITEST_message(MPITEST_NONFATAL, info_buf);
			MPI_Error_string(ierr, error_string, &size);
			MPITEST_message(MPITEST_FATAL, error_string);
			fail++;
		     ...
		     /* error test */
		     error = 0;
		     error = MPITEST_buffer_errors(test_type, length, value, recv_buffer);
