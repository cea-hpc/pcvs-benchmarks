...
int main(int argc, char *argv[])
{
   ...
   for (comm_count = 0; comm_count < MPITEST_num_comm_sizes(); comm_count++) {
	 ...
	 for (type_count = 0; type_count < MPITEST_num_datatypes(); type_count++) {
	    ...
	    MPITEST_get_buffer(test_type, (max_length + EXTRA_SPACE), &recv_buffer);
	    ...
	    for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++) {
		  ...
		  for (i = 0, j = 0; i < test_nump; i++, j = 1 - j) {
		     ...
		     MPITEST_dataTemplate_init(&values[2 * i], root + i);
		  ...
		  if (ierr != MPI_SUCCESS) {
		     sprintf(info_buf, "MPI_Scatterv() returned %d", ierr);
		     MPITEST_message(MPITEST_NONFATAL, info_buf);
		     MPI_Error_string(ierr, error_string, &size);
		     MPITEST_message(MPITEST_FATAL, error_string);
		     fail++;
		  ...
		  error = 0;
		  error = MPITEST_buffer_errors(test_type, length + j, value,
						recv_buffer);

		  /* check for recv_buffer overflow */
		  MPITEST_dataTemplate_init(&value, -1);
		  error += MPITEST_buffer_errors_ov(test_type,
					    length + j, value, recv_buffer);
