...
, ierr);
	...
	for (i = 0; i < 4*NUMMESG; i++)
	{
	    ...
	    if (ierr != MPI_SUCCESS)
	    {
		sprintf(info_buf, "Non-zero return code ierr/flag (%d/%d) from MPI_Testall on send, after %d calls to MPI_Testany", ierr, flag, send_loop_cnt);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		MPI_Error_string(ierr, error_string, &size);
		MPITEST_message(MPITEST_NONFATAL, error_string);
		fail++;
	    ...
	    sprintf(info_buf,"Testany on send:  Index %d   Error %d  #Testany loops/calls %d/%d ", index, ierr, i, send_loop_cnt );
...
, ierr);
	...
	for (i = 0; i < 4*NUMMESG; i++)
	{
	    ...
	    sprintf(info_buf,"Testany on receive:  Index %d    Error %d ", index, ierr, recv_loop_cnt );
	    MPITEST_message(MPITEST_INFO1, info_buf);
