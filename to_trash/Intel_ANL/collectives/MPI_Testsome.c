...
, ierr);
	...
	ierr = MPI_Testsome(4*NUMMESG, recv_request, &recv_cnt, recv_index, recv_stat);

	total_cnt++;
	if (ierr != MPI_SUCCESS)
	{
	    sprintf(info_buf, "Non-zero return code (%d) from second MPI_Testsome on receive", ierr);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	...
	sprintf(info_buf, "Testsome on receive: Loop %d  recv_cnt %d  Error %d ", i, recv_cnt, ierr);
	MPITEST_message(MPITEST_INFO1, info_buf);

	if (recv_cnt != MPI_UNDEFINED)
	{
	    sprintf(info_buf, "Second MPI_Testsome on receive has indicated %d active messages and there should be none", recv_cnt);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	    fail++;
	}
	...
	for (i = 0; i < 4*NUMMESG; i++)
	{
	    ...
	    {
		sprintf(info_buf, "recv_request[%d] not equal to  MPI_REQUEST_NULL(%d/%d) after second Testsome", i, recv_request[i], MPI_REQUEST_NULL);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		MPI_Error_string(ierr, error_string, &size);
		MPITEST_message(MPITEST_NONFATAL, error_string);
		fail++;
