...
, ierr);
	...
	if (ierr != MPI_SUCCESS)
	{
	    sprintf(info_buf, "Non-zero return code (%d) from second MPI_Waitall on receive", ierr);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	...
	for (i = 0; i < 4*NUMMESG; i++)
	{
	    ...
	    {
		sprintf(info_buf, "recv_request[%d] not equal to  MPI_REQUEST_NULL(%d/%d) after second Waitall", i, recv_request[i], MPI_REQUEST_NULL);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		MPI_Error_string(ierr, error_string, &size);
		MPITEST_message(MPITEST_NONFATAL, error_string);
		fail++;
