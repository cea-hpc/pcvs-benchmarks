	...
	*attr,
	...
	if (MPITEST_current_rank != MPI_UNDEFINED)
	{
	    ...
	    ierr = MPI_Comm_split(comm, color,
			2*test_nump - MPITEST_current_rank, &comm2);
	    if (ierr != MPI_SUCCESS)
	    {
		...
		{
		    sprintf(info_buf, "MPI_Comm_compare returned %d, expected MPI_SIMILAR (comm_index %d)", result, comm_index);
		    MPITEST_message(MPITEST_NONFATAL, info_buf);
		    fail++;
		}
	    }
	    ...
	    {
		sprintf(info_buf, "MPI_Comm_size returned %d, expected %d (comm_index %d)", resultsplit, test_nump, comm_index);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		fail++;
	    }
	    loop_cnt++;
		...
		sprintf(info_buf, "MPI_Comm_rank returned %d, expected %d (comm_index %d)",
			resultsplit, test_nump-MPITEST_current_rank-1, comm_index);
		MPITEST_message(MPITEST_NONFATAL, info_buf);
		fail++;
	    }
	    loop_cnt++;
