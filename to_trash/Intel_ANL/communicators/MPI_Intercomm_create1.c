	...
	*attr,		/* Returned value of the attribute */
	...
	if (MPITEST_current_rank != MPI_UNDEFINED)
	{
	    ...
	    /* Now combine the two new communicators to create an
	       intercommunicator */
	    ierr = MPI_Intercomm_create(commsplit, 0, comm, 1-color, 27, &comminter);
	    if (ierr != MPI_SUCCESS)
	    {
		...
		if ((recv1 != send1) || (recv2 != send2) || 
		    (recv3 != send3))
		{
		    sprintf(info_buf, "Receive data corrupted by MPI_Intercomm_create: %d/%d/% (expected 19/29/39) (commm_index %d)", 
