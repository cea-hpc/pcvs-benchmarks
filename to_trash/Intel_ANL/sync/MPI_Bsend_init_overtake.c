...
int main(int argc, char *argv[])
{
	...
	if (MPITEST_me < 2)
	{
		...
		{	/* sender */
			...
			buffsize = ((LONGLENGTH + NUMSHORT + (NUMSHORT + 1) * MPI_BSEND_OVERHEAD) *2*sizeof(char));
			...
			ierr = MPI_Bsend_init(&buffer, LONGLENGTH, MPI_CHAR, 1, 1, MPI_COMM_WORLD, &send_request[0]);
			if (ierr != MPI_SUCCESS)
			{
			...
			ierr = MPI_Bsend_init(&shortb[0], 1, MPI_CHAR, 1, 1, MPI_COMM_WORLD, &send_request[1]);
		...
		{	/* receiver */
			...
			MPI_Barrier(MPI_COMM_WORLD);
			ierr = MPI_Recv_init(&buffer, LONGLENGTH, MPI_CHAR, 0, 1, MPI_COMM_WORLD, &recv_request[0]);
			if (ierr != MPI_SUCCESS)
			{
			...
			for (i = 0; i < NUMSHORT; i++)
			{
				ierr = MPI_Recv_init(&shortb[i], 1, MPI_CHAR, 0, 1, MPI_COMM_WORLD, &recv_request[i+1]);
				if (ierr != MPI_SUCCESS)
				{
			...
			*/
			...
			loop_cnt = 1 + NUMSHORT;
			MPITEST_dataTemplate_init(&value, 0);
			errors = MPITEST_buffer_errors_inc(MPITEST_char, LONGLENGTH, value, &buffer);
			if (errors)
			{
			...
			MPITEST_dataTemplate_init(&value, 1);
			errors = MPITEST_buffer_errors_inc(MPITEST_char, NUMSHORT, value, &shortb);
			if (errors)
			{
