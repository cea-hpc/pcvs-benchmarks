...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Bsend_init:  Persistent root buffered send to all others");
	...
	if (MPITEST_current_rank != MPI_UNDEFINED)
	{
		...
		{	/* Do for left group of INTER-communicator */
		    ...
		    {	/* Do for node zero of left bank of
			 * INTER-communicator */
			print_node_flag = 1;
		...
		for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++)
		{
			...
			for (root = 0; root < senders; root++)
			{
				...
				for (i = 0; i < send_to; i++)
				{
				    ...
				    while (!flag)
				    {
					ierr = MPI_Test(&send_request, &flag, &send_stat);
				    }
				    if (ierr != MPI_SUCCESS)
				    {
					sprintf(info_buf, "Non-zero return code (%d) from MPI_Test on Start for Bsend_init", ierr);
					MPITEST_message(MPITEST_NONFATAL, info_buf);
					MPI_Error_string(ierr, error_string, &size);
					MPITEST_message(MPITEST_NONFATAL, error_string);
					loop_fail++;
				    ...
				    if (ierr != MPI_SUCCESS)
				    {
					sprintf(info_buf, "Non-zero return code (%d) from MPI_Request_free after send", ierr);
					MPITEST_message(MPITEST_NONFATAL, info_buf);
					MPI_Error_string(ierr, error_string, &size);
					MPITEST_message(MPITEST_NONFATAL, error_string);
					fail++;
			    ...
			    if (((!inter_comm) ||
				(inter_comm && MPITEST_inter == recv_group)) && MPITEST_current_rank != root)
				...
				if (ierr != MPI_SUCCESS)
				{
				    sprintf(info_buf, "Non-zero return code (%d) from MPI_Request_free after receive", ierr);
				    MPITEST_message(MPITEST_NONFATAL, info_buf);
				    MPI_Error_string(ierr, error_string, &size);
				    MPITEST_message(MPITEST_NONFATAL, error_string);
				    fail++;
