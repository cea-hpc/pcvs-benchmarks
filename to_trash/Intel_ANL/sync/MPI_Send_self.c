...
int main(int argc, char *argv[])
{
            ...
            (MPITEST_inter == MPITEST_NOT_INTER))
	{
		...
		for (type_count = 0; type_count < MPITEST_num_datatypes(); type_count++)
		{
			...
			fprintf(stderr,"%d envoie un message type %d, length %d\n", MPITEST_current_rank, MPITEST_mpi_datatypes[test_type], length); 
			...
			fprintf(stderr,"%d recoit un message type %d, length %d\n", MPITEST_current_rank, MPITEST_mpi_datatypes[test_type], length); 
			ierr = MPI_Recv(recv_buffer, length,
				     MPITEST_mpi_datatypes[test_type],
				     MPITEST_current_rank, 1, comm, &recv_stat);
