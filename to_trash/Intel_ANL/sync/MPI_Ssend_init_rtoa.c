...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Ssend_init_rtoa:  Persistent synchronous root send to all");
	...
	if (MPITEST_current_rank != MPI_UNDEFINED)
	{
		...
		for (length_count = 0; length_count < MPITEST_num_message_lengths(); length_count++)
		{
			...
			for (root = 0; root < senders; root++)
			{
				...
				if (ierr != MPI_SUCCESS)
				{
				    sprintf(info_buf, "Non-zero return code (%d) from MPI_Start for MPI_Recv_init", ierr);
				    MPITEST_message(MPITEST_NONFATAL, info_buf);
				    MPI_Error_string(ierr, error_string, &size);
				    MPITEST_message(MPITEST_NONFATAL, error_string);
				    loop_fail++;
