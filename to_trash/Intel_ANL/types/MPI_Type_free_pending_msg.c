   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 for (length_count=0;length_count<MPITEST_num_message_lengths();length_count++) { 
	    ...
	    for (root=0; root<test_nump; root++) {
		  ...
		  sprintf(info_buf, "Waiting for recv request to be completed");
		  MPITEST_message(MPITEST_INFO1, info_buf);
		  ...
		  if (error + error2) {
	       ...
	       if ((MPITEST_current_rank != root) && (MPITEST_current_rank != dest)) {
		  ...
		  if (ierr != MPI_SUCCESS) {
	       ...
	       /* The code here is erroneous because there is no wait on the
		  request before freeing the send buffer.  This code 
		  fixes that */
	       if (MPITEST_current_rank == root) {
