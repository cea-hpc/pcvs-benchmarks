...
int main(int argc, char *argv[])
{
   ...
   for (comm_count=0; comm_count<MPITEST_num_comm_sizes();comm_count++) {
	 ...
	 if (type_size != 0) {
	    fail++;
	    sprintf(info_buf, "MPI_Type_size() w/ MPI_LB returned unexpected Expected: %d, Actual: %d", 0, type_size);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	 }
	 ...
	 if (type_size != 0) {
	    fail++;
	    sprintf(info_buf, "MPI_Type_size() w/ MPI_UB returned unexpected Expected: %d, Actual: %d", 0, type_size);
	    MPITEST_message(MPITEST_NONFATAL, info_buf);
	 }
      }
